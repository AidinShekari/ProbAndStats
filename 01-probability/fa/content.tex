...
نمونه‌برداری از ولتاژ خروجی \lr{ADC} & پردازش سیگنال & کوانتیزه‌سازی + نویز \\
\end{longtable}
}
...
\tightlist
\item
  \lr{A \ensuremath{\cup} B \ensuremath{\cup} C} = «برد دست‌کم یک عیب دارد»
\item
  \lr{A \ensuremath{\cap} B} = «برد هم عیب پل لحیم و هم یک قطعه مفقود دارد»
\item
...
\textbf{اصل موضوعه 3 (جمع‌پذیری شمارا):}\PSbr{}برای رویدادهای ناسازگار متقابل \lr{A\textsubscript{1}}، \lr{A\textsubscript{2}}، \lr{A\textsubscript{3}}، \lr{…}:\PSdisplay{P(A_1 \cup A_2 \cup A_3 \cup \cdots) = P(A_1) + P(A_2) + P(A_3) + \cdots}
...
\emph{معنای مهندسی:} هنگام محاسبه احتمال «خرابی \lr{A} یا خرابی \lr{B}»، باید هم‌پوشانی‌ای را که در آن هر دو به‌طور هم‌زمان خراب می‌شوند، کم کنیم.
...
برای رویدادهای ناسازگار متقابل \lr{(A \ensuremath{\cap} B = \ensuremath{\emptyset})}:\PSdisplay{P(A \cup B) = P(A) + P(B)}
...
به‌طور معادل: \lr{P(A\textbar{}B) = P(A)} \lr{—} دانستن اینکه \lr{B} رخ داده است، هیچ اطلاعاتی درباره \lr{A} نمی‌دهد.
...
\lr{\PSno{}} «رویدادهای ناسازگار متقابل مستقل هستند»
...
\lr{P(\rl{خطای بسته}) = P(error\textbar{}path 1)P(path 1) + P(error\textbar{}path 2)P(path 2) + P(error\textbar{}path 3)P(path 3)}\PSbr{}= \lr{(0.01)(0.50) + (0.05)(0.30) + (0.10)(0.20)}\PSbr{}= \lr{0.005 + 0.015 + 0.020 = 0.040}
...
\lr{P(\rl{معیوب}) = P(\rl{معیوب\lr{\textbar{}}}A)P(A) + P(\rl{معیوب\lr{\textbar{}}}B)P(B)}\PSbr{}= \lr{(0.02)(0.60) + (0.05)(0.40)}\PSbr{}= \lr{0.012 + 0.020 = 0.032}
...
\item
  سیستم در حال کار = \lr{NOT(\rl{هر دو خراب شوند})} = دست‌کم یکی کار می‌کند.\PSbr{}\lr{P(P1 failed \textbar{} system works) = P(P1 failed \ensuremath{\cap} P2 works) / P(system works)}\PSbr{}= \lr{(0.02)(0.97) / (1 - 0.0006) = 0.0194 / 0.9994 \ensuremath{\approx} 0.0194}
\end{enumerate}
...
\tightlist
\item
  \lr{P(\rl{عدم وجود خطا در بسته})} چقدر است؟
\item
  \lr{P(\rl{دقیقاً یک خطا})} چقدر است؟
\item
  \lr{P(\rl{دست‌کم یک خطا})} چقدر است؟
\item
...
\tightlist
\item
  اگر حسگرها مستقل باشند، \lr{P(\rl{هر دو هشدار کاذب می‌دهند})} چقدر است؟
\item
  اگر حسگرها یک منبع نویز مشترک داشته باشند و \lr{P(\rl{هر دو هشدار کاذب}) = 0.01} باشد، آیا مستقل هستند؟
\item
...
\def\labelenumi{(\alph{enumi})}
\item
  در صورت استقلال: \lr{P(\rl{هر دو}) = (0.05)(0.03) = 0.0015}
\item
  اگر \lr{P(\rl{هر دو}) = 0.01 \ensuremath{\neq} 0.0015}، آن‌ها مستقل نیستند. منبع نویز مشترک وابستگی ایجاد می‌کند.
\item
...
\emph{ماژول بعدی: {ماژول \lr{2} \lr{—} یک متغیر تصادفی و توابع آن}}\PSbr{}
