...
\textbf{فضای نمونه} \lr{\mbox{S}} (یا \lr{\mbox{\ensuremath{\Omega}}}) مجموعه تمام خروجی‌های ممکن یک آزمایش تصادفی است.
...
\tightlist
\item
  ارسال یک بیت: \lr{\mbox{S = \{0, 1\}}} (دریافت صحیح یا دریافت همراه با خطا)
\item
  تعداد خطاها در یک بسته \lr{\mbox{n}} بیتی: \lr{\mbox{S = \{0, 1, 2, …, n\}}}
\item
  تعداد ورودها به یک سرور: \lr{\mbox{S = \{0, 1, 2, 3, …\}}}
\end{itemize}
...
\tightlist
\item
  ولتاژ اندازه‌گیری‌شده: \lr{\mbox{S = \ensuremath{\mathbb{R}}}} (یا یک بازه عملی مانند \lr{\mbox{[0, 5] V}})
\item
  زمان تا خرابی قطعه: \lr{\mbox{S = [0, \ensuremath{\infty})}}
\item
  فاز سیگنال دریافتی: \lr{\mbox{S = [0, 2\ensuremath{\pi})}}
\end{itemize}
...
\tightlist
\item
  \textbf{رویداد ساده}: دقیقاً شامل یک خروجی است. \lr{\mbox{E = \{(0,1)\}}}
\item
  \textbf{رویداد مرکب}: شامل چندین خروجی است. \lr{\mbox{E = \{(0,1), (1,0)\}}} (هر خطایی)
\item
  \textbf{رویداد قطعی}: کل فضای نمونه \lr{\mbox{S}} (همیشه رخ می‌دهد)
\item
  \textbf{رویداد ناممکن}: مجموعه تهی \lr{\mbox{\ensuremath{\emptyset}}} (هرگز رخ نمی‌دهد)
\end{itemize}
...
\bottomrule\noalign{}
\endlastfoot
اجتماع & \lr{\mbox{A \ensuremath{\cup} B}} & \lr{\mbox{A}} یا \lr{\mbox{B}} (یا هر دو) رخ می‌دهد & هر یک از حالت‌های خرابی \lr{\mbox{A}} یا \lr{\mbox{B}} موجب فعال‌شدن هشدار می‌شود \\
اشتراک & \lr{\mbox{A \ensuremath{\cap} B}} & \lr{\mbox{A}} و \lr{\mbox{B}} هر دو رخ می‌دهند & هر دو حسگر سیگنال را تشخیص می‌دهند \\
متمم & \lr{\mbox{A\textsuperscript{c} or Ā}} & \lr{\mbox{A}} رخ نمی‌دهد & قطعه خراب نمی‌شود \\
تفاضل & \lr{\mbox{A - B = A \ensuremath{\cap} B\textsuperscript{c}}} & \lr{\mbox{A}} رخ می‌دهد ولی \lr{\mbox{B}} رخ نمی‌دهد & حسگر \lr{\mbox{A}} فعال می‌شود ولی حسگر \lr{\mbox{B}} فعال نمی‌شود \\
ناسازگار متقابل & \lr{\mbox{A \ensuremath{\cap} B = \ensuremath{\emptyset}}} & \lr{\mbox{A}} و \lr{\mbox{B}} نمی‌توانند هر دو رخ دهند & قطعه نمی‌تواند هم سالم و هم خراب باشد \\
\end{longtable}
}
...
\tightlist
\item
  \lr{\mbox{A}} = عیب پل لحیم
\item
  \lr{\mbox{B}} = قطعه مفقود
\item
  \lr{\mbox{C}} = مقدار نادرست قطعه
\end{itemize}
...
\tightlist
\item
  \lr{\mbox{A \ensuremath{\cup} B \ensuremath{\cup} C}} = «برد دست‌کم یک عیب دارد»
\item
  \lr{\mbox{A \ensuremath{\cap} B}} = «برد هم عیب پل لحیم و هم یک قطعه مفقود دارد»
\item
  \lr{(A \ensuremath{\cup} B \ensuremath{\cup} C)\textsuperscript{c}} = «برد بدون عیب است»
\end{itemize}
...
برای یک فضای نمونه \lr{\mbox{S}} که رویدادهایی روی آن تعریف شده‌اند، تابع احتمال \lr{\mbox{P}} شرایط زیر را برآورده می‌کند:
...
\textbf{اصل موضوعه 3 (جمع‌پذیری شمارا):}\PSbr{}برای رویدادهای ناسازگار متقابل \lr{\mbox{A\textsubscript{1}}}، \lr{\mbox{A\textsubscript{2}}}، \lr{\mbox{A\textsubscript{3}}}، \lr{…}:\PSdisplay{P(A_1 \cup A_2 \cup A_3 \cup \cdots) = P(A_1) + P(A_2) + P(A_3) + \cdots}

\PSsubsection{ویژگی‌های استخراج‌شده از اصول موضوعه}

\textbf{ویژگی 1: قاعده متمم}\PSdisplay{P(A^c) = 1 - P(A)}

\emph{اثبات:} \lr{\mbox{S = A \ensuremath{\cup} A\textsuperscript{c}}} و \lr{A \ensuremath{\cap} A\textsuperscript{c} = \ensuremath{\emptyset}}، بنابراین \lr{\mbox{P(S) = P(A) + P(A\textsuperscript{c}) = 1}}.

\emph{معنای مهندسی:} \lr{P(\rl{عملکرد قطعه}) = 1 - P(\rl{خرابی قطعه})}

\textbf{ویژگی 2: رویداد ناممکن}\PSdisplay{P(\emptyset) = 0}

\textbf{ویژگی 3: یکنوایی}\PSbr{}اگر \lr{\mbox{A \ensuremath{\subseteq} B}}، آنگاه \lr{\mbox{P(A) \ensuremath{\le} P(B)}}
...
\emph{معنای مهندسی:} هنگام محاسبه احتمال «خرابی \lr{\mbox{A}} یا خرابی \lr{\mbox{B}}»، باید هم‌پوشانی‌ای را که در آن هر دو به‌طور هم‌زمان خراب می‌شوند، کم کنیم.
...
برای رویدادهای ناسازگار متقابل \lr{\mbox{(A \ensuremath{\cap} B = \ensuremath{\emptyset})}}:\PSdisplay{P(A \cup B) = P(A) + P(B)}
...
\lr{P(\rl{دست‌کم یک خطا در 100 بیت}) = 1 - P(\rl{صفر خطا در 100 بیت})}
...
\tightlist
\item
  \lr{\mbox{A}} = خرابی پیوند 1، \lr{\mbox{B}} = خرابی پیوند 2، \lr{\mbox{C}} = خرابی پیوند 3
\item
  \lr{P(\rl{دست‌کم یکی خراب می‌شود}) = P(A \ensuremath{\cup} B \ensuremath{\cup} C)}
\item
  با استفاده از شمول و عدم‌شمول:
\end{itemize}

\lr{P(A \ensuremath{\cup} B \ensuremath{\cup} C) = 3(0.05) - 3(0.05)\textsuperscript{2} + (0.05)\textsuperscript{3} = 0.15 - 0.0075 + 0.000125 = 0.142625}

به‌صورت جایگزین: \lr{P(\rl{دست‌کم یکی خراب می‌شود}) = 1 - P(\rl{هیچ‌کدام خراب نمی‌شوند}) = 1 - (0.95)\textsuperscript{3} = 0.142625 \PSyes{}}
...
\textbf{احتمال شرطی} رویداد \lr{\mbox{A}} با توجه به اینکه رویداد \lr{\mbox{B}} رخ داده است:
...
احتمال شرطی دانش ما را \textbf{به‌روزرسانی} می‌کند. هنگامی که می‌دانیم \lr{\mbox{B}} رخ داده است، فضای نمونه مؤثر از \lr{\mbox{S}} به \lr{\mbox{B}} کاهش می‌یابد و می‌پرسیم: «درون \lr{\mbox{B}}، احتمال \lr{\mbox{A}} چقدر است؟»
...
\bottomrule\noalign{}
\endlastfoot
احتمال \lr{\mbox{A}} & \lr{\mbox{P(A)}} & احتمال \lr{\mbox{A}} بدون اطلاعات اضافی \\
احتمال شرطی & \lr{\mbox{P(A\textbar{}B)}} & احتمال \lr{\mbox{A}} \textbf{با توجه به اینکه} \lr{\mbox{B}} رخ داده است \\
احتمال مشترک & \lr{\mbox{P(A \ensuremath{\cap} B)}} & احتمال اینکه \textbf{هر دو} \lr{\mbox{A}} و \lr{\mbox{B}} رخ دهند \\
\end{longtable}
}

این کمیت‌ها با یکدیگر مرتبط‌اند: \lr{P(A \ensuremath{\cap} B) = P(A\textbar{}B) \ensuremath{\cdot} P(B) = P(B\textbar{}A) \ensuremath{\cdot} P(A)}
...
\tightlist
\item
  \lr{\mbox{H\textsubscript{1}}} = هواپیما وجود دارد (هدف)
\item
  \lr{\mbox{H\textsubscript{0}}} = هواپیما وجود ندارد (بدون هدف)
\item
  \lr{\mbox{D}} = آشکارساز می‌گوید «هدف وجود دارد»
\end{itemize}
...
\tightlist
\item
  \lr{\mbox{P(D \textbar{} H\textsubscript{1})}} = \textbf{احتمال آشکارسازی} (حساسیت)
\item
  \lr{\mbox{P(D \textbar{} H\textsubscript{0})}} = \textbf{احتمال هشدار کاذب}
\item
  \lr{\mbox{P(D\textsuperscript{c} \textbar{} H\textsubscript{1})}} = \textbf{احتمال عدم‌آشکارسازی} = \lr{\mbox{1 - P(D \textbar{} H\textsubscript{1})}}
\end{itemize}

فرض کنید: \lr{\mbox{P(H\textsubscript{1}) = 0.01}}، \lr{\mbox{P(D\textbar{}H\textsubscript{1}) = 0.95}}، \lr{\mbox{P(D\textbar{}H\textsubscript{0}) = 0.02}}

\lr{P(\rl{هدف واقعاً وجود دارد \lr{\textbar{}} آشکارساز وجود هدف را اعلام می‌کند})} چقدر است؟
...
یک کانال متقارن دودویی دارای احتمال وارونگی \lr{\mbox{p = 0.01}} است:

\begin{itemize}
\tightlist
\item
  \lr{P(\rl{دریافت 1 \lr{\textbar{}} ارسال 0}) = p = 0.01}
\item
  \lr{P(\rl{دریافت 0 \lr{\textbar{}} ارسال 1}) = p = 0.01}
\item
  \lr{P(\rl{دریافت 0 \lr{\textbar{}} ارسال 0}) = 1 - p = 0.99}
\item
  \lr{P(\rl{دریافت 1 \lr{\textbar{}} ارسال 1}) = 1 - p = 0.99}
\end{itemize}
...
\tightlist
\item
  \lr{P(\rl{ارسال 0} \ensuremath{\cap} \rl{دریافت 1}) = P(\rl{دریافت 1 \lr{\textbar{}} ارسال 0}) \ensuremath{\cdot} P(\rl{ارسال 0}) = 0.01 \ensuremath{\times} 0.5 = 0.005}
\item
  \lr{P(\rl{خطا}) = P(\rl{ارسال 0} \ensuremath{\cap} \rl{دریافت 1}) + P(\rl{ارسال 1} \ensuremath{\cap} \rl{دریافت 0}) = 0.005 + 0.005 = 0.01}
\end{itemize}
...
\tightlist
\item
  \lr{\mbox{P(A\textbar{}B) \ensuremath{\ge} 0}}
\item
  \lr{\mbox{P(S\textbar{}B) = 1}}
\item
  \lr{P(A\textsubscript{1} \ensuremath{\cup} A\textsubscript{2}\textbar{}B) = P(A\textsubscript{1}\textbar{}B) + P(A\textsubscript{2}\textbar{}B)} اگر \lr{A\textsubscript{1} \ensuremath{\cap} A\textsubscript{2} = \ensuremath{\emptyset}}
\end{enumerate}
...
رویدادهای \lr{\mbox{A}} و \lr{\mbox{B}} از نظر \textbf{آماری مستقل} هستند اگر:

\PSdisplay{P(A \cap B) = P(A) \cdot P(B)}

به‌طور معادل: \lr{\mbox{P(A\textbar{}B) = P(A)}} \lr{—} دانستن اینکه \lr{\mbox{B}} رخ داده است، هیچ اطلاعاتی درباره \lr{\mbox{A}} نمی‌دهد.
...
برای سه رویداد \lr{\mbox{A}}، \lr{\mbox{B}}، \lr{\mbox{C}}:
...
\tightlist
\item
  \lr{\mbox{P(A \ensuremath{\cap} B) = P(A)P(B)}}
\item
  \lr{\mbox{P(A \ensuremath{\cap} C) = P(A)P(C)}}
\item
  \lr{\mbox{P(B \ensuremath{\cap} C) = P(B)P(C)}}
\end{itemize}
...
\item
  \lr{\mbox{P(A \ensuremath{\cap} B \ensuremath{\cap} C) = P(A)P(B)P(C)}}
\end{itemize}
...
\lr{P(\rl{هر سه خراب شوند}) = P(A)P(B)P(C) = (0.1)\textsuperscript{3} = 0.001}

\lr{P(\rl{سیستم کار می‌کند}) = P(\rl{دست‌کم یکی کار می‌کند}) = 1 - P(\rl{همه خراب شوند}) = 1 - 0.001 = 0.999}
...
\tightlist
\item
  \lr{\mbox{P(A)}}، \lr{\mbox{P(B)}} و \lr{\mbox{P(A \ensuremath{\cap} B)}} را به‌صورت تجربی محاسبه کنید
\item
  بررسی کنید که آیا \lr{P(A \ensuremath{\cap} B) \ensuremath{\approx} P(A) \ensuremath{\cdot} P(B)} است
\end{enumerate}
...
این گزاره \textbf{نادرست} است. اگر \lr{\mbox{A \ensuremath{\cap} B = \ensuremath{\emptyset}}} و \lr{\mbox{P(A) \textgreater{} 0}}، \lr{\mbox{P(B) \textgreater{} 0}}، آنگاه:

\begin{itemize}
\tightlist
\item
  \lr{\mbox{P(A \ensuremath{\cap} B) = 0}}
\item
  \lr{\mbox{P(A)P(B) \textgreater{} 0}}
\item
  بنابراین \lr{\mbox{P(A \ensuremath{\cap} B) \ensuremath{\neq} P(A)P(B)}}
\end{itemize}
...
اگر \lr{\mbox{B\textsubscript{1}}}، \lr{\mbox{B\textsubscript{2}}}، \lr{…}، \lr{\mbox{B\textsubscript{n}}} فضای نمونه را افراز کنند (ناسازگار متقابل و جامع باشند):
...
\bottomrule\noalign{}
\endlastfoot
\lr{\mbox{P(B\textsubscript{i})}} & احتمال پیشین & آنچه پیش از مشاهده داده‌ها باور داشتیم \\
\lr{\mbox{P(A\textbar{}B\textsubscript{i})}} & درست‌نمایی & با توجه به هر فرضیه، مشاهده با چه احتمالی رخ می‌دهد \\
\lr{\mbox{P(B\textsubscript{i}\textbar{}A)}} & احتمال پسین & باور به‌روزشده پس از مشاهده داده‌ها \\
\lr{\mbox{P(A)}} & شواهد & احتمال کل مشاهده \\
\end{longtable}
}
...
\tightlist
\item
  \lr{\mbox{B\textsubscript{1}}} = خطای منبع تغذیه، \lr{\mbox{P(B\textsubscript{1}) = 0.3}}
\item
  \lr{\mbox{B\textsubscript{2}}} = خطای پردازنده، \lr{\mbox{P(B\textsubscript{2}) = 0.5}}
\item
  \lr{\mbox{B\textsubscript{3}}} = خطای حافظه، \lr{\mbox{P(B\textsubscript{3}) = 0.2}}
\end{itemize}

یک آزمون تشخیصی علامت \lr{\mbox{A}} (گرم‌شدن بیش از حد) را ایجاد می‌کند:

\begin{itemize}
\tightlist
\item
  \lr{\mbox{P(A\textbar{}B\textsubscript{1}) = 0.8}} (خطاهای منبع تغذیه معمولاً باعث گرم‌شدن بیش از حد می‌شوند)
\item
  \lr{\mbox{P(A\textbar{}B\textsubscript{2}) = 0.3}} (خطاهای پردازنده گاهی باعث گرم‌شدن بیش از حد می‌شوند)
\item
  \lr{\mbox{P(A\textbar{}B\textsubscript{3}) = 0.1}} (خطاهای حافظه به‌ندرت باعث گرم‌شدن بیش از حد می‌شوند)
\end{itemize}
...
\lr{P(A) = P(A\textbar{}B\textsubscript{1})P(B\textsubscript{1}) + P(A\textbar{}B\textsubscript{2})P(B\textsubscript{2}) + P(A\textbar{}B\textsubscript{3})P(B\textsubscript{3})}\PSbr{}\lr{\mbox{= (0.8)(0.3) + (0.3)(0.5) + (0.1)(0.2)}}\PSbr{}\lr{\mbox{= 0.24 + 0.15 + 0.02 = 0.41}}

\lr{P(B\textsubscript{1}\textbar{}A) = (0.8)(0.3) / 0.41 = 0.585 (most likely!)}\PSbr{}\lr{P(B\textsubscript{2}\textbar{}A) = (0.3)(0.5) / 0.41 = 0.366}\PSbr{}\lr{P(B\textsubscript{3}\textbar{}A) = (0.1)(0.2) / 0.41 = 0.049}
...
\tightlist
\item
  \lr{\mbox{P(H\textsubscript{1}) = 0.01}}، \lr{\mbox{P(D\textbar{}H\textsubscript{1}) = 0.95}}، \lr{\mbox{P(D\textbar{}H\textsubscript{0}) = 0.02}}
\end{itemize}

\lr{P(D) = P(D\textbar{}H\textsubscript{1})P(H\textsubscript{1}) + P(D\textbar{}H\textsubscript{0})P(H\textsubscript{0}) = (0.95)(0.01) + (0.02)(0.99) = 0.0095 + 0.0198 = 0.0293}

\lr{P(H\textsubscript{1}\textbar{}D) = (0.95)(0.01) / 0.0293 = 0.324}
...
اگر \lr{\mbox{B\textsubscript{1}}}، \lr{\mbox{B\textsubscript{2}}}، \lr{…}، \lr{\mbox{B\textsubscript{n}}} یک \textbf{افراز} از \lr{\mbox{S}} تشکیل دهند (ناسازگار متقابل و جامع):

\PSdisplay{P(A) = \sum_{i=1}^{n} P(A|B_i) \cdot P(B_i)}

\PSsubsection{تفسیر}

برای یافتن احتمال کل رویداد \lr{\mbox{A}}، آن را به تمام «مسیرهای» ممکن منتهی به \lr{\mbox{A}} از طریق افراز تجزیه کنید.
...
\lr{P(\rl{خطای بسته}) = P(error\textbar{}path 1)P(path 1) + P(error\textbar{}path 2)P(path 2) + P(error\textbar{}path 3)P(path 3)}\PSbr{}\lr{\mbox{= (0.01)(0.50) + (0.05)(0.30) + (0.10)(0.20)}}\PSbr{}\lr{\mbox{= 0.005 + 0.015 + 0.020 = 0.040}}
...
\lr{P(\rl{معیوب}) = P(\rl{معیوب\lr{\textbar{}}}A)P(A) + P(\rl{معیوب\lr{\textbar{}}}B)P(B)}\PSbr{}\lr{\mbox{= (0.02)(0.60) + (0.05)(0.40)}}\PSbr{}\lr{\mbox{= 0.012 + 0.020 = 0.032}}

اگر یک خازن معیوب پیدا شود، کدام تأمین‌کننده احتمالاً آن را تولید کرده است؟

\lr{P(A\textbar{}\rl{معیوب}) = (0.02)(0.60) / 0.032 = 0.375}\PSbr{}\lr{P(B\textbar{}\rl{معیوب}) = (0.05)(0.40) / 0.032 = 0.625}

با وجود تأمین تعداد کمتری از واحدها، احتمال اینکه تأمین‌کننده \lr{\mbox{B}} مسیول یک عیب مشخص باشد بیشتر است.
...
\textbf{احتمال} & \lr{\mbox{P(A)}} = معیار احتمال وقوع & فراوانی نسبی بلندمدت وقوع \\
\textbf{احتمال شرطی} & \lr{\mbox{P(A\textbar{}B)}} = احتمال به‌روزشده با توجه به \lr{\mbox{B}} & اینکه دانستن یک رویداد چگونه احتمال رویداد دیگر را تغییر می‌دهد \\
\textbf{استقلال} & \lr{\mbox{P(A\ensuremath{\cap}B) = P(A)P(B)}} & دانستن یک رویداد هیچ اطلاعاتی درباره دیگری نمی‌دهد \\
\end{longtable}
}
...
\def\labelenumi{(\PSalph{enumi})}
\item
  \lr{P(\rl{خرابی سیستم}) = P(\rl{هر دو خراب شوند}) = (0.02)(0.03) = 0.0006}
\item
  \lr{P(\rl{دست‌کم یکی خراب شود}) = 1 - P(\rl{هیچ‌کدام خراب نشوند}) = 1 - (0.98)(0.97) = 1 - 0.9506 = 0.0494}
\item
  سیستم در حال کار = \lr{\mbox{NOT(\rl{هر دو خراب شوند})}} = دست‌کم یکی کار می‌کند.\PSbr{}\lr{P(P1 failed \textbar{} system works) = P(P1 failed \ensuremath{\cap} P2 works) / P(system works)}\PSbr{}\lr{= (0.02)(0.97) / (1 - 0.0006) = 0.0194 / 0.9994 \ensuremath{\approx} 0.0194}
\end{enumerate}
...
یک سیستم آشکارسازی آتش دارای مقادیر زیر است: \lr{\mbox{P(fire) = 0.001}}، \lr{\mbox{P(alarm \textbar{} fire) = 0.98}}، \lr{\mbox{P(alarm \textbar{} no fire) = 0.01}}.

\begin{enumerate}
\def\labelenumi{(\PSalph{enumi})}
\tightlist
\item
  \lr{\mbox{P(alarm)}} چقدر است؟
\item
  اگر هشدار به صدا درآید، \lr{\mbox{P(fire \textbar{} alarm)}} چقدر است؟
\item
  نتیجه را تفسیر کنید.
\end{enumerate}
...
\def\labelenumi{(\PSalph{enumi})}
\item
  \lr{P(alarm) = P(alarm\textbar{}fire)P(fire) + P(alarm\textbar{}no fire)P(no fire)}\PSbr{}\lr{= (0.98)(0.001) + (0.01)(0.999) = 0.000980 + 0.009990 = 0.010970}
\item
  \lr{P(fire\textbar{}alarm) = P(alarm\textbar{}fire)P(fire) / P(alarm)}\PSbr{}\lr{\mbox{= (0.98)(0.001) / 0.01097 = 0.0894}}
\item
...
\tightlist
\item
  \lr{\mbox{P(\rl{عدم وجود خطا در بسته})}} چقدر است؟
\item
  \lr{\mbox{P(\rl{دقیقاً یک خطا})}} چقدر است؟
\item
  \lr{\mbox{P(\rl{دست‌کم یک خطا})}} چقدر است؟
\item
...
\def\labelenumi{(\PSalph{enumi})}
\item
  \lr{P(\rl{عدم وجود خطا}) = (0.95)\textsuperscript{8} = 0.6634}
\item
  \lr{P(\rl{دقیقاً یک خطا}) = C(8,1)(0.05)\textsuperscript{1}(0.95)\textsuperscript{7} = 8 \ensuremath{\times} 0.05 \ensuremath{\times} 0.6983 = 0.2793}
\item
  \lr{P(\rl{دست‌کم یک خطا}) = 1 - P(\rl{عدم وجود خطا}) = 1 - 0.6634 = 0.3366}
\item
...
\tightlist
\item
  حالت 1 (کانال خوب): 60\% از زمان، \lr{\mbox{P(outage) = 0.01}}
\item
  حالت 2 (کانال متوسط): 30\% از زمان، \lr{\mbox{P(outage) = 0.10}}
\item
  حالت 3 (کانال ضعیف): 10\% از زمان، \lr{\mbox{P(outage) = 0.50}}
\end{itemize}
...
\def\labelenumi{(\PSalph{enumi})}
\item
  \lr{P(outage) = (0.01)(0.60) + (0.10)(0.30) + (0.50)(0.10) = 0.006 + 0.030 + 0.050 = 0.086}
\item
  \lr{\mbox{P(Mode 3 \textbar{} outage) = (0.50)(0.10) / 0.086 = 0.581}}
\end{enumerate}
...
دو حسگر یک فرایند را پایش می‌کنند. حسگر \lr{\mbox{A}} دارای احتمال هشدار کاذب \PSnum{0.05} است. حسگر \lr{\mbox{B}} دارای احتمال هشدار کاذب \PSnum{0.03} است.

\begin{enumerate}
\def\labelenumi{(\PSalph{enumi})}
\tightlist
\item
  اگر حسگرها مستقل باشند، \lr{\mbox{P(\rl{هر دو هشدار کاذب می‌دهند})}} چقدر است؟
\item
  اگر حسگرها یک منبع نویز مشترک داشته باشند و \lr{\mbox{P(\rl{هر دو هشدار کاذب}) = 0.01}} باشد، آیا مستقل هستند؟
\item
  در حالت \lr{\mbox{(b)}}، \lr{\mbox{P(A false alarm \textbar{} B false alarm)}} چقدر است؟
\end{enumerate}
...
\def\labelenumi{(\PSalph{enumi})}
\item
  در صورت استقلال: \lr{\mbox{P(\rl{هر دو}) = (0.05)(0.03) = 0.0015}}
\item
  اگر \lr{\mbox{P(\rl{هر دو}) = 0.01 \ensuremath{\neq} 0.0015}}، آن‌ها مستقل نیستند. منبع نویز مشترک وابستگی ایجاد می‌کند.
\item
  \lr{P(A\textbar{}B) = P(A\ensuremath{\cap}B) / P(B) = 0.01 / 0.03 = 0.333}
\end{enumerate}

این مقدار بسیار بیشتر از \lr{\mbox{P(A) = 0.05}} است و وابستگی شدید را تأیید می‌کند.
