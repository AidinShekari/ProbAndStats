...
\PSsubsection{\PSnum{16.1.1} برآورد \lr{BER} و بازه‌های اطمینان}

\textbf{مسیله:} برآورد نرخ خطای بیت یک لینک مخابراتی.

\textbf{مدل:} هر بیت یک آزمایش برنولی با احتمال خطای \lr{p} است (همان \lr{BER}).
...
\item
  \lr{\ensuremath{\hat{p}} = k/n} (برآوردگر \lr{MLE} برای \lr{BER})
\item
  بازه اطمینان \lr{95\%: \ensuremath{\hat{p}} \ensuremath{\pm} 1.96\ensuremath{\surd}(\ensuremath{\hat{p}}(1-\ensuremath{\hat{p}})/n)}
\end{itemize}

\textbf{\lr{MATLAB}: شبیه‌سازی \lr{BER} و بازه اطمینان}
...
\PSsubsection{\PSnum{16.1.2} مدل‌سازی نویز \lr{AWGN}}

نویز گاوسی سفید جمع‌شونده \lr{(AWGN)} مدل نویز بنیادی است:

\begin{itemize}
\tightlist
\item
  \textbf{توزیع:} \lr{N(0, N\textsubscript{0}/2)} در هر بُعد
\item
  \textbf{چرا گاوسی؟} \lr{CLT —} نویز حرارتی جمع سهم‌های بسیاری از الکترون‌ها است
\item
  \textbf{مشخصه‌سازی:} کاملاً با چگالی طیفی توان \lr{N\textsubscript{0}} یا واریانس \lr{\ensuremath{\sigma}\textsuperscript{2} = N\textsubscript{0}B} توصیف می‌شود (\lr{B =} پهنای‌باند)
\end{itemize}

\PSsubsection{\PSnum{16.1.3} کانال محوشدگی رایلی}

\textbf{مدل:} ضریب کانال \lr{h = h\textsubscript{I} + j\ensuremath{\cdot}h\textsubscript{Q}} که در آن \lr{h\textsubscript{I}, h\textsubscript{Q} \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2})} و مستقل هستند.

\begin{itemize}
\tightlist
\item
  پوش \lr{\textbar{}h\textbar{} \textasciitilde{} Rayleigh(\ensuremath{\sigma})}
\item
  توان \lr{\textbar{}h\textbar{}\textsuperscript{2} \textasciitilde{} Exponential(1/(2\ensuremath{\sigma}\textsuperscript{2}))}
\item
  \textbf{احتمال قطع:} \lr{P(\textbar{}h\textbar{}\textsuperscript{2} \textless{} \ensuremath{\gamma}\textsubscript{th}) = 1 - exp(-\ensuremath{\gamma}\textsubscript{th}/(2\ensuremath{\sigma}\textsuperscript{2}))}
\end{itemize}
...
هنگامی که مؤلفه خط دید \lr{(LOS)} وجود دارد:

\begin{itemize}
\tightlist
\item
  \lr{h = h\textsubscript{LOS} + h\textsubscript{scatter}}
\item
  \lr{\textbar{}h\textbar{} \textasciitilde{} Rician(\ensuremath{\nu}, \ensuremath{\sigma})} که در آن \lr{K = \ensuremath{\nu}\textsuperscript{2}/(2\ensuremath{\sigma}\textsuperscript{2})} ضریب \lr{K} است
\end{itemize}

ضریب \lr{K: K = 0} (رایلی) \lr{\ensuremath{\to} K = \ensuremath{\infty}} (بدون محوشدگی، \lr{AWGN})

\PSsubsection{\PSnum{16.1.5} آماره‌های \lr{SNR}}

\textbf{\lr{SNR} لحظه‌ای:} \lr{\ensuremath{\gamma} = \textbar{}h\textbar{}\textsuperscript{2} \ensuremath{\times} (E\textsubscript{s}/N\textsubscript{0})}

برای رایلی: \lr{\ensuremath{\gamma} \textasciitilde{} Exponential(\ensuremath{\bar{\gamma}})} که در آن \lr{\ensuremath{\bar{\gamma}} = SNR} میانگین

\textbf{میانگین \lr{BER} در محوشدگی:}\PSbr{}\[\bar{P}_e = \int_0^\infty P_e(\gamma) \cdot f_\gamma(\gamma) \, d\gamma\]

برای \lr{BPSK} در کانال رایلی: \lr{\ensuremath{\bar{P}}\textsubscript{e} = (1/2)(1 - \ensuremath{\surd}(\ensuremath{\bar{\gamma}}/(1+\ensuremath{\bar{\gamma}})))}
...
\item
  آماره‌های نویز را برآورد کنید: \lr{\ensuremath{\hat{\mu}}\textsubscript{n}}، \lr{\ensuremath{\hat{\sigma}}\textsuperscript{2}\textsubscript{n}}
\item
  گاوسی‌بودن را آزمون کنید (هیستوگرام، نمودار \lr{QQ})
\item
  \lr{SNR} را محاسبه کنید: \lr{SNR = P\textsubscript{signal} / \ensuremath{\hat{\sigma}}\textsuperscript{2}\textsubscript{n}}
\end{enumerate}
...
\textbf{روش:} سیگنال دریافتی را با الگوی معلوم همبسته کنید. تحت \lr{H\textsubscript{0}} (فقط نویز)، خروجی همبسته‌ساز توزیع معلومی دارد \lr{\ensuremath{\to}} آستانه را تعیین کنید.
...
\textbf{مسیله:} مقاومت‌ها مقدار نامی \lr{R\textsubscript{0}} با رواداری ساخت دارند.

\textbf{مدل:} \lr{R \textasciitilde{} N(R\textsubscript{0}, \ensuremath{\sigma}\textsubscript{R}\textsuperscript{2})} که در آن \lr{\ensuremath{\sigma}\textsubscript{R}} به رده رواداری بستگی دارد.

برای رواداری \lr{\ensuremath{\pm}5\% (99.7\%} در محدوده): \lr{3\ensuremath{\sigma} = 0.05R\textsubscript{0} \ensuremath{\to} \ensuremath{\sigma} = 0.0167R\textsubscript{0}}

\PSsubsection{\PSnum{16.3.2} تحلیل مدار با مونت‌کارلو}

\textbf{مسیله:} یک تقسیم‌کننده ولتاژ از \lr{R\textsubscript{1}} و \lr{R\textsubscript{2}} استفاده می‌کند. توزیع ولتاژ خروجی چیست؟

\lr{V\textsubscript{out} = V\textsubscript{in} \ensuremath{\times} R\textsubscript{2}/(R\textsubscript{1} + R\textsubscript{2})}

با تصادفی‌بودن \lr{R\textsubscript{1}} و \lr{R\textsubscript{2}: V\textsubscript{out}} یک متغیر تصادفی است!
...
\textbf{مثال:} توان \lr{P = V\textsuperscript{2}/R. \ensuremath{\sigma}\textsubscript{P}\textsuperscript{2} \ensuremath{\approx} (2V/R)\textsuperscript{2}\ensuremath{\sigma}\textsubscript{V}\textsuperscript{2} + (V\textsuperscript{2}/R\textsuperscript{2})\textsuperscript{2}\ensuremath{\sigma}\textsubscript{R}\textsuperscript{2}}

\PSsection{16.4}{سیستم‌های قدرت}

\PSsubsection{\PSnum{16.4.1} عدم‌قطعیت بار}

\textbf{مسیله:} بار اوج نامعین است. مدل: \lr{L \textasciitilde{} N(\ensuremath{\mu}\textsubscript{L}, \ensuremath{\sigma}\textsubscript{L}\textsuperscript{2})}

پرسش طراحی: چه ظرفیت \lr{C} تضمین می‌کند \lr{P}(بار \lr{\textgreater{}} ظرفیت) \lr{\textless{} 0.01}؟

\lr{C = \ensuremath{\mu}\textsubscript{L} + 2.326\ensuremath{\sigma}\textsubscript{L}} (برای احتمال تخطی 1\%)

\PSsubsection{\PSnum{16.4.2} قابلیت اطمینان سیستم}

\textbf{سیستم سری:} همه قطعات باید کار کنند.

\lr{R\textsubscript{sys} = R\textsubscript{1} \ensuremath{\times} R\textsubscript{2} \ensuremath{\times} … \ensuremath{\times} R\textsubscript{n}} (برای قطعات مستقل)

\textbf{سیستم موازی:} حداقل یکی باید کار کند.

\lr{R\textsubscript{sys} = 1 - (1-R\textsubscript{1})(1-R\textsubscript{2})…(1-R\textsubscript{n})}
...
اندازه‌گیری: \lr{y(k) = x(k) + n(k)} که در آن \lr{n \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2}\textsubscript{n})}

کنترل‌کننده روی \lr{y(k)} عمل می‌کند \lr{\ensuremath{\to}} خطای ردیابی شامل سهم نویز می‌شود.
...
\tightlist
\item
  میانگین‌گیری (فیلتر پایین‌گذر): نویز را به اندازه \lr{\ensuremath{\surd}n} کاهش می‌دهد
\item
...
مدل سیستم: \lr{G(s) = K/(\ensuremath{\tau}s + 1)} که در آن \lr{K} و \lr{\ensuremath{\tau}} نامعین هستند.

رویکرد مونت‌کارلو: \lr{K} و \lr{\ensuremath{\tau}} را از توزیع‌هایشان نمونه‌گیری کنید، پاسخ سیستم را بارها شبیه‌سازی کنید و آماره‌های خروجی را محاسبه کنید.

\PSsection{16.6}{ارتباط با یادگیری ماشین \lr{/} هوش مصنوعی}
...
\tightlist
\item
  داده آموزش \lr{=} نمونه‌ای از جامعه
\item
  داده آزمون \lr{=} نمونه مستقل برای اعتبارسنجی
\item
  فرض \lr{i.i.d}. زیربنای بیشتر نظریه یادگیری ماشین است
\end{itemize}
...
\tightlist
\item
  رگرسیون خطی \lr{= MLE} تحت فرض نویز گاوسی
\item
  آموزش شبکه عصبی \lr{= MLE} (یا \lr{MAP}) برای پارامترهای مدل
\item
  کمینه‌سازی تابع هزینه \lr{=} اصل بیشینه درست‌نمایی
\end{itemize}
...
\[\PSmtext{Test Error} = \PSmtext{Bias}^2 + \PSmtext{Variance} + \PSmtext{Irreducible Noise}\]

این دقیقاً همان تجزیه \lr{MSE} از نظریه برآورد است (ماژول 13)!
...
\item
  شبکه‌های عصبی بیزی: توزیع پسین روی وزن‌ها \lr{\ensuremath{\to}} عدم‌قطعیت پیش‌بینی
\item
  کالیبراسیون: احتمال‌های پیش‌بینی‌شده باید با فراوانی‌های مشاهده‌شده همخوانی داشته باشند
\end{itemize}

\PSsection{16.7}{پروژه جامع \lr{MATLAB}: تحلیل سیستم مخابراتی}
...
4. گشتاورها & توان سیگنال، توان نویز، \lr{SNR} \\
5. تبدیل‌ها & تبدیل به \lr{dB}، توان از ولتاژ \\
\PSnum{6-7.} مشترک/توابع & سیگنال+نویز، ترکیب‌های سیستم \\
8. شرطی & عملکرد در وضعیت‌های مختلف \\
9. همبستگی & کانال‌های \lr{MIMO}، آرایه‌های حسگر \\
10. چند متغیر تصادفی & پردازش برداری، شکل‌دهی پرتو \\
\lr{11. CLT} & چرا نویز گاوسی است، میانگین‌گیری \\
12. آمار & برآورد پارامترهای سیستم \\
13. برآورد & \lr{MLE} برای پارامترهای کانال/نویز \\
14. اطمینان & کمّی‌سازی عدم‌قطعیت اندازه‌گیری \\
15. فرض & کنترل کیفیت، مقایسه عملکرد \\
\end{longtable}
}

\emph{پایان ماژول‌های درس. برای راهنماهای تصمیم، مرجع \lr{MATLAB} و برگه فرمول به {مطالب تکمیلی} مراجعه کنید.}\PSbr{}
