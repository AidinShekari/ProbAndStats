...
\[\bar{X}_n = \frac{1}{n}\sum_{i=1}^n X_i \xrightarrow{P} \mu \quad \PSmtext{as } n \to \infty\]

(\lr{«}همگرایی در احتمال\lr{»: P(\textbar{}\ensuremath{\bar{X}}\textsubscript{n} - \ensuremath{\mu}\textbar{} \textgreater{} \ensuremath{\varepsilon}) \ensuremath{\to} 0} برای هر \lr{\ensuremath{\varepsilon} \textgreater{} 0})

\PSsubsection{قانون قوی اعداد بزرگ}

\[\bar{X}_n \xrightarrow{a.s.} \mu \quad \PSmtext{as } n \to \infty\]

(\lr{«}همگرایی تقریباً مطمین\lr{»: P(\ensuremath{\bar{X}}\textsubscript{n} \ensuremath{\to} \ensuremath{\mu}) = 1})
...
\item
  این گزاره را توجیه می‌کند: \lr{«}اگر به‌قدر کافی اندازه‌گیری کنم، میانگین من به مقدار واقعی نزدیک خواهد بود\lr{»}
\end{itemize}
...
برای اطمینان 95\% از این‌که خطا \lr{\textless{} \ensuremath{\varepsilon}} باشد: به \lr{n \ensuremath{\ge} \ensuremath{\sigma}\textsuperscript{2}/(0.05 \ensuremath{\cdot} \ensuremath{\varepsilon}\textsuperscript{2})} نیاز است
...
برای \lr{n} بزرگ:\PSbr{}\[\bar{X}_n \approx N\left(\mu, \frac{\sigma^2}{n}\right)\]
...
\item
  \lr{n «}به‌قدر کافی بزرگ\lr{»} است (قاعده سرانگشتی: \lr{n \ensuremath{\ge} 30}، اما به شکل توزیع بستگی دارد)
\end{enumerate}
...
\lr{CLT} با چه سرعتی \lr{«}اثر می‌گذارد\lr{»}؟
...
نویز حرارتی \lr{=} اثر تجمعی میلیاردها الکترون که به‌طور تصادفی حرکت می‌کنند. سهم هر الکترون ناچیز و مستقل است \lr{\ensuremath{\to} CLT} اعمال می‌شود \lr{\ensuremath{\to}} نویز گاوسی است.

\PSsubsection{چرا تداخل تجمعی گاوسی است}

در یک شبکه سلولی، تداخل کل \lr{=} جمع سیگنال‌های بسیاری از کاربران مستقل. \lr{CLT \ensuremath{\to}} تداخل \lr{\ensuremath{\approx}} گاوسی (برای کاربران زیاد).

\PSsubsection{چرا خطاهای اندازه‌گیری گاوسی هستند}

خطای کل \lr{=} جمع بسیاری از منابع خطای کوچک و مستقل (کالیبراسیون، کوانتیزاسیون، گرمایی، لرزش و \lr{…) \ensuremath{\to} CLT \ensuremath{\to}} خطا \lr{\ensuremath{\approx}} گاوسی.

\PSsubsection{چرا تغییرات ساخت گاوسی هستند}

مقدار مقاومت \lr{=} مقدار نامی \lr{+} جمع بسیاری از تغییرات کوچک فرایند (دوپینگ، حکاکی، دما و \lr{…) \ensuremath{\to} CLT \ensuremath{\to}} مقدار \lr{\ensuremath{\approx}} گاوسی حول مقدار نامی.
...
یک کارخانه پیچ تولید می‌کند. طول \lr{X \textasciitilde{}} توزیعی با \lr{\ensuremath{\mu} = 10cm} و \lr{\ensuremath{\sigma} = 0.2cm}. یک دسته 100 پیچی اندازه‌گیری می‌شود.
...
یک لینک مخابراتی 10000 بیت با \lr{BER = 0.001} دارد. با استفاده از \lr{CLT} مقدار \lr{P}(بیش از 15 خطا) را تقریب بزنید.
...
\emph{ماژول بعدی: {ماژول \lr{12 —} مقدمه‌ای بر آمار}}\PSbr{}
