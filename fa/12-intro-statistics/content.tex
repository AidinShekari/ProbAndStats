...
\item
  گذار از احتمال (مدل\lr{\ensuremath{\to}}پیش‌بینی) به آمار (داده\lr{\ensuremath{\to}}مدل) را درک کنند
\end{enumerate}
...
\lr{«}با داشتن توزیع، داده چگونه است؟\lr{»} & \lr{«}با داشتن داده، توزیع چگونه است؟\lr{»} \\
\end{longtable}
}

\textbf{احتمال} (ماژول‌های \lr{1–11}): با داشتن یک مدل \lr{\ensuremath{\to}} مشاهده‌ها را پیش‌بینی کنید.\PSbr{}\textbf{آمار} (ماژول‌های \lr{12–16}): با داشتن مشاهده‌ها \lr{\ensuremath{\to}} مدل را استنباط کنید.
...
\tightlist
\item
  \textbf{احتمال:} \lr{«}اگر نویز \lr{N(0, 0.01)} باشد، \lr{P(error)} چقدر است؟\lr{»}
\item
  \textbf{آمار:} \lr{«1000} نمونه نویز اندازه‌گیری کردم. \lr{\ensuremath{\sigma}\textsuperscript{2}} چقدر است؟\lr{»}
\end{itemize}
...
\tightlist
\item
  \lr{\ensuremath{\mu} =} میانگین واقعی جامعه
\item
  \lr{\ensuremath{\sigma}\textsuperscript{2} =} واریانس واقعی جامعه
\item
  \lr{p =} نرخ واقعی نقص
\item
  \lr{\ensuremath{\lambda} =} نرخ واقعی خرابی
\end{itemize}
...
\tightlist
\item
  \lr{\ensuremath{\bar{X}} =} میانگین نمونه (برآورد \lr{\ensuremath{\mu}})
\item
  \lr{S\textsuperscript{2} =} واریانس نمونه (برآورد \lr{\ensuremath{\sigma}\textsuperscript{2}})
\item
  \lr{\ensuremath{\hat{p}} =} نسبت نمونه (برآورد \lr{p})
\end{itemize}

\begin{PSquote}

\lr{\PSwarn{}} \textbf{نکته کلیدی:} آماره‌ها متغیرهای تصادفی هستند! نمونه‌های مختلف مقادیر مختلفی می‌دهند.

\end{PSquote}

\begin{PSbox}[unbreakable]{ex}{مثال}

توان واقعی نویز: \lr{\ensuremath{\sigma}\textsuperscript{2} = 0.04 V\textsuperscript{2}} (پارامتر \lr{—} ثابت، نامعلوم)

آزمایش 1: جمع‌آوری 100 نمونه \lr{\ensuremath{\to}} محاسبه \lr{S\textsuperscript{2} = 0.038} (یک تحقق)\PSbr{}آزمایش 2: جمع‌آوری 100 نمونه \lr{\ensuremath{\to}} محاسبه \lr{S\textsuperscript{2} = 0.042} (تحقق متفاوت)\PSbr{}آزمایش 3: جمع‌آوری 100 نمونه \lr{\ensuremath{\to}} محاسبه \lr{S\textsuperscript{2} = 0.041} (تحقق دیگر)

هر \lr{S\textsuperscript{2}} به‌دلیل تصادفی‌بودن نمونه‌گیری متفاوت است.
...
یک \textbf{نمونه تصادفی} \lr{X\textsubscript{1}, X\textsubscript{2}, …, X\textsubscript{n}} مجموعه‌ای از \lr{n} متغیر تصادفی مستقل و هم‌توزیع \lr{(i.i.d.)} از توزیع جامعه است.
...
\PSsubsection{زمانی که \lr{i.i.d}. برقرار است}

\lr{\PSyes{}} انتخاب تصادفی قطعات از یک دسته بزرگ\PSbr{}\lr{\PSyes{}} اندازه‌گیری‌های مستقل از یک فرایند پایدار\PSbr{}\lr{\PSyes{}} نمونه‌های نویز با فاصله بیش از زمان همبستگی

\PSsubsection{زمانی که \lr{i.i.d}. برقرار نیست}

\PSno{} نمونه‌های متوالی یک سیگنال همبسته\PSbr{}\PSno{} اندازه‌گیری‌ها در طول یک فرایند رانش‌دار\PSbr{}\PSno{} انتخاب غیرتصادفی (مثلاً آزمایش فقط قطعات \lr{«}مشکوک\lr{»})
...
یک آماره (مانند \lr{\ensuremath{\bar{X}}}) یک متغیر تصادفی است. توزیع آن به ما می‌گوید:
...
\PSsection{12.6}{میانگین نمونه \lr{\ensuremath{\bar{X}}}}
...
\textbf{میانگین \lr{\ensuremath{\bar{X}}}:}\PSbr{}\[E[\bar{X}] = \mu\]

\lr{\ensuremath{\bar{X}}} یک برآوردگر \textbf{نااریب} برای \lr{\ensuremath{\mu}} است (به‌طور میانگین برابر مقدار واقعی است).

\textbf{واریانس \lr{\ensuremath{\bar{X}}}:}\PSbr{}\[\PSmtext{Var}(\bar{X}) = \frac{\sigma^2}{n}\]

اندازه‌گیری‌های بیشتر \lr{\ensuremath{\to}} تغییرپذیری کمتر \lr{\ensuremath{\to}} برآورد دقیق‌تر.

\textbf{خطای معیار:}\PSbr{}\[\PSmtext{SE}(\bar{X}) = \frac{\sigma}{\sqrt{n}}\]

\PSsubsection{توزیع نمونه‌گیری \lr{\ensuremath{\bar{X}}}}

\begin{itemize}
\tightlist
\item
  اگر جامعه گاوسی باشد: \lr{\ensuremath{\bar{X}} \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}/n)} \textbf{به‌طور دقیق} برای هر \lr{n}
\item
  اگر جامعه گاوسی نباشد: \lr{\ensuremath{\bar{X}} \ensuremath{\approx} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}/n)} \textbf{به‌طور تقریبی} برای \lr{n} بزرگ (بر اساس \lr{CLT})
\end{itemize}

\PSsection{12.7}{واریانس نمونه \lr{S\textsuperscript{2}}}
...
استفاده از \lr{n} در مخرج یک برآوردگر \textbf{اریب} می‌دهد (به‌طور نظام‌مند \lr{\ensuremath{\sigma}\textsuperscript{2}} را کم‌برآورد می‌کند):

\lr{E[1/n \ensuremath{\cdot} \ensuremath{\Sigma}(X\textsubscript{i} - \ensuremath{\bar{X}})\textsuperscript{2}] = ((n-1)/n)\ensuremath{\sigma}\textsuperscript{2} \ensuremath{\neq} \ensuremath{\sigma}\textsuperscript{2}}

استفاده از \lr{n-1} این را تصحیح می‌کند:\PSbr{}\[E[S^2] = \sigma^2\]

\textbf{شهود:} \lr{\ensuremath{\bar{X}}} از همان داده محاسبه می‌شود، بنابراین انحراف‌ها از \lr{\ensuremath{\bar{X}}} کوچک‌تر از انحراف‌ها از \lr{\ensuremath{\mu}} هستند. با برآورد میانگین، \lr{«}یک درجه آزادی را از دست می‌دهیم\lr{»}.

\PSsubsection{درجات آزادی}

با \lr{n} نقطه داده و برآورد \lr{\ensuremath{\bar{X}}}:

\begin{itemize}
\tightlist
\item
  \lr{n} انحراف \lr{(X\textsubscript{i} - \ensuremath{\bar{X}})} جمعاً صفر می‌شوند: \lr{\ensuremath{\Sigma}(X\textsubscript{i} - \ensuremath{\bar{X}}) = 0}
\item
  تنها \lr{n-1} تای آن‌ها \lr{«}آزاد\lr{»} هستند \lr{—} آخری تعیین‌شده است
\item
  \lr{df = n - 1}
\end{itemize}
...
اگر جامعه \lr{N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2})} باشد:\PSbr{}\[(n-1)S^2/\sigma^2 \sim \chi^2(n-1)\]

\PSsection{12.8}{انحراف معیار نمونه}

\[S = \sqrt{S^2} = \sqrt{\frac{1}{n-1}\sum_{i=1}^n (X_i - \bar{X})^2}\]

توجه: \lr{E[S] \ensuremath{\neq} \ensuremath{\sigma} (S} برای \lr{\ensuremath{\sigma}} کمی اریب است، هرچند \lr{S\textsuperscript{2}} برای \lr{\ensuremath{\sigma}\textsuperscript{2}} نااریب است).

\PSsection{12.9}{ارتباط با \lr{CLT}}

از ماژول \lr{11: \ensuremath{\bar{X}} \ensuremath{\to} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}/n)} وقتی \lr{n \ensuremath{\to} \ensuremath{\infty}}.

این بدان معناست که توزیع نمونه‌گیری \lr{\ensuremath{\bar{X}}} تقریباً گاوسی است. این همان چیزی است که به ما امکان می‌دهد:
...
\PSsection{12.10}{مثال‌های \lr{MATLAB}}

\begin{PSbox}[unbreakable]{ex}{مثال 1: توزیع نمونه‌گیری \lr{\ensuremath{\bar{X}}}}
...
یک جامعه دارای \lr{\ensuremath{\mu} = 50} و \lr{\ensuremath{\sigma} = 10} است. یک نمونه تصادفی به اندازه \lr{n = 64} گرفته می‌شود.

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\tightlist
\item
  \lr{E[\ensuremath{\bar{X}}]} چقدر است؟ \lr{(b) Var(\ensuremath{\bar{X}})} چقدر است؟ \lr{(c) P(48 \textless{} \ensuremath{\bar{X}} \textless{} 52)} چقدر است؟
\end{enumerate}
...
\tightlist
\item
  \lr{E[\ensuremath{\bar{X}}] = 50}
\item
  \lr{Var(\ensuremath{\bar{X}}) = 100/64 = 1.5625}، \lr{SE = 1.25}
\item
  \lr{P(48 \textless{} \ensuremath{\bar{X}} \textless{} 52) = P(-1.6 \textless{} Z \textless{} 1.6) = 2\ensuremath{\Phi}(1.6) - 1 = 0.8904}
\end{enumerate}
...
\PSsolution{} از نظر ریاضی: \lr{E[(1/n)\ensuremath{\Sigma}(X\textsubscript{i}-\ensuremath{\bar{X}})\textsuperscript{2}] = (n-1)/n \ensuremath{\cdot} \ensuremath{\sigma}\textsuperscript{2}}، بنابراین تقسیم بر \lr{n-1} نتیجه می‌دهد \lr{E[S\textsuperscript{2}] = \ensuremath{\sigma}\textsuperscript{2}}. از نظر شهودی: ما از \lr{\ensuremath{\bar{X}}} همان داده استفاده می‌کنیم (نه \lr{\ensuremath{\mu}} واقعی)، بنابراین انحراف‌ها از \lr{\ensuremath{\bar{X}}} به‌طور مصنوعی کوچک‌اند. یک درجه آزادی \lr{«}صرف\lr{»} برآورد میانگین می‌شود.

\end{PSbox}

\begin{PSbox}[unbreakable]{prob}{مسیله 3}

یک مهندس توان نویز را از 20 نمونه اندازه‌گیری می‌کند و \lr{S\textsuperscript{2} = 0.045 V\textsuperscript{2}} به دست می‌آید. واریانس واقعی \lr{\ensuremath{\sigma}\textsuperscript{2} = 0.04 V\textsuperscript{2}} است. آیا این نگران‌کننده است؟ برای ارزیابی از توزیع نمونه‌گیری استفاده کنید.

\PSsolution{} تحت \lr{H\textsubscript{0}: \ensuremath{\sigma}\textsuperscript{2}=0.04}، داریم \lr{(n-1)S\textsuperscript{2}/\ensuremath{\sigma}\textsuperscript{2} \textasciitilde{} \ensuremath{\chi}\textsuperscript{2}(19)}. مقدار مشاهده‌شده: \lr{19(0.045)/0.04 = 21.375. P(\ensuremath{\chi}\textsuperscript{2}(19) \textgreater{} 21.375) \ensuremath{\approx} 0.31}. این غیرمعمول نیست \lr{—} حدود 31\% از نمونه‌ها هنگامی که \lr{\ensuremath{\sigma}\textsuperscript{2}=0.04} است، \lr{S\textsuperscript{2} \ensuremath{\ge} 0.045} می‌دهند. نگران‌کننده نیست.

\emph{ماژول بعدی: {ماژول \lr{13 —} برآورد پارامتر}}\PSbr{}
