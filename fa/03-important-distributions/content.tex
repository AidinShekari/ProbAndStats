...
\item
  \lr{PMF/PDF}، \lr{CDF}، میانگین و واریانس را برای تمام توزیع‌های اصلی بیان کنند
\item
  تفسیر فیزیکی و زمینه مهندسی هر توزیع را توضیح دهند
\item
  تعیین کنند چه زمانی از هر توزیع استفاده شود (و چه زمانی استفاده نشود)
\item
  توزیع‌ها را برای شبیه‌سازی در \lr{MATLAB} پیاده‌سازی کنند
\item
...
\lr{«}کدام توزیع احتمال این کمیت نامعین را بهتر توصیف می‌کند؟\lr{»}
...
\tightlist
\item
  شمارش موفقیت‌ها \lr{\ensuremath{\to}} دوجمله‌ای
\item
  انتظار برای رویدادها \lr{\ensuremath{\to}} نمایی/هندسی
\item
  جمع بسیاری از اثرهای کوچک \lr{\ensuremath{\to}} گاوسی
\item
  پوش سیگنال در محوشدگی \lr{\ensuremath{\to}} رایلی
\end{itemize}
...
یک \textbf{آزمایش منفرد} با دو خروجی ممکن را مدل می‌کند: \lr{«}موفقیت\lr{» (1)} یا \lr{«}شکست\lr{» (0)}.
...
\lr{X \textasciitilde{} Bernoulli(p)}، که در آن \lr{p =} احتمال موفقیت.
...
شکل فشرده: \lr{p\textsubscript{X}(x) = p\textsuperscript{x}(1-p)\textsuperscript{1-x}} برای \lr{x \ensuremath{\in} \{0, 1\}}
...
\lr{\PSyes{}} آزمایش دودویی منفرد با احتمال ثابت\PSbr{}\lr{\PSyes{}} بلوک سازنده برای توزیع‌های پیچیده‌تر\PSbr{}\lr{\PSyes{}} مدل‌سازی حالت‌های روشن/خاموش، خروجی‌های قبول/رد

\PSsubsection{زمان عدم استفاده}

\PSno{} آزمایش‌های متعدد (از دوجمله‌ای استفاده کنید)\PSbr{}\PSno{} خروجی دودویی نیست\PSbr{}\PSno{} احتمال میان آزمایش‌ها تغییر می‌کند

\PSsubsection{پیاده‌سازی در \lr{MATLAB}}
...
\item
  تعداد قطعات معیوب در یک دسته \lr{n}‌تایی
\item
...
\lr{X \textasciitilde{} Binomial(n, p)}
...
که در آن \lr{C(n,k) = n! / (k!(n-k)!)}

\PSsubsection{\lr{CDF}}

\[F_X(k) = \sum_{i=0}^{\lfloor k \rfloor} \binom{n}{i} p^i (1-p)^{n-i}\]

فرم بسته ندارد \lr{—} از جدول‌ها یا \lr{MATLAB} استفاده کنید.
...
\[\PSmtext{Var}(X) = np(1-p)\]

\PSsubsection{زمان استفاده}

\lr{\PSyes{}} تعداد ثابت آزمایش‌ها \lr{n}\PSbr{}\lr{\PSyes{}} هر آزمایش مستقل است\PSbr{}\lr{\PSyes{}} هر آزمایش همان احتمال \lr{p} را دارد\PSbr{}\lr{\PSyes{}} شمارش تعداد \lr{«}موفقیت‌ها\lr{»}

\PSsubsection{زمان عدم استفاده}

\PSno{} آزمایش‌ها مستقل نیستند (خرابی‌های همبسته)\PSbr{}\PSno{} احتمال میان آزمایش‌ها تغییر می‌کند\PSbr{}\lr{\PSno{} n} ثابت نیست (از دوجمله‌ای منفی یا پواسون استفاده کنید)\PSbr{}\lr{\PSno{} n} بسیار بزرگ با \lr{p} کوچک (از تقریب پواسون استفاده کنید)
...
\lr{X \textasciitilde{} Geometric(p)}

قرارداد: \lr{X =} شماره آزمایشی که نخستین موفقیت در آن رخ می‌دهد \lr{(X \ensuremath{\in} \{1, 2, 3, …\})}.
...
\textbf{معنای مهندسی:} اگر یک سیستم در \lr{m} تلاش موفق نشده باشد، احتمال نیاز به \lr{n} تلاش بیشتر همانند شروع از صفر است. \lr{«}شکست‌های گذشته بر احتمال آینده اثر نمی‌گذارند.\lr{»}

\PSsubsection{زمان استفاده}

\lr{\PSyes{}} آزمایش‌های مستقل مکرر تا نخستین موفقیت\PSbr{}\lr{\PSyes{}} هر آزمایش همان احتمال را دارد\PSbr{}\lr{\PSyes{}} فرض بی‌حافظه مناسب است (مثلاً ارسال‌های مجدد مستقل)

\PSsubsection{زمان عدم استفاده}

\PSno{} احتمال موفقیت در طول زمان تغییر می‌کند (پیری، یادگیری)\PSbr{}\PSno{} آزمایش‌ها مستقل نیستند\PSbr{}\PSno{} به‌دنبال \lr{r-}امین موفقیت هستید (از دوجمله‌ای منفی استفاده کنید)
...
\textbf{تعداد آزمایش‌ها تا \lr{r-}امین موفقیت} را مدل می‌کند (تعمیم هندسی).
...
\lr{X \textasciitilde{} NegBin(r, p) =} تعداد آزمایش‌ها تا \lr{r-}امین موفقیت.
...
(انتخاب این‌که کدام \lr{r-1} آزمایش از میان \lr{k-1} آزمایش نخست موفق بوده‌اند؛ آزمایش \lr{k-}ام همان \lr{r-}امین موفقیت است.)

\PSsubsection{\lr{CDF}}

فرم بسته ساده‌ای ندارد. با جمع‌بندی یا \lr{MATLAB} محاسبه می‌شود.
...
\lr{\PSyes{}} آزمایش‌های مستقل مکرر تا \lr{r-}امین موفقیت\PSbr{}\lr{\PSyes{}} احتمال ثابت در هر آزمایش\PSbr{}\lr{\PSyes{}} تعمیم هندسی به چندین موفقیت موردنیاز

\PSsubsection{زمان عدم استفاده}

\PSno{} فقط نخستین موفقیت لازم است (از هندسی استفاده کنید \lr{—} ساده‌تر)\PSbr{}\PSno{} آزمایش‌ها مستقل نیستند\PSbr{}\PSno{} تعداد آزمایش‌ها ثابت است (از دوجمله‌ای استفاده کنید)
...
\tightlist
\item
  تعداد فوتون‌های برخوردکننده با یک آشکارساز در \lr{1 \ensuremath{\mu}s}
\item
  تعداد بسته‌های واردشده به یک مسیریاب در 1 ثانیه
\item
  تعداد نقص‌ها در هر \lr{cm\textsuperscript{2}} روی یک ویفر سیلیکونی
\item
...
\lr{X \textasciitilde{} Poisson(\ensuremath{\lambda})}، که در آن \lr{\ensuremath{\lambda} =} میانگین تعداد رویدادها در هر بازه.
...
\[\PSmtext{Var}(X) = \lambda\]

\textbf{ویژگی منحصربه‌فرد:} میانگین برابر واریانس است! این یک بررسی سریع است \lr{—} اگر میانگین نمونه \lr{\ensuremath{\approx}} واریانس نمونه باشد، داده ممکن است پواسون باشد.

\PSsubsection{پواسون به‌عنوان تقریب دوجمله‌ای}

وقتی \lr{n} بزرگ و \lr{p} کوچک باشد، با \lr{\ensuremath{\lambda} = np}:

\[\PSmtext{Binomial}(n, p) \approx \PSmtext{Poisson}(\lambda = np)\]

قاعده سرانگشتی: \lr{n \ensuremath{\ge} 20} و \lr{p \ensuremath{\le} 0.05}.

\PSsubsection{زمان استفاده}

\lr{\PSyes{}} شمارش رویدادها در یک بازه ثابت\PSbr{}\lr{\PSyes{}} رویدادها مستقل رخ می‌دهند\PSbr{}\lr{\PSyes{}} رویدادها با نرخ میانگین ثابت رخ می‌دهند\PSbr{}\lr{\PSyes{}} رویدادهای نادر با فرصت‌های فراوان (\lr{n} بزرگ، \lr{p} کوچک)\PSbr{}\lr{\PSyes{}} میانگین \lr{\ensuremath{\approx}} واریانس در داده

\PSsubsection{زمان عدم استفاده}

\PSno{} رویدادها مستقل نیستند (ورودهای خوشه‌ای)\PSbr{}\PSno{} نرخ با زمان تغییر می‌کند (فرایند ناهمگن)\PSbr{}\PSno{} رویدادها نادر نیستند (\lr{p} کوچک نیست) \lr{—} از دوجمله‌ای استفاده کنید\PSbr{}\PSno{} میانگین \lr{\ensuremath{\neq}} واریانس (به بیش‌پراکندگی/کم‌پراکندگی توجه کنید)
...
\tightlist
\item
  فاز یک سیگنال دریافتی (نامعلوم، با احتمال برابر در \lr{[0, 2\ensuremath{\pi})})
\item
  خطای گِردکردن در یک مبدل آنالوگ به دیجیتال (یکنواخت در \lr{[-\ensuremath{\Delta}/2, \ensuremath{\Delta}/2]} که \lr{\ensuremath{\Delta} =} گام کوانتیزاسیون)
\item
...
\lr{X \textasciitilde{} Uniform(a, b)} یا \lr{X \textasciitilde{} U(a, b)}

\end{PSbox}

\PSsubsection{\lr{PDF}}

\[f_X(x) = \begin{cases} \frac{1}{b-a} & a \leq x \leq b \\ 0 & \PSmtext{otherwise} \end{cases}\]
...
\[\PSmtext{Var}(X) = \frac{(b-a)^2}{12}\]

\PSsubsection{زمان استفاده}

\lr{\PSyes{}} تمام مقادیر در یک محدوده احتمال برابر دارند\PSbr{}\lr{\PSyes{}} هیچ اطلاعاتی به نفع مقدار خاصی نیست\PSbr{}\lr{\PSyes{}} مدل‌سازی خطای کوانتیزاسیون\PSbr{}\lr{\PSyes{}} فاز حامل بدون مدولاسیون

\PSsubsection{زمان عدم استفاده}

\PSno{} مقادیر نزدیک به مرکز محتمل‌تر هستند (از گاوسی استفاده کنید)\PSbr{}\PSno{} مقادیر نامنفی با کاهش نمایی هستند (از نمایی استفاده کنید)\PSbr{}\PSno{} محدوده کران‌دار نیست
...
\lr{X \textasciitilde{} Exponential(\ensuremath{\lambda})} که در آن \lr{\ensuremath{\lambda} =} پارامتر نرخ (رویداد در واحد زمان).

پارامتربندی جایگزین: \lr{X \textasciitilde{} Exp(\ensuremath{\beta})} که در آن \lr{\ensuremath{\beta} = 1/\ensuremath{\lambda} =} میانگین.
...
یا با میانگین \lr{\ensuremath{\beta}: f\textsubscript{X}(x) = (1/\ensuremath{\beta})e\textsuperscript{-x/\ensuremath{\beta}}}
...
\[\PSmtext{Var}(X) = \frac{1}{\lambda^2} = \beta^2\]
...
\textbf{معنای مهندسی:} قطعه‌ای که \lr{s} ساعت کار کرده است، از نظر آماری \lr{«}به‌خوبی نو\lr{»} است. عمر باقی‌مانده آن همان توزیع یک قطعه کاملاً نو را دارد.

\PSsubsection{رابطه با پواسون}

اگر رویدادها به‌صورت یک فرایند پواسون با نرخ \lr{\ensuremath{\lambda}} وارد شوند:

\begin{itemize}
\tightlist
\item
  تعداد رویدادها در زمان \lr{T \textasciitilde{} Poisson(\ensuremath{\lambda}T)}
\item
  زمان میان رویدادها \lr{\textasciitilde{} Exponential(\ensuremath{\lambda})}
\end{itemize}

\PSsubsection{زمان استفاده}

\lr{\PSyes{}} مدل‌سازی زمان میان رویدادها در یک فرایند پواسون\PSbr{}\lr{\PSyes{}} عمر قطعه با نرخ خرابی ثابت\PSbr{}\lr{\PSyes{}} فرض بی‌حافظه معقول است

\PSsubsection{زمان عدم استفاده}

\PSno{} نرخ خرابی با سن افزایش می‌یابد (فرسودگی) \lr{—} از وایبول یا گاما استفاده کنید\PSbr{}\PSno{} نرخ خرابی با سن کاهش می‌یابد (سوختگی اولیه) \lr{—} از وایبول استفاده کنید\PSbr{}\PSno{} داده افزایش خطر را نشان می‌دهد \lr{—} بی‌حافظه نیست
...
\item
  هر کمیتی که به‌خوبی با \lr{CLT} مدل می‌شود
\end{itemize}

\begin{PSbox}[unbreakable]{def}{تعریف ریاضی}

\lr{X \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2})} که در آن \lr{\ensuremath{\mu} =} میانگین و \lr{\ensuremath{\sigma}\textsuperscript{2} =} واریانس.
...
\[F_X(x) = \Phi\left(\frac{x-\mu}{\sigma}\right) = \frac{1}{2}\left[1 + \PSmtext{erf}\left(\frac{x-\mu}{\sigma\sqrt{2}}\right)\right]\]
...
\[\PSmtext{Var}(X) = \sigma^2\]

\PSsubsection{نرمال استاندارد}

\lr{Z \textasciitilde{} N(0, 1)}: شکل استانداردشده.

هر توزیع نرمال را می‌توان استانداردسازی کرد: \lr{Z = (X - \ensuremath{\mu})/\ensuremath{\sigma}}

\PSsubsection{قواعد کلیدی احتمال (قواعد \lr{\ensuremath{\sigma}})}
...
\bottomrule\noalign{}
\endlastfoot
\lr{\ensuremath{\mu} \ensuremath{\pm} 1\ensuremath{\sigma}} & \PSnum{68.27\%} \\
\lr{\ensuremath{\mu} \ensuremath{\pm} 2\ensuremath{\sigma}} & \PSnum{95.45\%} \\
\lr{\ensuremath{\mu} \ensuremath{\pm} 3\ensuremath{\sigma}} & \PSnum{99.73\%} \\
\lr{\ensuremath{\mu} \ensuremath{\pm} 4\ensuremath{\sigma}} & \PSnum{99.9937\%} \\
\end{longtable}
}
...
\tightlist
\item
  \textbf{متقارن} حول \lr{\ensuremath{\mu}}
\item
  \textbf{تبدیل خطی:} اگر \lr{X \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2})}، آنگاه \lr{aX + b \textasciitilde{} N(a\ensuremath{\mu} + b, a\textsuperscript{2}\ensuremath{\sigma}\textsuperscript{2})}
\item
  \textbf{جمع گاوسی‌ها:} اگر \lr{X \textasciitilde{} N(\ensuremath{\mu}\textsubscript{1}, \ensuremath{\sigma}\textsubscript{1}\textsuperscript{2})} و \lr{Y \textasciitilde{} N(\ensuremath{\mu}\textsubscript{2}, \ensuremath{\sigma}\textsubscript{2}\textsuperscript{2})} مستقل باشند، آنگاه \lr{X + Y \textasciitilde{} N(\ensuremath{\mu}\textsubscript{1}+\ensuremath{\mu}\textsubscript{2}, \ensuremath{\sigma}\textsubscript{1}\textsuperscript{2}+\ensuremath{\sigma}\textsubscript{2}\textsuperscript{2})}
\item
...
\lr{\PSyes{}} نویز حرارتی در مدارها\PSbr{}\lr{\PSyes{}} خطاهای اندازه‌گیری\PSbr{}\lr{\PSyes{}} هر جمعی از بسیاری از اثرهای تصادفی مستقل \lr{(CLT)}\PSbr{}\lr{\PSyes{}} تغییرات ساخت\PSbr{}\lr{\PSyes{}} هنگامی که هیستوگرام داده زنگوله‌ای و متقارن است

\PSsubsection{زمان عدم استفاده}

\PSno{} کمیت‌های اکیداً مثبت (گاوسی مقادیر منفی را مجاز می‌داند)\PSbr{}\PSno{} داده‌های به‌شدت چوله\PSbr{}\PSno{} کمیت‌های کران‌دار (گاوسی تکیه‌گاه نامتناهی دارد)\PSbr{}\PSno{} شمارش‌های گسسته\PSbr{}\PSno{} پدیده‌های سنگین‌دنباله (از توزیع \lr{t} یا کوشی استفاده کنید)
...
\begin{PSbox}[breakable]{ex}{مثال شبیه‌سازی: تابع \lr{Q} و \lr{BER}}
...
\textbf{کل زمان انتظار تا \lr{k-}امین رویداد} در یک فرایند پواسون، یا هر جمعی از \lr{k} متغیر تصادفی نمایی مستقل را مدل می‌کند.
...
\lr{X \textasciitilde{} Gamma(\ensuremath{\alpha}, \ensuremath{\beta})} که در آن \lr{\ensuremath{\alpha} =} پارامتر شکل و \lr{\ensuremath{\beta} =} پارامتر مقیاس (یا نرخ \lr{\ensuremath{\lambda} = 1/\ensuremath{\beta}}).
...
که در آن برای \lr{\ensuremath{\alpha}} صحیح \lr{\ensuremath{\Gamma}(\ensuremath{\alpha}) = (\ensuremath{\alpha}-1)}!، و در حالت کلی \lr{\ensuremath{\Gamma}(\ensuremath{\alpha}) = \ensuremath{\int}\textsubscript{0}\textsuperscript{\ensuremath{\infty}} t\textsuperscript{\lr{(\ensuremath{\alpha}-1)e}}(-t)dt}.

\PSsubsection{\lr{CDF}}

برای \lr{\ensuremath{\alpha}} کلی فرم بسته ندارد. از طریق تابع گامای ناقص در دسترس است.
...
\bottomrule\noalign{}
\endlastfoot
\lr{\ensuremath{\alpha} = 1} & \lr{Exponential(\ensuremath{\beta})} \\
\lr{\ensuremath{\alpha} = n/2, \ensuremath{\beta} = 2} & \lr{Chi-square(n)} \\
\lr{\ensuremath{\alpha} = integer k} & \lr{Erlang(k, \ensuremath{\beta})} \\
\end{longtable}
}

\PSsubsection{زمان استفاده}

\lr{\PSyes{}} جمع عمرهای نمایی مستقل\PSbr{}\lr{\PSyes{}} زمان انتظار برای \lr{k-}امین رویداد پواسون\PSbr{}\lr{\PSyes{}} داده‌های چوله و نامنفی\PSbr{}\lr{\PSyes{}} شکل انعطاف‌پذیر (چولگی قابل تنظیم)

\PSsubsection{زمان عدم استفاده}

\PSno{} داده می‌تواند منفی باشد\PSbr{}\PSno{} داده متقارن است (از گاوسی استفاده کنید)\PSbr{}\PSno{} توزیع ساده‌تری برازش می‌شود (نمایی برای \lr{k=1})
...
\textbf{اندازه (پوش)} یک بردار تصادفی گاوسی دوبعدی را مدل می‌کند. هنگامی که \lr{X} و \lr{Y} مستقل و \lr{N(0, \ensuremath{\sigma}\textsuperscript{2})} باشند، آنگاه \lr{R = \ensuremath{\surd}(X\textsuperscript{2} + Y\textsuperscript{2})} از توزیع رایلی پیروی می‌کند.
...
\tightlist
\item
  پوش نویز باریک‌باند (هم‌فاز \lr{+} مربعی)
\item
...
\lr{R \textasciitilde{} Rayleigh(\ensuremath{\sigma})}، که در آن \lr{\ensuremath{\sigma}} پارامتر است (نه انحراف معیار \lr{R}).
...
\[\PSmtext{Var}(R) = \frac{4-\pi}{2}\sigma^2 \approx 0.4292\sigma^2\]
...
\tightlist
\item
  \lr{R\textsuperscript{2} \textasciitilde{} Exponential(2\ensuremath{\sigma}\textsuperscript{2}) —} توان به‌صورت نمایی توزیع شده است
\item
  اگر \lr{X, Y \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2})} مستقل باشند، آنگاه \lr{\ensuremath{\surd}(X\textsuperscript{2}+Y\textsuperscript{2}) \textasciitilde{} Rayleigh(\ensuremath{\sigma})}
\end{itemize}

\PSsubsection{زمان استفاده}

\lr{\PSyes{}} پوش/اندازه یک سیگنال گاوسی مختلط\PSbr{}\lr{\PSyes{}} مدل‌سازی کانال محوشدگی رایلی (بدون خط دید)\PSbr{}\lr{\PSyes{}} فاصله دوبعدی با خطاهای گاوسی در هر بُعد\PSbr{}\lr{\PSyes{} RSS} (جذر مجموع مربعات) دو مؤلفه گاوسی مستقل

\PSsubsection{زمان عدم استفاده}

\PSno{} مؤلفه خط دید قوی موجود است (از رایسی استفاده کنید)\PSbr{}\PSno{} اندازه/پوش را مدل نمی‌کنید\PSbr{}\PSno{} مؤلفه‌ها گاوسی نیستند یا واریانس برابر ندارند
...
اگر \lr{Z\textsubscript{1}, Z\textsubscript{2}, …, Z\textsubscript{n}} مستقل و \lr{N(0,1)} باشند، آنگاه:\PSbr{}\[\chi^2 = Z_1^2 + Z_2^2 + \cdots + Z_n^2 \sim \chi^2(n)\]
...
\tightlist
\item
  \lr{Chi-square(n) = Gamma(n/2, 2)}
\item
  \lr{Chi-square(1) =} مربع \lr{N(0,1)}
\item
  \lr{Chi-square(2) = Exponential(1/2)}
\item
  برای \lr{n} بزرگ: \lr{\ensuremath{\chi}\textsuperscript{2}(n) \ensuremath{\approx} N(n, 2n)} بر اساس \lr{CLT}
\end{itemize}

\PSsubsection{زمان استفاده}

\lr{\PSyes{}} جمع مربعات متغیرهای تصادفی گاوسی\PSbr{}\lr{\PSyes{}} آزمون واریانس و بازه‌های اطمینان\PSbr{}\lr{\PSyes{}} آزمون‌های نیکویی برازش\PSbr{}\lr{\PSyes{}} آشکارسازی انرژی در \lr{N} بُعد

\PSsubsection{زمان عدم استفاده}

\PSno{} متغیرها گاوسی نیستند\PSbr{}\PSno{} متغیرها مستقل نیستند\PSbr{}\PSno{} متغیرها نرمال استاندارد نیستند (نیاز به مقیاس‌بندی دارند)
...
\endlastfoot
خروجی دودویی & برنولی \\
\lr{«n} آزمایش، \lr{k} موفقیت\lr{»} & دوجمله‌ای \\
\lr{«}چند تا تا نخستین\lr{…»} & هندسی \\
\lr{«}رویداد در هر بازه\lr{»} & پواسون \\
\lr{«}شانس برابر در هر جای \lr{[a,b]»} & یکنواخت \\
\lr{«}زمان تا رویداد بعدی\lr{»} & نمایی \\
\lr{«}جمع بسیاری از اثرها\lr{»} یا \lr{«}نویز\lr{»} & گاوسی \\
\lr{«}کل زمان انتظار برای \lr{k} رویداد\lr{»} & گاما \\
\lr{«}پوش سیگنال\lr{»} یا \lr{«}محوشدگی\lr{»} & رایلی \\
\lr{«}جمع مربعات نرمال‌ها\lr{»} & کای‌دو \\
\end{longtable}
}
...
\toprule\noalign{}
\rowcolor{PSnavy}\PSth{توزیع} & \PSth{نوع} & \PSth{پارامترها} & \PSth{میانگین} & \PSth{واریانس} & \PSth{\lr{MATLAB}} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
برنولی & گسسته & \lr{p} & \lr{p} & \lr{p(1-p)} & \lr{binornd(1,p)} \\
دوجمله‌ای & گسسته & \lr{n, p} & \lr{np} & \lr{np(1-p)} & \lr{binornd(n,p)} \\
هندسی & گسسته & \lr{p} & \lr{1/p} & \lr{(1-p)/p\textsuperscript{2}} & \lr{geornd(p)+1} \\
دوجمله‌ای منفی & گسسته & \lr{r, p} & \lr{r/p} & \lr{r(1-p)/p\textsuperscript{2}} & \lr{nbinrnd(r,p)+r} \\
پواسون & گسسته & \lr{\ensuremath{\lambda}} & \lr{\ensuremath{\lambda}} & \lr{\ensuremath{\lambda}} & \lr{poissrnd(\ensuremath{\lambda})} \\
یکنواخت & پیوسته & \lr{a, b} & \lr{(a+b)/2} & \lr{(b-a)\textsuperscript{2}/12} & \lr{unifrnd(a,b)} \\
نمایی & پیوسته & \lr{\ensuremath{\lambda}} (یا \lr{\ensuremath{\beta}=1/\ensuremath{\lambda}}) & \lr{1/\ensuremath{\lambda}} & \lr{1/\ensuremath{\lambda}\textsuperscript{2}} & \lr{exprnd(\ensuremath{\beta})} \\
گاوسی & پیوسته & \lr{\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}} & \lr{\ensuremath{\mu}} & \lr{\ensuremath{\sigma}\textsuperscript{2}} & \lr{normrnd(\ensuremath{\mu},\ensuremath{\sigma})} \\
گاما & پیوسته & \lr{\ensuremath{\alpha}, \ensuremath{\beta}} & \lr{\ensuremath{\alpha}\ensuremath{\beta}} & \lr{\ensuremath{\alpha}\ensuremath{\beta}\textsuperscript{2}} & \lr{gamrnd(\ensuremath{\alpha},\ensuremath{\beta})} \\
رایلی & پیوسته & \lr{\ensuremath{\sigma}} & \lr{\ensuremath{\sigma}\ensuremath{\surd}(\ensuremath{\pi}/2)} & \lr{(4-\ensuremath{\pi})\ensuremath{\sigma}\textsuperscript{2}/2} & \lr{raylrnd(\ensuremath{\sigma})} \\
کای‌دو & پیوسته & \lr{n} & \lr{n} & \lr{2n} & \lr{chi2rnd(n)} \\
\end{longtable}
}
...
\item
  پوش سیگنال دریافتی از مسیر چندگانه بدون \lr{LOS}
\end{enumerate}

\PSsolution{} \lr{(a) Poisson(\ensuremath{\lambda}=2)}، \lr{(b) Gaussian(0, \ensuremath{\sigma}\textsuperscript{2})}، \lr{(c) Exponential(\ensuremath{\lambda})}، \lr{(d) Binomial(100, 0.03)}، \lr{(e) Rayleigh(\ensuremath{\sigma})}

\end{PSbox}

\begin{PSbox}[unbreakable]{prob}{مسیله 2: نمایی در برابر پواسون}

بسته‌ها به‌صورت یک فرایند پواسون با نرخ \lr{\ensuremath{\lambda} = 5} بسته بر ثانیه به یک مسیریاب می‌رسند.

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\tightlist
\item
  \lr{P}(بیش از 8 بسته در 1 ثانیه) چقدر است؟
\item
  \lr{P}(هیچ بسته‌ای در \PSnum{0.5} ثانیه بعدی نرسد) چقدر است؟
\item
...
\tightlist
\item
  \lr{X \textasciitilde{} Poisson(5): P(X \textgreater{} 8) = 1 - poisscdf(8, 5) = 0.0681}
\item
  \lr{T \textasciitilde{} Exp(5): P(T \textgreater{} 0.5) = e\textsuperscript{-5\ensuremath{\times}0.5} = e\textsuperscript{-2.5} = 0.0821}
\item
  نمایی با نرخ \lr{\ensuremath{\lambda} = 5}
\item
  میانگین \lr{= 1/\ensuremath{\lambda} = 0.2} ثانیه
\end{enumerate}
...
یک مقاومت مقدار نامی \lr{1000\ensuremath{\Omega}} با تغییرات ساخت مدل‌شده به‌صورت \lr{N(1000, 25)} دارد \lr{(\ensuremath{\sigma} = 5\ensuremath{\Omega})}.

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\tightlist
\item
  چه کسری از مقاومت‌ها در محدوده \lr{\ensuremath{\pm}10\ensuremath{\Omega}} حول مقدار نامی هستند (مشخصه: \lr{990-1010\ensuremath{\Omega}})؟
\item
  چه کسری در آزمون رواداری \lr{\ensuremath{\pm}1\%} رد می‌شوند؟
\item
...
\tightlist
\item
  \lr{P(990 \ensuremath{\le} X \ensuremath{\le} 1010) = P(-2 \ensuremath{\le} Z \ensuremath{\le} 2) = 0.9545 \ensuremath{\to} 95.45\%}
\item
  \lr{\ensuremath{\pm}1\% = \ensuremath{\pm}10\ensuremath{\Omega} \ensuremath{\to}} همانند \lr{(a)}، نرخ رد \lr{= 1 - 0.9545 = 4.55\%}
\item
  \lr{\ensuremath{\pm}3\ensuremath{\sigma} = \ensuremath{\pm}15\ensuremath{\Omega} \ensuremath{\to}} محدوده \lr{[985, 1015]\ensuremath{\Omega}}
\end{enumerate}

\emph{ماژول بعدی: {ماژول \lr{4 —} امید ریاضی و گشتاورها}}\PSbr{}
