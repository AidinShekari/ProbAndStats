...
\tightlist
\item
  میانگین \lr{BER} چقدر است هنگامی که کانال به‌طور تصادفی میان خوب و بد جابه‌جا می‌شود؟
\item
  واریانس کل اندازه‌گیری چقدر است هنگامی که حسگر در محیط‌های مختلف کار می‌کند؟
\end{itemize}

\PSsection{8.2}{امید ریاضی شرطی \lr{E[X\textbar{}Y=y]}}

\begin{PSbox}[unbreakable]{def}{تعریف}

\textbf{پیوسته:}\PSbr{}\[E[X|Y=y] = \int_{-\infty}^{\infty} x \cdot f_{X|Y}(x|y) \, dx\]

\textbf{گسسته:}\PSbr{}\[E[X|Y=y] = \sum_x x \cdot p_{X|Y}(x|y)\]

\end{PSbox}

\PSsubsection{تفسیر}

\lr{E[X\textbar{}Y=y]} همان \textbf{مقدار میانگین \lr{X}} است هنگامی که \lr{Y = y} را می‌دانیم. این تابعی از \lr{y} است.

\PSsubsection{\lr{E[X\textbar{}Y]} به‌عنوان یک متغیر تصادفی}

هنگامی که خود \lr{Y} تصادفی باشد، \lr{E[X\textbar{}Y]} یک \textbf{متغیر تصادفی} است \lr{—} وقتی \lr{Y = y} باشد مقدار \lr{E[X\textbar{}Y=y]} را اختیار می‌کند.
...
\tightlist
\item
  کانال خوب \lr{(Y=1): E[P\textbar{}Y=1] = 10 mW}
\item
  کانال ضعیف \lr{(Y=0): E[P\textbar{}Y=0] = 1 mW}
\item
...
\PSsection{8.3}{واریانس شرطی \lr{Var(X\textbar{}Y=y)}}

\begin{PSbox}[unbreakable]{def}{تعریف}

\[\PSmtext{Var}(X|Y=y) = E[(X - E[X|Y=y])^2 | Y=y] = E[X^2|Y=y] - (E[X|Y=y])^2\]
...
\tightlist
\item
  در \lr{T=20\ensuremath{^{\circ}}C: Var(X\textbar{}T=20) = 0.01} (دقت بالا)
\item
  در \lr{T=80\ensuremath{^{\circ}}C: Var(X\textbar{}T=80) = 0.09} (دقت پایین به‌دلیل اثرهای گرمایی)
\end{itemize}
...
\textbf{نسخه گسسته:}\PSbr{}\[E[X] = \sum_y E[X|Y=y] \cdot P(Y=y)\]

\textbf{نسخه پیوسته:}\PSbr{}\[E[X] = \int_{-\infty}^{\infty} E[X|Y=y] \cdot f_Y(y) \, dy\]

\PSsubsection{اثبات (گسسته)}

\lr{E[E[X\textbar{}Y]] = \ensuremath{\Sigma}\textsubscript{y} E[X\textbar{}Y=y] \ensuremath{\cdot} P(Y=y)}\PSbr{}\lr{= \ensuremath{\Sigma}\textsubscript{y} [\ensuremath{\Sigma}\textsubscript{x} x \ensuremath{\cdot} P(X=x\textbar{}Y=y)] \ensuremath{\cdot} P(Y=y)}\PSbr{}\lr{= \ensuremath{\Sigma}\textsubscript{x} \ensuremath{\Sigma}\textsubscript{y} x \ensuremath{\cdot} P(X=x, Y=y)}\PSbr{}\lr{= \ensuremath{\Sigma}\textsubscript{x} x \ensuremath{\cdot} P(X=x) = E[X] \PSyes{}}

\PSsubsection{تفسیر}

\textbf{میانگین کل} برابر است با \textbf{میانگین وزنی میانگین‌های شرطی}، با وزن احتمال هر شرط.

\begin{PSbox}[unbreakable]{ex}{مثال مهندسی: میانگین \lr{BER} در وضعیت‌های کانال}
...
\tightlist
\item
  حالت 1 (60\% زمان): \lr{BER = 10\textsuperscript{-6}}
\item
  حالت 2 (30\% زمان): \lr{BER = 10\textsuperscript{-4}}
\item
  حالت 3 (10\% زمان): \lr{BER = 10\textsuperscript{-2}}
\end{itemize}

\lr{BER} کل \lr{= E[BER] = 0.6(10\textsuperscript{-6}) + 0.3(10\textsuperscript{-4}) + 0.1(10\textsuperscript{-2})}\PSbr{}\lr{= 0.6\ensuremath{\times}10\textsuperscript{-6} + 30\ensuremath{\times}10\textsuperscript{-6} + 10000\ensuremath{\times}10\textsuperscript{-6} \ensuremath{\approx} 1.003 \ensuremath{\times} 10\textsuperscript{-3}}

\textbf{نکته مهندسی:} \lr{BER} کل تحت تسلط بدترین حالت است، هرچند که این حالت تنها در 10\% زمان رخ می‌دهد.
...
\tightlist
\item
  بدون نقص: \lr{E[T\textbar{}good] = 10000} ساعت
\item
  با نقص: \lr{E[T\textbar{}defect] = 500} ساعت
\end{itemize}
...
\[\PSmtext{Var}(X) = E[\PSmtext{Var}(X|Y)] + \PSmtext{Var}(E[X|Y])\]
...
\bottomrule\noalign{}
\endlastfoot
\lr{Var(X)} & واریانس کل & عدم‌قطعیت کل در \lr{X} \\
\lr{E[Var(X\textbar{}Y)]} & واریانس شرطی مورد انتظار & تغییرپذیری \lr{«}درون‌گروهی\lr{»} میانگین \\
\lr{Var(E[X\textbar{}Y])} & واریانس میانگین‌های شرطی & تغییرپذیری \lr{«}بین‌گروهی\lr{»} \\
\end{longtable}
}

\PSsubsection{اثبات}

\lr{Var(X) = E[X\textsuperscript{2}] - (E[X])\textsuperscript{2}}

\lr{E[X\textsuperscript{2}] = E[E[X\textsuperscript{2}\textbar{}Y]]} (خاصیت برج)\PSbr{}\lr{= E[Var(X\textbar{}Y) + (E[X\textbar{}Y])\textsuperscript{2}]}\PSbr{}\lr{= E[Var(X\textbar{}Y)] + E[(E[X\textbar{}Y])\textsuperscript{2}]}

\lr{(E[X])\textsuperscript{2} = (E[E[X\textbar{}Y]])\textsuperscript{2}}

\lr{Var(X) = E[Var(X\textbar{}Y)] + E[(E[X\textbar{}Y])\textsuperscript{2}] - (E[E[X\textbar{}Y]])\textsuperscript{2}}\PSbr{}\lr{= E[Var(X\textbar{}Y)] + Var(E[X\textbar{}Y]) \PSyes{}}

\PSsubsection{تفسیر: تجزیه عدم‌قطعیت}

عدم‌قطعیت کل \lr{=} میانگین عدم‌قطعیت درون هر شرط \lr{+} تغییرپذیری میانگین میان شرایط
...
\tightlist
\item
  فضای بسته: \lr{E[X\textbar{}Y=1] = 25\ensuremath{^{\circ}}C}، \lr{Var(X\textbar{}Y=1) = 1}
\item
  فضای باز: \lr{E[X\textbar{}Y=2] = 15\ensuremath{^{\circ}}C}، \lr{Var(X\textbar{}Y=2) = 16}
\end{itemize}

\lr{\textbf{\lr{E[Var(X\textbar{}Y)]}} = 0.6(1) + 0.4(16) = 7.0} (میانگین واریانس درون‌شرطی)

\lr{\textbf{\lr{Var(E[X\textbar{}Y])}} = E[(E[X\textbar{}Y])\textsuperscript{2}] - (E[E[X\textbar{}Y]])\textsuperscript{2}}\PSbr{}\lr{= 0.6(25\textsuperscript{2}) + 0.4(15\textsuperscript{2}) - [0.6(25) + 0.4(15)]\textsuperscript{2} = 375 + 90 - (21)\textsuperscript{2} = 465 - 441 = 24}

\lr{\textbf{\lr{Var(X)}} = 7.0 + 24.0 = 31.0}

بیشتر واریانس از تفاوت \textbf{بین‌شرطی} ناشی می‌شود (24 از 31).

\end{PSbox}

\PSsection{8.6}{مثال‌های \lr{MATLAB}}
...
\tightlist
\item
  نوع \lr{A} (احتمال \lr{0.7): E[delay\textbar{}A] = 5ms}، \lr{Var(delay\textbar{}A) = 2}
\item
  نوع \lr{B} (احتمال \lr{0.3): E[delay\textbar{}B] = 20ms}، \lr{Var(delay\textbar{}B) = 25}
\end{itemize}

موارد زیر را بیابید: \lr{(a) E[delay]}، \lr{(b) Var(delay)}.
...
\tightlist
\item
  \lr{E[D] = 0.7(5) + 0.3(20) = 3.5 + 6 = 9.5 ms}
\item
  \lr{E[Var(D\textbar{}type)] = 0.7(2) + 0.3(25) = 1.4 + 7.5 = 8.9}\PSbr{}\lr{Var(E[D\textbar{}type]) = 0.7(5\textsuperscript{2}) + 0.3(20\textsuperscript{2}) - 9.5\textsuperscript{2} = 17.5 + 120 - 90.25 = 47.25}\PSbr{}\lr{Var(D) = 8.9 + 47.25 = 56.15}
\end{enumerate}
...
تعداد تصادفی \lr{N} از قطعات آزمایش می‌شوند. \lr{N \textasciitilde{} Poisson(10)}. هر قطعه احتمال خرابی مستقل \lr{p=0.05} دارد. فرض کنید \lr{X =} تعداد خرابی‌ها. \lr{E[X]} و \lr{Var(X)} را بیابید.

\PSsolution{} \lr{X\textbar{}N \textasciitilde{} Binomial(N, 0.05). E[X\textbar{}N] = 0.05N. Var(X\textbar{}N) = N(0.05)(0.95) = 0.0475N}.\PSbr{}\lr{E[X] = E[E[X\textbar{}N]] = E[0.05N] = 0.05(10) = 0.5}\PSbr{}\lr{Var(X) = E[Var(X\textbar{}N)] + Var(E[X\textbar{}N]) = E[0.0475N] + Var(0.05N) = 0.0475(10) + 0.05\textsuperscript{2}(10) = 0.475 + 0.025 = 0.5}
...
\emph{ماژول بعدی: {ماژول \lr{9 —} کوواریانس و همبستگی}}\PSbr{}
