...
\item
  \textbf{سیستم \lr{MIMO}:} ضرایب کانال \lr{M\ensuremath{\times}N}
\item
...
به چارچوبی برای بردارهای تصادفی \textbf{\lr{N-}بعدی} نیاز داریم.

\PSsection{10.2}{توزیع‌های مشترک برای \lr{N} متغیر تصادفی}

\PSsubsection{\lr{PDF} مشترک}

برای \lr{X\textsubscript{1}, X\textsubscript{2}, …, X\textsubscript{n}}:\PSbr{}\[f_{X_1,...,X_n}(x_1,...,x_n) \geq 0, \quad \int\cdots\int f_{X_1,...,X_n} \, dx_1 \cdots dx_n = 1\]

\PSsubsection{توزیع‌های حاشیه‌ای}

متغیرهای ناخواسته را انتگرال بگیرید (یا جمع کنید):\PSbr{}\[f_{X_1}(x_1) = \int\cdots\int f_{X_1,...,X_n}(x_1,...,x_n) \, dx_2 \cdots dx_n\]
...
\lr{X\textsubscript{1}, …, X\textsubscript{n}} به‌طور متقابل مستقل هستند اگر:\PSbr{}\[f_{X_1,...,X_n}(x_1,...,x_n) = \prod_{i=1}^n f_{X_i}(x_i)\]
...
\bottomrule\noalign{}
\endlastfoot
خروجی آرایه حسگر & \lr{X\textsubscript{1},…,X\textsubscript{n} =} قرایت‌های حسگر & پردازش آرایه \\
کانال \lr{MIMO} & \lr{h\textsubscript{1},…,h\textsubscript{n} =} بهره‌های کانال & مخابرات بی‌سیم \\
بردار نویز & \lr{N\textsubscript{1},…,N\textsubscript{n} =} نمونه‌های نویز & پردازش سیگنال \\
...
\tightlist
\item
  \lr{E[AX + b] = AE[X] + b = A\ensuremath{\mu} + b}
\item
...
درایه \lr{(i,j): \ensuremath{\Sigma}\textsubscript{ij} = Cov(X\textsubscript{i}, X\textsubscript{j})}

\[\boldsymbol{\Sigma} = \begin{bmatrix} \sigma_1^2 & \PSmtext{Cov}(X_1,X_2) & \cdots & \PSmtext{Cov}(X_1,X_n) \\ \PSmtext{Cov}(X_2,X_1) & \sigma_2^2 & \cdots & \PSmtext{Cov}(X_2,X_n) \\ \vdots & & \ddots & \vdots \\ \PSmtext{Cov}(X_n,X_1) & \cdots & \cdots & \sigma_n^2 \end{bmatrix}\]
...
\item
  \textbf{نیم‌معین مثبت:} \lr{v\textsuperscript{T}\ensuremath{\Sigma}v \ensuremath{\ge} 0} برای تمام \lr{v}
\item
  \textbf{قطر \lr{=} واریانس‌ها:} \lr{\ensuremath{\Sigma}\textsubscript{ii} = Var(X\textsubscript{i})}
\item
  \textbf{غیرقطری \lr{=} کوواریانس‌ها:} \lr{\ensuremath{\Sigma}\textsubscript{ij} = Cov(X\textsubscript{i}, X\textsubscript{j})}
\item
  \textbf{مؤلفه‌های مستقل \lr{\ensuremath{\to} \ensuremath{\Sigma}} قطری}
\end{enumerate}
...
\lr{X \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\Sigma})} دارای \lr{PDF} زیر است:
...
که در آن \lr{\textbar{}\ensuremath{\Sigma}\textbar{} =} دترمینان \lr{\ensuremath{\Sigma}}.
...
\item
  \textbf{ناهمبسته \lr{=} مستقل} (ویژه گاوسی!)
\item
  \textbf{کاملاً مشخص‌شده} با \lr{\ensuremath{\mu}} و \lr{\ensuremath{\Sigma}} (فقط 2 پارامتر برای هر متغیر به‌علاوه همبستگی‌ها لازم است)
\end{enumerate}
...
\tightlist
\item
  قضیه حد مرکزی: جمع‌های بسیاری از متغیرهای تصادفی \lr{\ensuremath{\to}} گاوسی چندمتغیره
\item
...
برای \lr{n=2} با \lr{\ensuremath{\mu} = [0,0]\textsuperscript{T}}، \lr{\ensuremath{\sigma}\textsubscript{1} = \ensuremath{\sigma}\textsubscript{2} = 1}:

\[f(x,y) = \frac{1}{2\pi\sqrt{1-\rho^2}} \exp\left(-\frac{x^2 - 2\rho xy + y^2}{2(1-\rho^2)}\right)\]

خطوط تراز \textbf{بیضی} هستند (در حالت \lr{\ensuremath{\rho} \ensuremath{\neq} 0} کج شده‌اند).

\PSsection{10.7}{تبدیل‌های خطی بردارهای تصادفی}

\PSsubsection{تبدیل \lr{Y = AX + b}}

اگر \lr{X} یک بردار تصادفی با میانگین \lr{\ensuremath{\mu}\textsubscript{X}} و کوواریانس \lr{\ensuremath{\Sigma}\textsubscript{X}} باشد:

\[E[\mathbf{Y}] = A\boldsymbol{\mu}_X + \mathbf{b}\]\PSbr{}\[\boldsymbol{\Sigma}_Y = A\boldsymbol{\Sigma}_X A^T\]

\PSsubsection{کاربردهای مهندسی}

\textbf{شکل‌دهی پرتو \lr{(Beamforming)}:} خروجی \lr{y = w\textsuperscript{T}X} (بردار وزن اعمال‌شده به آرایه حسگر)

\begin{itemize}
\tightlist
\item
  \lr{E[y] = w\textsuperscript{T}\ensuremath{\mu}}
\item
  \lr{Var(y) = w\textsuperscript{T}\ensuremath{\Sigma}w}
\end{itemize}

\textbf{سفیدسازی:} ماتریس \lr{W} را بیابید که در آن \lr{W\ensuremath{\Sigma}W\textsuperscript{T} = I} (داده را ناهمبسته می‌کند)

\begin{itemize}
\tightlist
\item
  \lr{W = \ensuremath{\Sigma}\textsuperscript{-1/2}}
\end{itemize}

\textbf{تحلیل مؤلفه‌های اصلی:} چرخش برای قطری‌کردن \lr{\ensuremath{\Sigma}}

\PSsection{10.8}{مثال‌های \lr{MATLAB}}
...
بردار تصادفی \lr{X = [X\textsubscript{1}, X\textsubscript{2}, X\textsubscript{3}]\textsuperscript{T}} با \lr{\ensuremath{\mu} = [1, 0, -1]\textsuperscript{T}} و ماتریس کوواریانس \lr{\ensuremath{\Sigma} = [4 1 0; 1 2 -1; 0 -1 3]}.

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\tightlist
\item
  \lr{Var(X\textsubscript{1} + X\textsubscript{2} + X\textsubscript{3})} را بیابید. \lr{(b) Cov(X\textsubscript{1}, X\textsubscript{2}+X\textsubscript{3})} را بیابید. \lr{(c)} آیا \lr{X\textsubscript{1}} و \lr{X\textsubscript{3}} ناهمبسته هستند؟
\end{enumerate}
...
\tightlist
\item
  \lr{Var(X\textsubscript{1}+X\textsubscript{2}+X\textsubscript{3}) = 1\textsuperscript{T}\ensuremath{\Sigma}1 = 4+2+3 + 2(1) + 2(0) + 2(-1) = 9 + 0 = 9}\PSbr{}توجه: \lr{= \ensuremath{\Sigma}\textsubscript{ij}} تمام درایه‌ها \lr{= 4+1+0+1+2+(-1)+0+(-1)+3 = 9}
\item
...
\item
  \lr{Cov(X\textsubscript{1},X\textsubscript{3}) = 0}، بنابراین بله، \lr{X\textsubscript{1}} و \lr{X\textsubscript{3}} ناهمبسته هستند.
\end{enumerate}
...
\lr{X \textasciitilde{} N(0, \ensuremath{\Sigma})} با \lr{\ensuremath{\Sigma} = [1 \ensuremath{\rho}; \ensuremath{\rho} 1]}. توزیع شرطی \lr{X\textsubscript{1}\textbar{}X\textsubscript{2}=x\textsubscript{2}} را بیابید.
...
میانگین شرطی تابعی خطی از \lr{x\textsubscript{2}} است و واریانس شرطی با ضریب \lr{(1-\ensuremath{\rho}\textsuperscript{2})} کاهش می‌یابد.

\end{PSbox}

\begin{PSbox}[unbreakable]{prob}{مسیله 3}

کد \lr{MATLAB} را برای تولید یک بردار گاوسی سه‌بعدی با \lr{\ensuremath{\mu}} و \lr{\ensuremath{\Sigma}} معلوم، محاسبه کوواریانس نمونه و بررسی آن بنویسید.
...
\emph{ماژول بعدی: {ماژول \lr{11 —} قضیه حد مرکزی}}\PSbr{}
