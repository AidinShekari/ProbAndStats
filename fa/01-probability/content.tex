...
\item
  آزمایش‌های تصادفی را در \lr{MATLAB} شبیه‌سازی کنند
\end{enumerate}
...
\lr{«}اگر 1000 بیت را روی یک کانال نویزی ارسال کنم، انتظار دارم چند خطا رخ دهد؟ آیا می‌توانم کمتر از 5 خطا را تضمین کنم؟\lr{»}
...
نمونه‌برداری از ولتاژ خروجی \lr{ADC} & پردازش سیگنال & کوانتیزه‌سازی \lr{+} نویز \\
\end{longtable}
}
...
\textbf{فضای نمونه} \lr{S} (یا \lr{\ensuremath{\Omega}}) مجموعه تمام خروجی‌های ممکن یک آزمایش تصادفی است.
...
\item
  تعداد خطاها در یک بسته \lr{n} بیتی: \lr{S = \{0, 1, 2, …, n\}}
\item
  تعداد ورودها به یک سرور: \lr{S = \{0, 1, 2, 3, …\}}
\end{itemize}
...
\tightlist
\item
  ولتاژ اندازه‌گیری‌شده: \lr{S = \ensuremath{\mathbb{R}}} (یا یک بازه عملی مانند \lr{[0, 5] V})
\item
  زمان تا خرابی قطعه: \lr{S = [0, \ensuremath{\infty})}
\item
  فاز سیگنال دریافتی: \lr{S = [0, 2\ensuremath{\pi})}
\end{itemize}
...
\item
  \textbf{رویداد ناممکن}: مجموعه تهی \lr{\ensuremath{\emptyset}} (هرگز رخ نمی‌دهد)
\end{itemize}
...
\bottomrule\noalign{}
\endlastfoot
اجتماع & \lr{A \ensuremath{\cup} B} & \lr{A} یا \lr{B} (یا هر دو) رخ می‌دهد & هر یک از حالت‌های خرابی \lr{A} یا \lr{B} موجب فعال‌شدن هشدار می‌شود \\
اشتراک & \lr{A \ensuremath{\cap} B} & \lr{A} و \lr{B} هر دو رخ می‌دهند & هر دو حسگر سیگنال را تشخیص می‌دهند \\
متمم & \lr{A\textsuperscript{c} or Ā} & \lr{A} رخ نمی‌دهد & قطعه خراب نمی‌شود \\
تفاضل & \lr{A - B = A \ensuremath{\cap} B\textsuperscript{c}} & \lr{A} رخ می‌دهد ولی \lr{B} رخ نمی‌دهد & حسگر \lr{A} فعال می‌شود ولی حسگر \lr{B} فعال نمی‌شود \\
ناسازگار متقابل & \lr{A \ensuremath{\cap} B = \ensuremath{\emptyset}} & \lr{A} و \lr{B} نمی‌توانند هر دو رخ دهند & قطعه نمی‌تواند هم سالم و هم خراب باشد \\
\end{longtable}
}
...
\tightlist
\item
  \lr{A \ensuremath{\cup} B \ensuremath{\cup} C = «}برد دست‌کم یک عیب دارد\lr{»}
\item
  \lr{A \ensuremath{\cap} B = «}برد هم عیب پل لحیم و هم یک قطعه مفقود دارد\lr{»}
\item
  \lr{(A \ensuremath{\cup} B \ensuremath{\cup} C)\textsuperscript{c} = «}برد بدون عیب است\lr{»}
\end{itemize}
...
\textbf{اصل موضوعه 1 (نامنفی‌بودن):}\PSbr{}\[P(A) \geq 0 \quad \PSmtext{for any event } A\]

\textbf{اصل موضوعه 2 (نرمال‌سازی):}\PSbr{}\[P(S) = 1\]

\textbf{اصل موضوعه 3 (جمع‌پذیری شمارا):}\PSbr{}برای رویدادهای ناسازگار متقابل \lr{A\textsubscript{1}}، \lr{A\textsubscript{2}}، \lr{A\textsubscript{3}}، \lr{…}:\PSbr{}\[P(A_1 \cup A_2 \cup A_3 \cup \cdots) = P(A_1) + P(A_2) + P(A_3) + \cdots\]

\PSsubsection{ویژگی‌های استخراج‌شده از اصول موضوعه}

\textbf{ویژگی 1: قاعده متمم}\PSbr{}\[P(A^c) = 1 - P(A)\]

\emph{اثبات:} \lr{S = A \ensuremath{\cup} A\textsuperscript{c}} و \lr{A \ensuremath{\cap} A\textsuperscript{c} = \ensuremath{\emptyset}}، بنابراین \lr{P(S) = P(A) + P(A\textsuperscript{c}) = 1}.

\emph{معنای مهندسی:} \lr{P}(عملکرد قطعه) \lr{= 1 - P}(خرابی قطعه)

\textbf{ویژگی 2: رویداد ناممکن}\PSbr{}\[P(\emptyset) = 0\]

\textbf{ویژگی 3: یکنوایی}\PSbr{}اگر \lr{A \ensuremath{\subseteq} B}، آنگاه \lr{P(A) \ensuremath{\le} P(B)}

\textbf{ویژگی 4: کران}\PSbr{}\[0 \leq P(A) \leq 1\]

\textbf{ویژگی 5: شمول و عدم‌شمول (دو رویداد)}\PSbr{}\[P(A \cup B) = P(A) + P(B) - P(A \cap B)\]

\emph{معنای مهندسی:} هنگام محاسبه احتمال \lr{«}خرابی \lr{A} یا خرابی \lr{B»}، باید هم‌پوشانی‌ای را که در آن هر دو به‌طور هم‌زمان خراب می‌شوند، کم کنیم.

\PSsection{1.6}{قواعد احتمال}

\PSsubsection{قاعده جمع}

برای هر دو رویداد:\PSbr{}\[P(A \cup B) = P(A) + P(B) - P(A \cap B)\]

برای رویدادهای ناسازگار متقابل \lr{(A \ensuremath{\cap} B = \ensuremath{\emptyset})}:\PSbr{}\[P(A \cup B) = P(A) + P(B)\]
...
\lr{P}(دست‌کم یک خطا در 100 بیت) \lr{= 1 - P}(صفر خطا در 100 بیت)
...
\item
  \lr{P}(دست‌کم یکی خراب می‌شود) \lr{= P(A \ensuremath{\cup} B \ensuremath{\cup} C)}
\item
...
به‌صورت جایگزین: \lr{P}(دست‌کم یکی خراب می‌شود) \lr{= 1 - P}(هیچ‌کدام خراب نمی‌شوند) \lr{= 1 - (0.95)\textsuperscript{3} = 0.142625 \PSyes{}}
...
احتمال شرطی دانش ما را \textbf{به‌روزرسانی} می‌کند. هنگامی که می‌دانیم \lr{B} رخ داده است، فضای نمونه مؤثر از \lr{S} به \lr{B} کاهش می‌یابد و می‌پرسیم: \lr{«}درون \lr{B}، احتمال \lr{A} چقدر است؟\lr{»}
...
احتمال شرطی & \lr{P(A\textbar{}B)} & احتمال \lr{A} \textbf{با توجه به اینکه} \lr{B} رخ داده است \\
احتمال مشترک & \lr{P(A \ensuremath{\cap} B)} & احتمال اینکه \textbf{هر دو} \lr{A} و \lr{B} رخ دهند \\
\end{longtable}
}
...
\tightlist
\item
  \lr{H\textsubscript{1} =} هواپیما وجود دارد (هدف)
\item
  \lr{H\textsubscript{0} =} هواپیما وجود ندارد (بدون هدف)
\item
  \lr{D =} آشکارساز می‌گوید \lr{«}هدف وجود دارد\lr{»}
\end{itemize}
...
\tightlist
\item
  \lr{P(D \textbar{} H\textsubscript{1}) =} \textbf{احتمال آشکارسازی} (حساسیت)
\item
  \lr{P(D \textbar{} H\textsubscript{0}) =} \textbf{احتمال هشدار کاذب}
\item
  \lr{P(D\textsuperscript{c} \textbar{} H\textsubscript{1}) =} \textbf{احتمال عدم‌آشکارسازی} \lr{= 1 - P(D \textbar{} H\textsubscript{1})}
\end{itemize}

فرض کنید: \lr{P(H\textsubscript{1}) = 0.01}، \lr{P(D\textbar{}H\textsubscript{1}) = 0.95}، \lr{P(D\textbar{}H\textsubscript{0}) = 0.02}

\lr{P}(هدف واقعاً وجود دارد \lr{\textbar{}} آشکارساز وجود هدف را اعلام می‌کند) چقدر است؟
...
\tightlist
\item
  \lr{P}(دریافت 1 \lr{\textbar{}} ارسال \lr{0) = p = 0.01}
\item
  \lr{P}(دریافت 0 \lr{\textbar{}} ارسال \lr{1) = p = 0.01}
\item
  \lr{P}(دریافت 0 \lr{\textbar{}} ارسال \lr{0) = 1 - p = 0.99}
\item
  \lr{P}(دریافت 1 \lr{\textbar{}} ارسال \lr{1) = 1 - p = 0.99}
\end{itemize}
...
\tightlist
\item
  \lr{P}(ارسال \lr{0 \ensuremath{\cap}} دریافت \lr{1) = P}(دریافت 1 \lr{\textbar{}} ارسال \lr{0) \ensuremath{\cdot} P}(ارسال \lr{0) = 0.01 \ensuremath{\times} 0.5 = 0.005}
\item
  \lr{P}(خطا) \lr{= P}(ارسال \lr{0 \ensuremath{\cap}} دریافت \lr{1) + P}(ارسال \lr{1 \ensuremath{\cap}} دریافت \lr{0) = 0.005 + 0.005 = 0.01}
\end{itemize}
...
\tightlist
\item
  \lr{P(A\textbar{}B) \ensuremath{\ge} 0}
\item
  \lr{P(S\textbar{}B) = 1}
\item
...
برای سه رویداد:\PSbr{}\[P(A \cap B \cap C) = P(A) \cdot P(B|A) \cdot P(C|A \cap B)\]
...
به‌طور معادل: \lr{P(A\textbar{}B) = P(A) —} دانستن اینکه \lr{B} رخ داده است، هیچ اطلاعاتی درباره \lr{A} نمی‌دهد.
...
\tightlist
\item
  \lr{P(A \ensuremath{\cap} B) = P(A)P(B)}
\item
  \lr{P(A \ensuremath{\cap} C) = P(A)P(C)}
\item
  \lr{P(B \ensuremath{\cap} C) = P(B)P(C)}
\end{itemize}
...
\item
  \lr{P(A \ensuremath{\cap} B \ensuremath{\cap} C) = P(A)P(B)P(C)}
\end{itemize}

\begin{PSquote}

\lr{\PSwarn{}} استقلال زوجی، استقلال متقابل را نتیجه نمی‌دهد!
...
\lr{P}(هر سه خراب شوند) \lr{= P(A)P(B)P(C) = (0.1)\textsuperscript{3} = 0.001}

\lr{P}(سیستم کار می‌کند) \lr{= P}(دست‌کم یکی کار می‌کند) \lr{= 1 - P}(همه خراب شوند) \lr{= 1 - 0.001 = 0.999}
...
\tightlist
\item
  \lr{P(A)}، \lr{P(B)} و \lr{P(A \ensuremath{\cap} B)} را به‌صورت تجربی محاسبه کنید
\item
...
\lr{\PSno{} «}رویدادهای ناسازگار متقابل مستقل هستند\lr{»}

\end{PSquote}

این گزاره \textbf{نادرست} است. اگر \lr{A \ensuremath{\cap} B = \ensuremath{\emptyset}} و \lr{P(A) \textgreater{} 0}، \lr{P(B) \textgreater{} 0}، آنگاه:

\begin{itemize}
\tightlist
\item
  \lr{P(A \ensuremath{\cap} B) = 0}
\item
  \lr{P(A)P(B) \textgreater{} 0}
\item
  بنابراین \lr{P(A \ensuremath{\cap} B) \ensuremath{\neq} P(A)P(B)}
\end{itemize}
...
اگر \lr{B\textsubscript{1}}، \lr{B\textsubscript{2}}، \lr{…}، \lr{B\textsubscript{n}} فضای نمونه را افراز کنند (ناسازگار متقابل و جامع باشند):
...
\bottomrule\noalign{}
\endlastfoot
\lr{P(B\textsubscript{i})} & احتمال پیشین & آنچه پیش از مشاهده داده‌ها باور داشتیم \\
\lr{P(A\textbar{}B\textsubscript{i})} & درست‌نمایی & با توجه به هر فرضیه، مشاهده با چه احتمالی رخ می‌دهد \\
\lr{P(B\textsubscript{i}\textbar{}A)} & احتمال پسین & باور به‌روزشده پس از مشاهده داده‌ها \\
...
\tightlist
\item
  \lr{B\textsubscript{1} =} خطای منبع تغذیه، \lr{P(B\textsubscript{1}) = 0.3}
\item
  \lr{B\textsubscript{2} =} خطای پردازنده، \lr{P(B\textsubscript{2}) = 0.5}
\item
  \lr{B\textsubscript{3} =} خطای حافظه، \lr{P(B\textsubscript{3}) = 0.2}
\end{itemize}
...
\tightlist
\item
  \lr{P(A\textbar{}B\textsubscript{1}) = 0.8} (خطاهای منبع تغذیه معمولاً باعث گرم‌شدن بیش از حد می‌شوند)
\item
  \lr{P(A\textbar{}B\textsubscript{2}) = 0.3} (خطاهای پردازنده گاهی باعث گرم‌شدن بیش از حد می‌شوند)
\item
  \lr{P(A\textbar{}B\textsubscript{3}) = 0.1} (خطاهای حافظه به‌ندرت باعث گرم‌شدن بیش از حد می‌شوند)
\end{itemize}
...
\lr{P(A) = P(A\textbar{}B\textsubscript{1})P(B\textsubscript{1}) + P(A\textbar{}B\textsubscript{2})P(B\textsubscript{2}) + P(A\textbar{}B\textsubscript{3})P(B\textsubscript{3})}\PSbr{}\lr{= (0.8)(0.3) + (0.3)(0.5) + (0.1)(0.2)}\PSbr{}\lr{= 0.24 + 0.15 + 0.02 = 0.41}
...
\tightlist
\item
  \lr{P(H\textsubscript{1}) = 0.01, P(D\textbar{}H\textsubscript{1}) = 0.95, P(D\textbar{}H\textsubscript{0}) = 0.02}
\end{itemize}
...
اگر \lr{B\textsubscript{1}}، \lr{B\textsubscript{2}}، \lr{…}، \lr{B\textsubscript{n}} یک \textbf{افراز} از \lr{S} تشکیل دهند (ناسازگار متقابل و جامع):

\[P(A) = \sum_{i=1}^{n} P(A|B_i) \cdot P(B_i)\]

\PSsubsection{تفسیر}

برای یافتن احتمال کل رویداد \lr{A}، آن را به تمام \lr{«}مسیرهای\lr{»} ممکن منتهی به \lr{A} از طریق افراز تجزیه کنید.
...
\lr{P}(خطای بسته) \lr{= P(error\textbar{}path 1)P(path 1) + P(error\textbar{}path 2)P(path 2) + P(error\textbar{}path 3)P(path 3)}\PSbr{}\lr{= (0.01)(0.50) + (0.05)(0.30) + (0.10)(0.20)}\PSbr{}\lr{= 0.005 + 0.015 + 0.020 = 0.040}
...
\tightlist
\item
  تأمین‌کننده \lr{A: 60\%} خازن‌ها را تأمین می‌کند، نرخ عیب 2\%
\item
  تأمین‌کننده \lr{B: 40\%} خازن‌ها را تأمین می‌کند، نرخ عیب 5\%
\end{itemize}

\lr{P}(معیوب) \lr{= P}(معیوب\lr{\textbar{}}\lr{A)P(A) + P}(معیوب\lr{\textbar{}}\lr{B)P(B})\PSbr{}\lr{= (0.02)(0.60) + (0.05)(0.40)}\PSbr{}\lr{= 0.012 + 0.020 = 0.032}

اگر یک خازن معیوب پیدا شود، کدام تأمین‌کننده احتمالاً آن را تولید کرده است؟

\lr{P(A\textbar{}}معیوب) \lr{= (0.02)(0.60) / 0.032 = 0.375}\PSbr{}\lr{P(B\textbar{}}معیوب) \lr{= (0.05)(0.40) / 0.032 = 0.625}

با وجود تأمین تعداد کمتری از واحدها، احتمال اینکه تأمین‌کننده \lr{B} مسیول یک عیب مشخص باشد بیشتر است.

\end{PSbox}

\PSsection{1.11}{مثال‌های \lr{MATLAB}}
...
\textbf{احتمال شرطی} & \lr{P(A\textbar{}B) =} احتمال به‌روزشده با توجه به \lr{B} & اینکه دانستن یک رویداد چگونه احتمال رویداد دیگر را تغییر می‌دهد \\
\textbf{استقلال} & \lr{P(A\ensuremath{\cap}B) = P(A)P(B)} & دانستن یک رویداد هیچ اطلاعاتی درباره دیگری نمی‌دهد \\
\end{longtable}
}
...
\def\labelenumi{(\alph{enumi})}
\item
  \lr{P}(خرابی سیستم) \lr{= P}(هر دو خراب شوند) \lr{= (0.02)(0.03) = 0.0006}
\item
  \lr{P}(دست‌کم یکی خراب شود) \lr{= 1 - P}(هیچ‌کدام خراب نشوند) \lr{= 1 - (0.98)(0.97) = 1 - 0.9506 = 0.0494}
\item
  سیستم در حال کار \lr{= NOT}(هر دو خراب شوند) \lr{=} دست‌کم یکی کار می‌کند.\PSbr{}\lr{P(P1 failed \textbar{} system works) = P(P1 failed \ensuremath{\cap} P2 works) / P(system works)}\PSbr{}\lr{= (0.02)(0.97) / (1 - 0.0006) = 0.0194 / 0.9994 \ensuremath{\approx} 0.0194}
\end{enumerate}
...
یک سیستم آشکارسازی آتش دارای مقادیر زیر است: \lr{P(fire) = 0.001}، \lr{P(alarm \textbar{} fire) = 0.98}، \lr{P(alarm \textbar{} no fire) = 0.01}.
...
\item
  \lr{P(alarm)} چقدر است؟
\item
  اگر هشدار به صدا درآید، \lr{P(fire \textbar{} alarm)} چقدر است؟
\item
  نتیجه را تفسیر کنید.
\end{enumerate}
...
\item
  \lr{P(fire\textbar{}alarm) = P(alarm\textbar{}fire)P(fire) / P(alarm)}\PSbr{}\lr{= (0.98)(0.001) / 0.01097 = 0.0894}
\item
...
\tightlist
\item
  \lr{P}(عدم وجود خطا در بسته) چقدر است؟
\item
  \lr{P}(دقیقاً یک خطا) چقدر است؟
\item
  \lr{P}(دست‌کم یک خطا) چقدر است؟
\item
  کد \lr{MATLAB} را برای راستی‌آزمایی از طریق شبیه‌سازی بنویسید.
\end{enumerate}
...
\def\labelenumi{(\alph{enumi})}
\item
  \lr{P}(عدم وجود خطا) \lr{= (0.95)\textsuperscript{8} = 0.6634}
\item
  \lr{P}(دقیقاً یک خطا) \lr{= C(8,1)(0.05)\textsuperscript{1}(0.95)\textsuperscript{7} = 8 \ensuremath{\times} 0.05 \ensuremath{\times} 0.6983 = 0.2793}
\item
  \lr{P}(دست‌کم یک خطا) \lr{= 1 - P}(عدم وجود خطا) \lr{= 1 - 0.6634 = 0.3366}
\item
  راستی‌آزمایی \lr{MATLAB}:
\end{enumerate}
...
\tightlist
\item
  حالت 1 (کانال خوب): 60\% از زمان، \lr{P(outage) = 0.01}
\item
  حالت 2 (کانال متوسط): 30\% از زمان، \lr{P(outage) = 0.10}
\item
  حالت 3 (کانال ضعیف): 10\% از زمان، \lr{P(outage) = 0.50}
\end{itemize}
...
\item
  \lr{P(Mode 3 \textbar{} outage) = (0.50)(0.10) / 0.086 = 0.581}
\end{enumerate}
...
\tightlist
\item
  اگر حسگرها مستقل باشند، \lr{P}(هر دو هشدار کاذب می‌دهند) چقدر است؟
\item
  اگر حسگرها یک منبع نویز مشترک داشته باشند و \lr{P}(هر دو هشدار کاذب) \lr{= 0.01} باشد، آیا مستقل هستند؟
\item
  در حالت \lr{(b)}، \lr{P(A false alarm \textbar{} B false alarm)} چقدر است؟
\end{enumerate}
...
\def\labelenumi{(\alph{enumi})}
\item
  در صورت استقلال: \lr{P}(هر دو) \lr{= (0.05)(0.03) = 0.0015}
\item
  اگر \lr{P}(هر دو) \lr{= 0.01 \ensuremath{\neq} 0.0015}، آن‌ها مستقل نیستند. منبع نویز مشترک وابستگی ایجاد می‌کند.
\item
...
\item
  \textbf{قضیه بیز احتمال‌های شرطی را معکوس می‌کند} \lr{—} از \lr{«}احتمال شواهد با توجه به علت\lr{»} به \lr{«}احتمال علت با توجه به شواهد\lr{»}
\item
  \textbf{نرخ‌های پایه اهمیت دارند} \lr{—} حتی آشکارسازهای خوب نیز هنگامی که رویدادها نادر هستند، هشدار کاذب تولید می‌کنند
\item
  \textbf{شبیه‌سازی تحلیل را اعتبارسنجی می‌کند} \lr{—} مونت‌کارلوی \lr{MATLAB} محاسبات نظری را تأیید می‌کند
\end{enumerate}

\emph{ماژول بعدی: {ماژول \lr{2 —} یک متغیر تصادفی و توابع آن}}\PSbr{}
