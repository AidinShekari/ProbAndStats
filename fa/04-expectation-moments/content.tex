...
جمع (هر مقدار \lr{\ensuremath{\times}} احتمال آن).
...
\textbf{گسسته:}\PSbr{}\[E[g(X)] = \sum_{x} g(x) \cdot p_X(x)\]

\textbf{پیوسته:}\PSbr{}\[E[g(X)] = \int_{-\infty}^{\infty} g(x) \cdot f_X(x) \, dx\]
...
برای \lr{X \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}): E[X\textsuperscript{2}] = \ensuremath{\mu}\textsuperscript{2} + \ensuremath{\sigma}\textsuperscript{2}} (توان کل \lr{=} توان \lr{DC +} توان \lr{AC}).
...
\[\PSmtext{Var}(X) = E[(X - \mu_X)^2] = E[X^2] - (E[X])^2\]
...
اندازه‌گیری‌ها & دقت اندازه‌گیری (کوچک‌تر \lr{=} دقیق‌تر) \\
سیگنال & مؤلفه توان \lr{AC} \\
عمر & عمر چقدر قابل پیش‌بینی است؟ \\
\end{longtable}
}

\PSsubsection{نکته کلیدی: واریانس \lr{=} توان در مهندسی}
...
\tightlist
\item
  \lr{E[X\textsuperscript{2}] = \ensuremath{\mu}\textsuperscript{2} + \ensuremath{\sigma}\textsuperscript{2} =} \textbf{توان کل \lr{=} توان \lr{DC +} توان \lr{AC}}
\item
...
\[E[X + Y] = E[X] + E[Y] \quad \PSmtext{(ALWAYS, even if dependent!)}\]
...
اگر \lr{X} و \lr{Y} مستقل باشند:\PSbr{}\[E[XY] = E[X] \cdot E[Y]\]
...
\[\PSmtext{Var}(aX + b) = a^2 \PSmtext{Var}(X)\]
...
\[\PSmtext{Var}(X_1 + X_2 + \cdots + X_n) = \PSmtext{Var}(X_1) + \PSmtext{Var}(X_2) + \cdots + \PSmtext{Var}(X_n)\]
...
\[\PSmtext{Var}(\bar{X}) = \PSmtext{Var}\left(\frac{1}{n}\sum X_i\right) = \frac{\sigma^2}{n}\]
...
(کسر کردن 3 باعث می‌شود کشیدگی گاوسی صفر شود: \lr{«}کشیدگی مازاد\lr{»}.)
...
\PSsubsection{چرا \lr{«}مولد گشتاور\lr{»}؟}
...
\tightlist
\item
  \textbf{مؤلفه \lr{DC} (بایاس):} \lr{\ensuremath{\mu}\textsubscript{N} —} برای نویز ایده‌آل باید صفر باشد
\item
  \textbf{توان نویز \lr{AC}:} \lr{\ensuremath{\sigma}\textsubscript{N}\textsuperscript{2} —} تعیین‌کننده \lr{SNR}
\item
...
\textbf{\lr{SNR} (نسبت سیگنال به نویز):}\PSbr{}\[\PSmtext{SNR} = \frac{E[S^2]}{\sigma_N^2} = \frac{\PSmtext{Signal Power}}{\PSmtext{Noise Power}}\]
...
\item
  \lr{E[X] = \ensuremath{\theta} + E[E] —} بایاس در اندازه‌گیری
\item
  \lr{Var(X) = Var(E) —} دقت اندازه‌گیری
\end{itemize}

\textbf{صحّت:} \lr{\textbar{}E[E]\textbar{}} (میانگین چقدر به مقدار واقعی نزدیک است)\PSbr{}\textbf{دقت (تکرارپذیری):} \lr{\ensuremath{\sigma}\textsubscript{E}} (اندازه‌گیری‌ها چقدر تکرارپذیر هستند)
...
\item
  توان سیگنال \lr{= 2.5\textsuperscript{2} = 6.25}، توان نویز \lr{= 0.04}\PSbr{}\lr{SNR = 6.25/0.04 = 156.25 \ensuremath{\to} SNR\textsubscript{dB} = 10\ensuremath{\cdot}log\textsubscript{10}(156.25) = 21.94 dB}
\item
...
قطعات عمر \lr{T \textasciitilde{} Exp(\ensuremath{\lambda} = 0.002} در هر ساعت) دارند.
...
\item
  \lr{P(T \textgreater{} 1000} ساعت) را بیابید.
\item
...
\emph{ماژول بعدی: {ماژول \lr{5 —} توابع مهم متغیر تصادفی}}\PSbr{}
