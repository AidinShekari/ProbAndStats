...
\[X: S \rightarrow \mathbb{R}\]\PSbr{}\[\omega \mapsto X(\omega)\]
...
\item
  هر مقدار یک احتمال مشخص دارد (یک \lr{«}جرم\lr{»} از احتمال)
\item
...
تصور کنید یک ولتاژ را اندازه‌گیری می‌کنید. احتمال این‌که \lr{V} دقیقاً \lr{3.14159265…} ولت باشد (با دقت نامتناهی) صفر است. تنها می‌توانیم درباره بازه‌ها بپرسیم: \lr{P(3.14 \textless{} V \textless{} 3.15)}.
...
\item
  \textbf{احتمال \lr{=} مساحت زیر منحنی} میان دو نقطه
\end{enumerate}
...
و برعکس:\PSbr{}\[f_X(x) = \frac{dF_X(x)}{dx}\]
...
\textbf{دامنه مقادیر} & [0, 1] & \lr{[0, \ensuremath{\infty}) —} می‌تواند از 1 فراتر رود! & [0, 1] \\
...
احتمال را به‌عنوان \lr{«}ماده‌ای\lr{»} تصور کنید که روی مقادیر ممکن توزیع شده است:
...
\textbf{پیوسته \lr{(PDF)}:} مانند پهن‌کردن ماده در طول یک قفسه پیوسته. چگالی به شما می‌گوید ماده در هر نقطه چقدر ضخیم است. جرم را با محاسبه مساحت (ضخامت \lr{\ensuremath{\times}} عرض) به دست می‌آورید.
...
\tightlist
\item
  \lr{f\textsubscript{X}(0.3) = 2 —} این یک احتمال نیست! به این معناست که چگالی برابر \lr{2 V\textsuperscript{-1}} است.
\item
  \lr{P(0.2 \ensuremath{\le} X \ensuremath{\le} 0.4) = \ensuremath{\int}\textsubscript{0.2}\textsuperscript{0.4} 2 dx = 2 \ensuremath{\times} 0.2 = 0.4 —} این یک احتمال است.
\item
  \lr{P(X = 0.3) = 0 —} برای متغیرهای تصادفی پیوسته، یک نقطه منفرد احتمال صفر دارد.
\end{itemize}
...
\begin{longtable}[c]{@{}
  ...
  >{\centering\arraybackslash}p{(0.965\linewidth - 2\tabcolsep) * \real{0.5000}}@{}}
\toprule\noalign{}
\rowcolor{PSnavy}\PSth{\PSno{} گزاره نادرست} & \PSth{\lr{\PSyes{}} گزاره درست} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\lr{«}احتمال در \lr{x=2} برابر \lr{f(2)} است\lr{»} & \lr{«}چگالی احتمال در \lr{x=2} برابر \lr{f(2)} است\lr{»} \\
\lr{«f(x)} باید \lr{\ensuremath{\le} 1} باشد\lr{»} & \lr{«f(x)} می‌تواند هر مقدار نامنفی باشد\lr{»} \\
\lr{«}برای \lr{X} پیوسته \lr{P(X=3) = f(3)»} & \lr{«}برای \lr{X} پیوسته \lr{P(X=3) = 0»} \\
\lr{«PMF} و \lr{PDF} یک چیز هستند\lr{»} & \lr{«PMF} جرم می‌دهد؛ \lr{PDF} چگالی می‌دهد\lr{»} \\
\end{longtable}
}
...
\item
  نویز \lr{N} معلوم است \lr{\ensuremath{\to} \textbar{}N\textbar{}} (نویز یکسوسازی‌شده) لازم است
\end{itemize}
...
یا به‌طور معادل:\PSbr{}\[f_Y(y) = \frac{f_X(x)}{|g'(x)|}\bigg|_{x=g^{-1}(y)}\]
...
که در آن \lr{x\textsubscript{1}, x\textsubscript{2}, …} همگی جواب‌های \lr{g(x) = y} هستند.
...
اگر \lr{X} گاوسی با میانگین صفر باشد: \lr{f\textsubscript{X}(x) = f\textsubscript{X}(-x)}، پس:\PSbr{}\[f_Y(y) = \frac{f_X(\sqrt{y})}{\sqrt{y}}, \quad y > 0\]
...
\lr{g\textsuperscript{-1}(y) = (y-b)/a}، و \lr{\textbar{}d/dy[(y-b)/a]\textbar{} = 1/\textbar{}a\textbar{}}
...
\item
  پیوسته، \lr{X \ensuremath{\in} (-\ensuremath{\infty}, \ensuremath{\infty})} و در عمل حدود \lr{\textasciitilde{} (-120, 0) dBm}
\item
...
\emph{ماژول بعدی: {ماژول \lr{3 —} توزیع‌های مهم احتمال}}\PSbr{}
