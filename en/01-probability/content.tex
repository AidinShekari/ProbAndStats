...
The \textbf{sample space} S (or \ensuremath{\Omega}) is the set of all possible outcomes of a random experiment.
...
\tightlist
\item
  Measured voltage: S = \ensuremath{\mathbb{R}} (or a practical range like [0, 5] V)
\item
  Time until component failure: S = [0, \ensuremath{\infty})
\item
  Phase of received signal: S = [0, 2\ensuremath{\pi})
\end{itemize}
...
\item
  \textbf{Impossible event}: The empty set \ensuremath{\emptyset} (never occurs)
\end{itemize}
...
\bottomrule\noalign{}
\endlastfoot
Union & A \ensuremath{\cup} B & A or B (or both) occurs & Either failure mode A or B triggers alarm \\
Intersection & A \ensuremath{\cap} B & Both A and B occur & Both sensors detect the signal \\
Complement & A\textsuperscript{c} or Ā & A does not occur & Component does NOT fail \\
Difference & A - B = A \ensuremath{\cap} B\textsuperscript{c} & A occurs but not B & Sensor A triggers but sensor B doesn’t \\
Mutually exclusive & A \ensuremath{\cap} B = \ensuremath{\emptyset} & A and B cannot both occur & Component can’t be both working and failed \\
\end{longtable}
}
...
\tightlist
\item
  A \ensuremath{\cup} B \ensuremath{\cup} C = ``board has at least one defect''
\item
  A \ensuremath{\cap} B = ``board has both a solder bridge AND a missing component''
\item
  (A \ensuremath{\cup} B \ensuremath{\cup} C)\textsuperscript{c} = ``board is defect-free''
\end{itemize}
...
\textbf{Axiom 1 (Non-negativity):}\PSbr{}\[P(A) \geq 0 \quad \text{for any event } A\]

\textbf{Axiom 2 (Normalization):}\PSbr{}\[P(S) = 1\]

\textbf{Axiom 3 (Countable Additivity):}\PSbr{}For mutually exclusive events A\textsubscript{1}, A\textsubscript{2}, A\textsubscript{3}, …:\PSbr{}\[P(A_1 \cup A_2 \cup A_3 \cup \cdots) = P(A_1) + P(A_2) + P(A_3) + \cdots\]

\PSsubsection{Properties Derived from Axioms}

\textbf{Property 1: Complement Rule}\PSbr{}\[P(A^c) = 1 - P(A)\]

\emph{Proof:} S = A \ensuremath{\cup} A\textsuperscript{c} and A \ensuremath{\cap} A\textsuperscript{c} = \ensuremath{\emptyset}, so P(S) = P(A) + P(A\textsuperscript{c}) = 1.

\emph{Engineering meaning:} P(component works) = 1 - P(component fails)

\textbf{Property 2: Impossible Event}\PSbr{}\[P(\emptyset) = 0\]

\textbf{Property 3: Monotonicity}\PSbr{}If A \ensuremath{\subseteq} B, then P(A) \ensuremath{\le} P(B)

\textbf{Property 4: Bound}\PSbr{}\[0 \leq P(A) \leq 1\]

\textbf{Property 5: Inclusion-Exclusion (Two Events)}\PSbr{}\[P(A \cup B) = P(A) + P(B) - P(A \cap B)\]
...
For any two events:\PSbr{}\[P(A \cup B) = P(A) + P(B) - P(A \cap B)\]

For mutually exclusive events (A \ensuremath{\cap} B = \ensuremath{\emptyset}):\PSbr{}\[P(A \cup B) = P(A) + P(B)\]
...
\item
  P(at least one fails) = P(A \ensuremath{\cup} B \ensuremath{\cup} C)
\item
  Using inclusion-exclusion:
\end{itemize}

P(A \ensuremath{\cup} B \ensuremath{\cup} C) = 3(0.05) - 3(0.05)\textsuperscript{2} + (0.05)\textsuperscript{3} = 0.15 - 0.0075 + 0.000125 = 0.142625

Alternatively: P(at least one fails) = 1 - P(none fail) = 1 - (0.95)\textsuperscript{3} = 0.142625 \PSyes{}
...
Conditional probability & P(A\textbar{}B) & Likelihood of A \textbf{given} that B has occurred \\
Joint probability & P(A \ensuremath{\cap} B) & Likelihood that \textbf{both} A and B occur \\
\end{longtable}
}

These are related: P(A \ensuremath{\cap} B) = P(A\textbar{}B) \ensuremath{\cdot} P(B) = P(B\textbar{}A) \ensuremath{\cdot} P(A)
...
\tightlist
\item
  H\textsubscript{1} = aircraft is present (target)
\item
  H\textsubscript{0} = aircraft is absent (no target)
\item
...
\tightlist
\item
  P(D \textbar{} H\textsubscript{1}) = \textbf{probability of detection} (sensitivity)
\item
  P(D \textbar{} H\textsubscript{0}) = \textbf{probability of false alarm}
\item
  P(D\textsuperscript{c} \textbar{} H\textsubscript{1}) = \textbf{probability of miss} = 1 - P(D \textbar{} H\textsubscript{1})
\end{itemize}

Suppose: P(H\textsubscript{1}) = 0.01, P(D\textbar{}H\textsubscript{1}) = 0.95, P(D\textbar{}H\textsubscript{0}) = 0.02

What is P(target actually present \textbar{} detector says present)?
...
\tightlist
\item
  P(receive 1 \textbar{} sent 0) = p = 0.01
\item
  P(receive 0 \textbar{} sent 1) = p = 0.01
\item
  P(receive 0 \textbar{} sent 0) = 1 - p = 0.99
\item
  P(receive 1 \textbar{} sent 1) = 1 - p = 0.99
\end{itemize}
...
\tightlist
\item
  P(sent 0 \ensuremath{\cap} receive 1) = P(receive 1 \textbar{} sent 0) \ensuremath{\cdot} P(sent 0) = 0.01 \ensuremath{\times} 0.5 = 0.005
\item
  P(error) = P(sent 0 \ensuremath{\cap} receive 1) + P(sent 1 \ensuremath{\cap} receive 0) = 0.005 + 0.005 = 0.01
\end{itemize}
...
\tightlist
\item
  P(A\textbar{}B) \ensuremath{\ge} 0
\item
  P(S\textbar{}B) = 1
\item
  P(A\textsubscript{1} \ensuremath{\cup} A\textsubscript{2}\textbar{}B) = P(A\textsubscript{1}\textbar{}B) + P(A\textsubscript{2}\textbar{}B) if A\textsubscript{1} \ensuremath{\cap} A\textsubscript{2} = \ensuremath{\emptyset}
\end{enumerate}
...
For three events:\PSbr{}\[P(A \cap B \cap C) = P(A) \cdot P(B|A) \cdot P(C|A \cap B)\]
...
Equivalently: P(A\textbar{}B) = P(A) — knowing B occurred gives no information about A.
...
\tightlist
\item
  P(A \ensuremath{\cap} B) = P(A)P(B)
\item
  P(A \ensuremath{\cap} C) = P(A)P(C)
\item
  P(B \ensuremath{\cap} C) = P(B)P(C)
\end{itemize}
...
\item
  P(A \ensuremath{\cap} B \ensuremath{\cap} C) = P(A)P(B)P(C)
\end{itemize}
...
P(all three fail) = P(A)P(B)P(C) = (0.1)\textsuperscript{3} = 0.001
...
\tightlist
\item
  Compute P(A), P(B), and P(A \ensuremath{\cap} B) empirically
\item
  Check if P(A \ensuremath{\cap} B) \ensuremath{\approx} P(A) \ensuremath{\cdot} P(B)
\end{enumerate}
...
This is \textbf{FALSE}. If A \ensuremath{\cap} B = \ensuremath{\emptyset} and P(A) \textgreater{} 0, P(B) \textgreater{} 0, then:

\begin{itemize}
\tightlist
\item
  P(A \ensuremath{\cap} B) = 0
\item
  P(A)P(B) \textgreater{} 0
\item
  Therefore P(A \ensuremath{\cap} B) \ensuremath{\neq} P(A)P(B)
\end{itemize}
...
If B\textsubscript{1}, B\textsubscript{2}, …, B\textsubscript{n} partition the sample space (mutually exclusive and exhaustive):
...
\bottomrule\noalign{}
\endlastfoot
P(B\textsubscript{i}) & Prior probability & What we believed before observing data \\
P(A\textbar{}B\textsubscript{i}) & Likelihood & How likely is the observation given each hypothesis \\
P(B\textsubscript{i}\textbar{}A) & Posterior probability & Updated belief after observing data \\
...
\tightlist
\item
  B\textsubscript{1} = power supply fault, P(B\textsubscript{1}) = 0.3
\item
  B\textsubscript{2} = processor fault, P(B\textsubscript{2}) = 0.5
\item
  B\textsubscript{3} = memory fault, P(B\textsubscript{3}) = 0.2
\end{itemize}
...
\tightlist
\item
  P(A\textbar{}B\textsubscript{1}) = 0.8 (power supply faults usually cause overheating)
\item
  P(A\textbar{}B\textsubscript{2}) = 0.3 (processor faults sometimes cause overheating)
\item
  P(A\textbar{}B\textsubscript{3}) = 0.1 (memory faults rarely cause overheating)
\end{itemize}
...
P(A) = P(A\textbar{}B\textsubscript{1})P(B\textsubscript{1}) + P(A\textbar{}B\textsubscript{2})P(B\textsubscript{2}) + P(A\textbar{}B\textsubscript{3})P(B\textsubscript{3})\PSbr{}= (0.8)(0.3) + (0.3)(0.5) + (0.1)(0.2)\PSbr{}= 0.24 + 0.15 + 0.02 = 0.41

P(B\textsubscript{1}\textbar{}A) = (0.8)(0.3) / 0.41 = 0.585 (most likely!)\PSbr{}P(B\textsubscript{2}\textbar{}A) = (0.3)(0.5) / 0.41 = 0.366\PSbr{}P(B\textsubscript{3}\textbar{}A) = (0.1)(0.2) / 0.41 = 0.049
...
\tightlist
\item
  P(H\textsubscript{1}) = 0.01, P(D\textbar{}H\textsubscript{1}) = 0.95, P(D\textbar{}H\textsubscript{0}) = 0.02
\end{itemize}

P(D) = P(D\textbar{}H\textsubscript{1})P(H\textsubscript{1}) + P(D\textbar{}H\textsubscript{0})P(H\textsubscript{0}) = (0.95)(0.01) + (0.02)(0.99) = 0.0095 + 0.0198 = 0.0293

P(H\textsubscript{1}\textbar{}D) = (0.95)(0.01) / 0.0293 = 0.324
...
If B\textsubscript{1}, B\textsubscript{2}, …, B\textsubscript{n} form a \textbf{partition} of S (mutually exclusive, exhaustive):
...
P(packet error) = P(error\textbar{}path 1)P(path 1) + P(error\textbar{}path 2)P(path 2) + P(error\textbar{}path 3)P(path 3)\PSbr{}= (0.01)(0.50) + (0.05)(0.30) + (0.10)(0.20)\PSbr{}= 0.005 + 0.015 + 0.020 = 0.040
...
P(defective) = P(defective\textbar{}A)P(A) + P(defective\textbar{}B)P(B)\PSbr{}= (0.02)(0.60) + (0.05)(0.40)\PSbr{}= 0.012 + 0.020 = 0.032

If a defective capacitor is found, which supplier likely produced it?

P(A\textbar{}defective) = (0.02)(0.60) / 0.032 = 0.375\PSbr{}P(B\textbar{}defective) = (0.05)(0.40) / 0.032 = 0.625
...
\textbf{Conditional Probability} & P(A\textbar{}B) = updated probability given B & How knowledge of one event changes another’s likelihood \\
\textbf{Independence} & P(A\ensuremath{\cap}B) = P(A)P(B) & Knowing one event tells nothing about the other \\
\end{longtable}
}
...
\def\labelenumi{(\alph{enumi})}
\item
  P(system fails) = P(both fail) = (0.02)(0.03) = 0.0006
\item
  P(at least one fails) = 1 - P(none fail) = 1 - (0.98)(0.97) = 1 - 0.9506 = 0.0494
\item
  System working = NOT(both fail) = at least one works.\PSbr{}P(P1 failed \textbar{} system works) = P(P1 failed \ensuremath{\cap} P2 works) / P(system works)\PSbr{}= (0.02)(0.97) / (1 - 0.0006) = 0.0194 / 0.9994 \ensuremath{\approx} 0.0194
\end{enumerate}
...
A fire detection system has: P(fire) = 0.001, P(alarm \textbar{} fire) = 0.98, P(alarm \textbar{} no fire) = 0.01.
...
\item
  What is P(alarm)?
\item
  If the alarm sounds, what is P(fire \textbar{} alarm)?
\item
...
\def\labelenumi{(\alph{enumi})}
\item
  P(alarm) = P(alarm\textbar{}fire)P(fire) + P(alarm\textbar{}no fire)P(no fire)\PSbr{}= (0.98)(0.001) + (0.01)(0.999) = 0.000980 + 0.009990 = 0.010970
\item
  P(fire\textbar{}alarm) = P(alarm\textbar{}fire)P(fire) / P(alarm)\PSbr{}= (0.98)(0.001) / 0.01097 = 0.0894
\item
  Only \textasciitilde{}9\% of alarms correspond to real fires! This is the base rate fallacy. Even with an excellent detector, rare events produce many false alarms.
\end{enumerate}
...
\tightlist
\item
  What is P(no errors in the packet)?
\item
  What is P(exactly one error)?
\item
  What is P(at least one error)?
\item
...
\def\labelenumi{(\alph{enumi})}
\item
  P(no errors) = (0.95)\textsuperscript{8} = 0.6634
\item
  P(exactly one error) = C(8,1)(0.05)\textsuperscript{1}(0.95)\textsuperscript{7} = 8 \ensuremath{\times} 0.05 \ensuremath{\times} 0.6983 = 0.2793
\item
  P(at least one error) = 1 - P(no errors) = 1 - 0.6634 = 0.3366
\item
  MATLAB verification:
\end{enumerate}
...
\def\labelenumi{(\alph{enumi})}
\item
  P(outage) = (0.01)(0.60) + (0.10)(0.30) + (0.50)(0.10) = 0.006 + 0.030 + 0.050 = 0.086
\item
  P(Mode 3 \textbar{} outage) = (0.50)(0.10) / 0.086 = 0.581
\end{enumerate}
...
\tightlist
\item
  If sensors are independent, what is P(both give false alarm)?
\item
  If sensors share a noise source and P(both false alarm) = 0.01, are they independent?
\item
  What is P(A false alarm \textbar{} B false alarm) in case (b)?
\end{enumerate}
...
\def\labelenumi{(\alph{enumi})}
\item
  If independent: P(both) = (0.05)(0.03) = 0.0015
\item
  If P(both) = 0.01 \ensuremath{\neq} 0.0015, they are NOT independent. The shared noise source creates dependence.
\item
  P(A\textbar{}B) = P(A\ensuremath{\cap}B) / P(B) = 0.01 / 0.03 = 0.333
\end{enumerate}
