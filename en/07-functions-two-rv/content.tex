...
For independent X, Y:\PSbr{}\[f_Z(z) = \int_{-\infty}^{\infty} f_X(x) \cdot f_Y(x-z) \, dx\]
...
For independent X, Y:\PSbr{}\[f_Z(z) = \int_{-\infty}^{\infty} \frac{1}{|x|} f_X(x) \cdot f_Y(z/x) \, dx\]
...
For independent X, Y:\PSbr{}\[f_Z(z) = \int_{-\infty}^{\infty} |y| \cdot f_X(zy) \cdot f_Y(y) \, dy\]
...
\setcounter{enumi}{2}
\item
  Apply:\PSbr{}\[f_{U,V}(u,v) = f_{X,Y}(h_1(u,v), h_2(u,v)) \cdot |J|\]
\item
...
\PSsolution{} Convolution: f\textsubscript{Z}(z) = \ensuremath{\int}\textsubscript{0}\textsuperscript{z} e\textsuperscript{(-x)\ensuremath{\cdot}e}(-(z-x)) dx = \ensuremath{\int}\textsubscript{0}\textsuperscript{z} e\textsuperscript{-z} dx = ze\textsuperscript{-z}, z\ensuremath{\ge}0.\PSbr{}This is Gamma(2,1) = Erlang(2,1). E[Z] = 2, Var(Z) = 2.
