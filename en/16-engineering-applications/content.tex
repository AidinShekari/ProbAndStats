...
\item
  \ensuremath{\hat{p}} = k/n (MLE of BER)
\item
  95\% CI: \ensuremath{\hat{p}} \ensuremath{\pm} 1.96\ensuremath{\surd}(\ensuremath{\hat{p}}(1-\ensuremath{\hat{p}})/n)
\end{itemize}
...
\tightlist
\item
  \textbf{Distribution:} N(0, N\textsubscript{0}/2) per dimension
\item
  \textbf{Why Gaussian?} CLT — thermal noise is sum of many electron contributions
\item
  \textbf{Characterization:} Fully described by power spectral density N\textsubscript{0} or variance \ensuremath{\sigma}\textsuperscript{2} = N\textsubscript{0}B (B = bandwidth)
\end{itemize}

\PSsubsection{16.1.3 Rayleigh Fading Channel}

\textbf{Model:} Channel coefficient h = h\textsubscript{I} + j\ensuremath{\cdot}h\textsubscript{Q} where h\textsubscript{I}, h\textsubscript{Q} \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2}) independent.

\begin{itemize}
\tightlist
\item
  Envelope \textbar{}h\textbar{} \textasciitilde{} Rayleigh(\ensuremath{\sigma})
\item
  Power \textbar{}h\textbar{}\textsuperscript{2} \textasciitilde{} Exponential(1/(2\ensuremath{\sigma}\textsuperscript{2}))
\item
  \textbf{Outage probability:} P(\textbar{}h\textbar{}\textsuperscript{2} \textless{} \ensuremath{\gamma}\textsubscript{th}) = 1 - exp(-\ensuremath{\gamma}\textsubscript{th}/(2\ensuremath{\sigma}\textsuperscript{2}))
\end{itemize}
...
\tightlist
\item
  h = h\textsubscript{LOS} + h\textsubscript{scatter}
\item
  \textbar{}h\textbar{} \textasciitilde{} Rician(\ensuremath{\nu}, \ensuremath{\sigma}) where K = \ensuremath{\nu}\textsuperscript{2}/(2\ensuremath{\sigma}\textsuperscript{2}) is the K-factor
\end{itemize}

K-factor: K = 0 (Rayleigh) \ensuremath{\to} K = \ensuremath{\infty} (no fading, AWGN)

\PSsubsection{16.1.5 SNR Statistics}

\textbf{Instantaneous SNR:} \ensuremath{\gamma} = \textbar{}h\textbar{}\textsuperscript{2} \ensuremath{\times} (E\textsubscript{s}/N\textsubscript{0})

For Rayleigh: \ensuremath{\gamma} \textasciitilde{} Exponential(\ensuremath{\bar{\gamma}}) where \ensuremath{\bar{\gamma}} = average SNR

\textbf{Average BER over fading:}\PSbr{}\[\bar{P}_e = \int_0^\infty P_e(\gamma) \cdot f_\gamma(\gamma) \, d\gamma\]

For BPSK over Rayleigh: \ensuremath{\bar{P}}\textsubscript{e} = (1/2)(1 - \ensuremath{\surd}(\ensuremath{\bar{\gamma}}/(1+\ensuremath{\bar{\gamma}})))
...
\item
  Estimate noise statistics: \ensuremath{\hat{\mu}}\textsubscript{n}, \ensuremath{\hat{\sigma}}\textsuperscript{2}\textsubscript{n}
\item
  Test for Gaussianity (histogram, QQ-plot)
\item
  Compute SNR: SNR = P\textsubscript{signal} / \ensuremath{\hat{\sigma}}\textsuperscript{2}\textsubscript{n}
\end{enumerate}
...
\textbf{Method:} Correlate received signal with known template. Under H\textsubscript{0} (noise only), the correlator output has known distribution \ensuremath{\to} set threshold.
...
\textbf{Problem:} Resistors have nominal value R\textsubscript{0} with manufacturing tolerance.

\textbf{Model:} R \textasciitilde{} N(R\textsubscript{0}, \ensuremath{\sigma}\textsubscript{R}\textsuperscript{2}) where \ensuremath{\sigma}\textsubscript{R} depends on tolerance class.

For \ensuremath{\pm}5\% tolerance (99.7\% within bounds): 3\ensuremath{\sigma} = 0.05R\textsubscript{0} \ensuremath{\to} \ensuremath{\sigma} = 0.0167R\textsubscript{0}

\PSsubsection{16.3.2 Monte Carlo Circuit Analysis}

\textbf{Problem:} A voltage divider uses R\textsubscript{1} and R\textsubscript{2}. What is the distribution of output voltage?

V\textsubscript{out} = V\textsubscript{in} \ensuremath{\times} R\textsubscript{2}/(R\textsubscript{1} + R\textsubscript{2})

With R\textsubscript{1}, R\textsubscript{2} random: V\textsubscript{out} is a random variable!
...
\textbf{Example:} Power P = V\textsuperscript{2}/R. \ensuremath{\sigma}\textsubscript{P}\textsuperscript{2} \ensuremath{\approx} (2V/R)\textsuperscript{2}\ensuremath{\sigma}\textsubscript{V}\textsuperscript{2} + (V\textsuperscript{2}/R\textsuperscript{2})\textsuperscript{2}\ensuremath{\sigma}\textsubscript{R}\textsuperscript{2}

\PSsection{16.4}{Power Systems}

\PSsubsection{16.4.1 Load Uncertainty}

\textbf{Problem:} Peak load is uncertain. Model: L \textasciitilde{} N(\ensuremath{\mu}\textsubscript{L}, \ensuremath{\sigma}\textsubscript{L}\textsuperscript{2})

Design question: What capacity C ensures P(load \textgreater{} capacity) \textless{} 0.01?

C = \ensuremath{\mu}\textsubscript{L} + 2.326\ensuremath{\sigma}\textsubscript{L} (for 1\% exceedance probability)

\PSsubsection{16.4.2 System Reliability}

\textbf{Series system:} All components must work.

R\textsubscript{sys} = R\textsubscript{1} \ensuremath{\times} R\textsubscript{2} \ensuremath{\times} … \ensuremath{\times} R\textsubscript{n} (for independent components)

\textbf{Parallel system:} At least one must work.

R\textsubscript{sys} = 1 - (1-R\textsubscript{1})(1-R\textsubscript{2})…(1-R\textsubscript{n})
...
Measurement: y(k) = x(k) + n(k) where n \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2}\textsubscript{n})
...
\tightlist
\item
  Averaging (low-pass filter): reduces noise by \ensuremath{\surd}n
\item
...
System model: G(s) = K/(\ensuremath{\tau}s + 1) where K and \ensuremath{\tau} are uncertain.

Monte Carlo approach: sample K and \ensuremath{\tau} from their distributions, simulate system response many times, compute statistics of output.
