...
A \textbf{random variable} X is a function that maps each outcome \ensuremath{\omega} in the sample space S to a real number:

\[X: S \rightarrow \mathbb{R}\]\PSbr{}\[\omega \mapsto X(\omega)\]

\PSsubsection{Interpretation}

The random variable assigns a numerical value to each possible outcome of the experiment. The randomness comes from which outcome \ensuremath{\omega} occurs — once \ensuremath{\omega} is determined, X(\ensuremath{\omega}) is a definite number.

\PSsubsection{Engineering Example}

\textbf{Experiment:} Transmit 5 bits over a noisy channel.\PSbr{}\textbf{Sample space:} S = all possible sequences of correct/error for 5 bits (2\textsuperscript{5} = 32 outcomes)\PSbr{}\textbf{Random variable:} X = number of bit errors

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[c]{@{}cc@{}}
\toprule\noalign{}
\rowcolor{PSnavy}\PSth{Outcome \ensuremath{\omega}} & \PSth{X(\ensuremath{\omega})} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
Now instead of working with 32 outcomes, we work with a single number X \ensuremath{\in} \{0,1,2,3,4,5\}.
...
\tightlist
\item
  Values can be listed: \{x\textsubscript{1}, x\textsubscript{2}, x\textsubscript{3}, …\}
\item
...
Quantized voltage level & \{0, 1, …, 2\textsuperscript{N}-1\} & N-bit ADC output \\
...
\tightlist
\item
  X can take any value in a range (e.g., [0, \ensuremath{\infty}) or (-\ensuremath{\infty}, \ensuremath{\infty}))
\item
  Probability of any single exact value is ZERO: P(X = x) = 0
\item
  Probabilities are defined for intervals: P(a \ensuremath{\le} X \ensuremath{\le} b)
\end{itemize}
...
\bottomrule\noalign{}
\endlastfoot
Thermal noise voltage & (-\ensuremath{\infty}, \ensuremath{\infty}) & Circuit noise \\
Time until failure & [0, \ensuremath{\infty}) & Reliability \\
Phase of received signal & [0, 2\ensuremath{\pi}) & Communications \\
Resistor value & (R\textsubscript{0} - \ensuremath{\delta}, R\textsubscript{0} + \ensuremath{\delta}) & Manufacturing \\
Channel gain & [0, \ensuremath{\infty}) & Wireless fading \\
\end{longtable}
}

\PSsubsection{Why P(X = x) = 0 for Continuous RVs}

Imagine measuring a voltage. The probability that V = exactly 3.14159265… volts (infinite precision) is zero. We can only ask about intervals: P(3.14 \textless{} V \textless{} 3.15).
...
\tightlist
\item
  \textbf{Non-negativity:} p\textsubscript{X}(x) \ensuremath{\ge} 0 for all x
\item
  \textbf{Normalization:} \ensuremath{\Sigma} p\textsubscript{X}(x\textsubscript{i}) = 1 (sum over all possible values)
\item
  \textbf{Probability computation:} P(X \ensuremath{\in} A) = \ensuremath{\Sigma}\textsubscript{x\ensuremath{\in}A} p\textsubscript{X}(x)
\end{enumerate}
...
p\textsubscript{X}(k) = C(8,k) \ensuremath{\cdot} (0.1)\textsuperscript{k} \ensuremath{\cdot} (0.9)\textsuperscript{8-k}, for k = 0, 1, …, 8

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[c]{@{}cc@{}}
\toprule\noalign{}
\rowcolor{PSnavy}\PSth{k} & \PSth{p\textsubscript{X}(k)} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
For a continuous random variable X, the \textbf{probability density function} f\textsubscript{X}(x) is defined such that:
...
\tightlist
\item
  \textbf{Non-negativity:} f\textsubscript{X}(x) \ensuremath{\ge} 0 for all x
\item
  \textbf{Normalization:} \ensuremath{\int}\textsubscript{-\ensuremath{\infty}}\textsuperscript{\ensuremath{\infty}} f\textsubscript{X}(x) dx = 1
\item
  \textbf{Probability = Area under curve} between two points
\end{enumerate}

\PSsubsection{Critical Point: f\textsubscript{X}(x) is NOT a Probability!}

\begin{PSquote}

\PSwarn{} f\textsubscript{X}(x) can be GREATER than 1!
...
where \ensuremath{\sigma} is the RMS noise voltage.

P(noise exceeds 2\ensuremath{\sigma}) = P(\textbar{}X\textbar{} \textgreater{} 2\ensuremath{\sigma}) = 1 - P(-2\ensuremath{\sigma} \ensuremath{\le} X \ensuremath{\le} 2\ensuremath{\sigma}) \ensuremath{\approx} 1 - 0.9545 = 0.0455
...
\tightlist
\item
  \textbf{Non-decreasing:} if a \textless{} b, then F\textsubscript{X}(a) \ensuremath{\le} F\textsubscript{X}(b)
\item
  \textbf{Limits:} F\textsubscript{X}(-\ensuremath{\infty}) = 0, F\textsubscript{X}(+\ensuremath{\infty}) = 1
\item
  \textbf{Right-continuous:} F\textsubscript{X}(x) is continuous from the right
\item
  \textbf{Probability of interval:} P(a \textless{} X \ensuremath{\le} b) = F\textsubscript{X}(b) - F\textsubscript{X}(a)
\end{enumerate}
...
Jump at x = k has height p\textsubscript{X}(k).
...
And conversely:\PSbr{}\[f_X(x) = \frac{dF_X(x)}{dx}\]
...
\PSsubsection{Relationship: PDF \ensuremath{\leftrightarrow} CDF}
...
\bottomrule\noalign{}
\endlastfoot
PDF \ensuremath{\to} CDF & F\textsubscript{X}(x) = \ensuremath{\int}\textsubscript{-\ensuremath{\infty}}\textsuperscript{x} f\textsubscript{X}(t)dt & Integrate density to get cumulative probability \\
CDF \ensuremath{\to} PDF & f\textsubscript{X}(x) = dF\textsubscript{X}/dx & Differentiate CDF to get density \\
\end{longtable}
}
...
\begin{longtable}[c]{@{}
  ...
  >{\centering\arraybackslash}p{(0.965\linewidth - 6\tabcolsep) * \real{0.2558}}@{}}
\toprule\noalign{}
\rowcolor{PSnavy}\PSth{Property} & \PSth{PMF p\textsubscript{X}(x)} & \PSth{PDF f\textsubscript{X}(x)} & \PSth{CDF F\textsubscript{X}(x)} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{Applies to} & Discrete RVs only & Continuous RVs only & Both \\
\textbf{Gives directly} & P(X = x) & Probability density at x & P(X \ensuremath{\le} x) \\
\textbf{Range of values} & [0, 1] & [0, \ensuremath{\infty}) — can exceed 1! & [0, 1] \\
\textbf{Sum/Integral} & \ensuremath{\Sigma} p\textsubscript{X}(x) = 1 & \ensuremath{\int} f\textsubscript{X}(x)dx = 1 & Starts at 0, ends at 1 \\
\textbf{Get probability} & P(X=x) = p\textsubscript{X}(x) & P(a\ensuremath{\le}X\ensuremath{\le}b) = \ensuremath{\int} f\textsubscript{X}(x)dx & P(a\textless{}X\ensuremath{\le}b) = F(b)-F(a) \\
\end{longtable}
}
...
\textbf{Example:} A noise voltage X has PDF f\textsubscript{X}(x) = 2 for x \ensuremath{\in} [0, 0.5].

\begin{itemize}
\tightlist
\item
  f\textsubscript{X}(0.3) = 2 — this is NOT a probability! It means the density is 2 V\textsuperscript{-1}.
\item
  P(0.2 \ensuremath{\le} X \ensuremath{\le} 0.4) = \ensuremath{\int}\textsubscript{0.2}\textsuperscript{0.4} 2 dx = 2 \ensuremath{\times} 0.2 = 0.4 — THIS is a probability.
\item
...
\tightlist
\item
  Know voltage V \ensuremath{\to} need power P = V\textsuperscript{2}/R
\item
  Know linear signal X \ensuremath{\to} need dB value Y = 10\ensuremath{\cdot}log\textsubscript{10}(X)
\item
  Know noise N \ensuremath{\to} need \textbar{}N\textbar{} (rectified noise)
\end{itemize}

\begin{PSbox}[unbreakable]{prob}{Problem Statement}

Given: X with known PDF f\textsubscript{X}(x) or PMF p\textsubscript{X}(x)\PSbr{}Find: The distribution of Y = g(X)

\end{PSbox}

\PSsubsection{Case 1: Discrete RV Transformation}

If X is discrete with PMF p\textsubscript{X}(x), and Y = g(X):

\[p_Y(y) = \sum_{x: g(x)=y} p_X(x)\]

Sum the probabilities of all x values that map to the same y.

\textbf{Example:} X \ensuremath{\in} \{-2, -1, 0, 1, 2\} with equal probability 0.2 each.\PSbr{}Y = X\textsuperscript{2}

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[c]{@{}ccc@{}}
\toprule\noalign{}
\rowcolor{PSnavy}\PSth{Y = X\textsuperscript{2}} & \PSth{X values mapping to Y} & \PSth{p\textsubscript{Y}(y)} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
Or equivalently:\PSbr{}\[f_Y(y) = \frac{f_X(x)}{|g'(x)|}\bigg|_{x=g^{-1}(y)}\]
...
\tightlist
\item
  Start with CDF: F\textsubscript{Y}(y) = P(Y \ensuremath{\le} y) = P(g(X) \ensuremath{\le} y)
\item
  Invert: = P(X \ensuremath{\le} g\textsuperscript{-1}(y)) if g is increasing
\item
  Differentiate: f\textsubscript{Y}(y) = f\textsubscript{X}(g\textsuperscript{-1}(y)) \ensuremath{\cdot} d/dy[g\textsuperscript{-1}(y)]
\end{enumerate}
...
where x\textsubscript{1}, x\textsubscript{2}, … are all solutions of g(x) = y.

\begin{PSbox}[unbreakable]{ex}{Engineering Example: Power from Voltage}

A noise voltage X has PDF f\textsubscript{X}(x) (e.g., Gaussian with zero mean).\PSbr{}Power dissipated in a 1\ensuremath{\Omega} resistor: P = X\textsuperscript{2}

Since Y = X\textsuperscript{2} is NOT monotonic (both +x and -x give the same y):

For y \textgreater{} 0: x = +\ensuremath{\surd}y and x = -\ensuremath{\surd}y are both solutions.\PSbr{}g(x) = x\textsuperscript{2}, so g’(x) = 2x

\[f_Y(y) = \frac{f_X(\sqrt{y})}{2\sqrt{y}} + \frac{f_X(-\sqrt{y})}{2\sqrt{y}}, \quad y > 0\]

If X is zero-mean Gaussian: f\textsubscript{X}(x) = f\textsubscript{X}(-x), so:\PSbr{}\[f_Y(y) = \frac{f_X(\sqrt{y})}{\sqrt{y}}, \quad y > 0\]

\end{PSbox}

\begin{PSbox}[unbreakable]{ex}{Engineering Example: Linear Amplification}

Signal Y = aX + b (amplifier with gain a and offset b).\PSbr{}X has PDF f\textsubscript{X}(x).

g\textsuperscript{-1}(y) = (y-b)/a, and \textbar{}d/dy[(y-b)/a]\textbar{} = 1/\textbar{}a\textbar{}
...
\tightlist
\item
  Discrete, X \ensuremath{\in} \{0, 1, 2, …\}
\item
  Continuous, X \ensuremath{\in} (-\ensuremath{\infty}, \ensuremath{\infty}) practically \textasciitilde{} (-120, 0) dBm
\item
  Discrete, X \ensuremath{\in} \{0, 1, 2, …, 12000\}
\item
  Continuous, T \ensuremath{\in} [0, \ensuremath{\infty})
\item
  Discrete, X \ensuremath{\in} \{0, 1, 2, 3, 4, 5, 6, 7\}
\end{enumerate}
...
A random variable X has PMF: p\textsubscript{X}(0) = 0.1, p\textsubscript{X}(1) = 0.3, p\textsubscript{X}(2) = c, p\textsubscript{X}(3) = 0.2.
...
\item
  Find c.
\item
  Find P(X \ensuremath{\ge} 2).
\item
  Find F\textsubscript{X}(1.5).
\item
  Find P(0.5 \textless{} X \ensuremath{\le} 2.5).
\end{enumerate}
...
\def\labelenumi{(\alph{enumi})}
\item
  Sum = 1: 0.1 + 0.3 + c + 0.2 = 1 \ensuremath{\to} c = 0.4
\item
  P(X \ensuremath{\ge} 2) = p\textsubscript{X}(2) + p\textsubscript{X}(3) = 0.4 + 0.2 = 0.6
\item
  F\textsubscript{X}(1.5) = P(X \ensuremath{\le} 1.5) = p\textsubscript{X}(0) + p\textsubscript{X}(1) = 0.1 + 0.3 = 0.4
\item
  P(0.5 \textless{} X \ensuremath{\le} 2.5) = p\textsubscript{X}(1) + p\textsubscript{X}(2) = 0.3 + 0.4 = 0.7
\end{enumerate}
...
A random variable has PDF:\PSbr{}f\textsubscript{X}(x) = cx for 0 \ensuremath{\le} x \ensuremath{\le} 4, and 0 otherwise.
...
\item
  Find c.
\item
  Find the CDF F\textsubscript{X}(x).
\item
  Find P(1 \ensuremath{\le} X \ensuremath{\le} 3).
\item
  Find the value x\textsubscript{0} such that P(X \ensuremath{\le} x\textsubscript{0}) = 0.5.
\end{enumerate}
...
\def\labelenumi{(\alph{enumi})}
\item
  \ensuremath{\int}\textsubscript{0}\textsuperscript{4} cx dx = c\ensuremath{\cdot}[x\textsuperscript{2}/2]\textsubscript{0}\textsuperscript{4} = c\ensuremath{\cdot}8 = 1 \ensuremath{\to} c = 1/8
\item
  F\textsubscript{X}(x) = \ensuremath{\int}\textsubscript{0}\textsuperscript{x} (t/8)dt = x\textsuperscript{2}/16, for 0 \ensuremath{\le} x \ensuremath{\le} 4\PSbr{}F\textsubscript{X}(x) = 0 for x \textless{} 0; F\textsubscript{X}(x) = 1 for x \textgreater{} 4
\item
  P(1 \ensuremath{\le} X \ensuremath{\le} 3) = F\textsubscript{X}(3) - F\textsubscript{X}(1) = 9/16 - 1/16 = 8/16 = 0.5
\item
  x\textsubscript{0}\textsuperscript{2}/16 = 0.5 \ensuremath{\to} x\textsubscript{0}\textsuperscript{2} = 8 \ensuremath{\to} x\textsubscript{0} = 2\ensuremath{\surd}2 \ensuremath{\approx} 2.83
\end{enumerate}
...
\def\labelenumi{(\alph{enumi})}
\item
  X \textasciitilde{} Uniform[20, 30], so f\textsubscript{X}(x) = 1/10 for 20 \ensuremath{\le} x \ensuremath{\le} 30.\PSbr{}Y = 0.1X - 2, so a = 0.1, b = -2.\PSbr{}f\textsubscript{Y}(y) = (1/\textbar{}a\textbar{}) \ensuremath{\cdot} f\textsubscript{X}((y-b)/a) = (1/0.1) \ensuremath{\cdot} (1/10) = 1 for the valid range.
\item
  When X = 20: Y = 0.1(20) - 2 = 0; When X = 30: Y = 0.1(30) - 2 = 1\PSbr{}Range of Y: [0, 1]. So Y \textasciitilde{} Uniform[0, 1].
\item
  P(Y \textgreater{} 0.5) = 1 - F\textsubscript{Y}(0.5) = 1 - 0.5 = 0.5
\end{enumerate}
...
A voltage X \textasciitilde{} Uniform[-1, 1] V. The instantaneous power is Y = X\textsuperscript{2}.
...
\tightlist
\item
  For 0 \ensuremath{\le} y \ensuremath{\le} 1:\PSbr{}F\textsubscript{Y}(y) = P(X\textsuperscript{2} \ensuremath{\le} y) = P(-\ensuremath{\surd}y \ensuremath{\le} X \ensuremath{\le} \ensuremath{\surd}y) = F\textsubscript{X}(\ensuremath{\surd}y) - F\textsubscript{X}(-\ensuremath{\surd}y)
\end{enumerate}

Since X \textasciitilde{} Uniform[-1,1]: F\textsubscript{X}(x) = (x+1)/2

F\textsubscript{Y}(y) = (\ensuremath{\surd}y + 1)/2 - (-\ensuremath{\surd}y + 1)/2 = \ensuremath{\surd}y

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\setcounter{enumi}{1}
\item
  f\textsubscript{Y}(y) = dF\textsubscript{Y}/dy = 1/(2\ensuremath{\surd}y) for 0 \textless{} y \ensuremath{\le} 1
\item
  MATLAB:
\end{enumerate}
...
\item
  \textbf{CDF} works universally — gives cumulative probability P(X \ensuremath{\le} x)
\item
