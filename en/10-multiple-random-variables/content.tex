...
\item
  \textbf{MIMO system:} M\ensuremath{\times}N channel coefficients
\item
...
For X\textsubscript{1}, X\textsubscript{2}, …, X\textsubscript{n}:\PSbr{}\[f_{X_1,...,X_n}(x_1,...,x_n) \geq 0, \quad \int\cdots\int f_{X_1,...,X_n} \, dx_1 \cdots dx_n = 1\]

\PSsubsection{Marginal Distributions}

Integrate (or sum) out unwanted variables:\PSbr{}\[f_{X_1}(x_1) = \int\cdots\int f_{X_1,...,X_n}(x_1,...,x_n) \, dx_2 \cdots dx_n\]
...
X\textsubscript{1}, …, X\textsubscript{n} are mutually independent if:\PSbr{}\[f_{X_1,...,X_n}(x_1,...,x_n) = \prod_{i=1}^n f_{X_i}(x_i)\]
...
\bottomrule\noalign{}
\endlastfoot
Sensor array output & X\textsubscript{1},…,X\textsubscript{n} = sensor readings & Array processing \\
MIMO channel & h\textsubscript{1},…,h\textsubscript{n} = channel gains & Wireless communications \\
Noise vector & N\textsubscript{1},…,N\textsubscript{n} = noise samples & Signal processing \\
...
\tightlist
\item
  E[AX + b] = AE[X] + b = A\ensuremath{\mu} + b
\item
...
Element (i,j): \ensuremath{\Sigma}\textsubscript{ij} = Cov(X\textsubscript{i}, X\textsubscript{j})
...
\tightlist
\item
  \textbf{Symmetric:} \ensuremath{\Sigma} = \ensuremath{\Sigma}\textsuperscript{T}
\item
  \textbf{Positive semi-definite:} v\textsuperscript{T}\ensuremath{\Sigma}v \ensuremath{\ge} 0 for all v
\item
  \textbf{Diagonal = variances:} \ensuremath{\Sigma}\textsubscript{ii} = Var(X\textsubscript{i})
\item
  \textbf{Off-diagonal = covariances:} \ensuremath{\Sigma}\textsubscript{ij} = Cov(X\textsubscript{i}, X\textsubscript{j})
\item
  \textbf{Independent components \ensuremath{\to} diagonal \ensuremath{\Sigma}}
\end{enumerate}
...
X \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\Sigma}) has PDF:
...
where \textbar{}\ensuremath{\Sigma}\textbar{} = determinant of \ensuremath{\Sigma}.
...
\item
  \textbf{Conditionals are Gaussian:} X\textsubscript{1}\textbar{}X\textsubscript{2}=x\textsubscript{2} is Gaussian
\item
  \textbf{Linear transformations:} Y = AX + b \textasciitilde{} N(A\ensuremath{\mu}+b, A\ensuremath{\Sigma}A\textsuperscript{T})
\item
  \textbf{Uncorrelated = Independent} (unique to Gaussian!)
\item
  \textbf{Fully specified} by \ensuremath{\mu} and \ensuremath{\Sigma} (only 2 parameters needed per variable plus correlations)
\end{enumerate}
...
For n=2 with \ensuremath{\mu} = [0,0]\textsuperscript{T}, \ensuremath{\sigma}\textsubscript{1} = \ensuremath{\sigma}\textsubscript{2} = 1:

\[f(x,y) = \frac{1}{2\pi\sqrt{1-\rho^2}} \exp\left(-\frac{x^2 - 2\rho xy + y^2}{2(1-\rho^2)}\right)\]

Contours are \textbf{ellipses} (tilted when \ensuremath{\rho} \ensuremath{\neq} 0).

\PSsection{10.7}{Linear Transformations of Random Vectors}

\PSsubsection{Transformation Y = AX + b}

If X is a random vector with mean \ensuremath{\mu}\textsubscript{X} and covariance \ensuremath{\Sigma}\textsubscript{X}:

\[E[\mathbf{Y}] = A\boldsymbol{\mu}_X + \mathbf{b}\]\PSbr{}\[\boldsymbol{\Sigma}_Y = A\boldsymbol{\Sigma}_X A^T\]

\PSsubsection{Engineering Applications}

\textbf{Beamforming:} Output y = w\textsuperscript{T}X (weight vector applied to sensor array)

\begin{itemize}
\tightlist
\item
  E[y] = w\textsuperscript{T}\ensuremath{\mu}
\item
  Var(y) = w\textsuperscript{T}\ensuremath{\Sigma}w
\end{itemize}

\textbf{Whitening:} Find matrix W such that W\ensuremath{\Sigma}W\textsuperscript{T} = I (decorrelates data)

\begin{itemize}
\tightlist
\item
  W = \ensuremath{\Sigma}\textsuperscript{-1/2}
\end{itemize}

\textbf{Principal Component Analysis:} Rotate to diagonalize \ensuremath{\Sigma}
...
Random vector X = [X\textsubscript{1}, X\textsubscript{2}, X\textsubscript{3}]\textsuperscript{T} with \ensuremath{\mu} = [1, 0, -1]\textsuperscript{T}, and covariance matrix \ensuremath{\Sigma} = [4 1 0; 1 2 -1; 0 -1 3].

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\tightlist
\item
  Find Var(X\textsubscript{1} + X\textsubscript{2} + X\textsubscript{3}). (b) Find Cov(X\textsubscript{1}, X\textsubscript{2}+X\textsubscript{3}). (c) Are X\textsubscript{1} and X\textsubscript{3} uncorrelated?
\end{enumerate}
...
\tightlist
\item
  Var(X\textsubscript{1}+X\textsubscript{2}+X\textsubscript{3}) = 1\textsuperscript{T}\ensuremath{\Sigma}1 = 4+2+3 + 2(1) + 2(0) + 2(-1) = 9 + 0 = 9\PSbr{}Wait: = \ensuremath{\Sigma}\textsubscript{ij} all entries = 4+1+0+1+2+(-1)+0+(-1)+3 = 9
\item
  Cov(X\textsubscript{1}, X\textsubscript{2}+X\textsubscript{3}) = Cov(X\textsubscript{1},X\textsubscript{2}) + Cov(X\textsubscript{1},X\textsubscript{3}) = 1 + 0 = 1
\item
  Cov(X\textsubscript{1},X\textsubscript{3}) = 0, so yes, X\textsubscript{1} and X\textsubscript{3} are uncorrelated.
\end{enumerate}
...
X \textasciitilde{} N(0, \ensuremath{\Sigma}) with \ensuremath{\Sigma} = [1 \ensuremath{\rho}; \ensuremath{\rho} 1]. Find the conditional distribution of X\textsubscript{1}\textbar{}X\textsubscript{2}=x\textsubscript{2}.

\PSsolution{} For bivariate Gaussian:\PSbr{}X\textsubscript{1}\textbar{}X\textsubscript{2}=x\textsubscript{2} \textasciitilde{} N(\ensuremath{\mu}\textsubscript{1} + \ensuremath{\rho}(\ensuremath{\sigma}\textsubscript{1}/\ensuremath{\sigma}\textsubscript{2})(x\textsubscript{2}-\ensuremath{\mu}\textsubscript{2}), \ensuremath{\sigma}\textsubscript{1}\textsuperscript{2}(1-\ensuremath{\rho}\textsuperscript{2}))\PSbr{}= N(\ensuremath{\rho}x\textsubscript{2}, 1-\ensuremath{\rho}\textsuperscript{2})

The conditional mean is a linear function of x\textsubscript{2}, and conditional variance is reduced by factor (1-\ensuremath{\rho}\textsuperscript{2}).

\end{PSbox}

\begin{PSbox}[unbreakable]{prob}{Problem 3}

Write MATLAB code to generate a 3D Gaussian vector with given \ensuremath{\mu} and \ensuremath{\Sigma}, compute the sample covariance, and verify.
