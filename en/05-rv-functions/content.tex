...
If g is strictly increasing with inverse g\textsuperscript{-1}:\PSbr{}\[f_Y(y) = f_X(g^{-1}(y)) \cdot \frac{d}{dy}[g^{-1}(y)]\]

If g is strictly decreasing:\PSbr{}\[f_Y(y) = f_X(g^{-1}(y)) \cdot \left|\frac{d}{dy}[g^{-1}(y)]\right|\]

Combined (works for both):\PSbr{}\[f_Y(y) = \frac{f_X(x)}{|g'(x)|}\bigg|_{x=g^{-1}(y)}\]
