...
\item
  State the PMF/PDF, CDF, mean, and variance for all major distributions
\item
...
X \textasciitilde{} Bernoulli(p), where p = probability of success.
...
Compact form: p\textsubscript{X}(x) = p\textsuperscript{x}(1-p)\textsuperscript{1-x} for x \ensuremath{\in} \{0, 1\}
...
\PSyes{} Single binary trial with fixed probability\PSbr{}\PSyes{} Building block for more complex distributions\PSbr{}\PSyes{} Modeling ON/OFF states, pass/fail outcomes
...
X \textasciitilde{} Geometric(p)

Convention: X = number of the trial on which first success occurs (X \ensuremath{\in} \{1, 2, 3, …\}).
...
\PSno{} Success probability changes over time (aging, learning)\PSbr{}\PSno{} Trials are not independent\PSbr{}\PSno{} Looking for the r-th success (use Negative Binomial)
...
Models the \textbf{number of trials until the r-th success} (generalization of Geometric).
...
X \textasciitilde{} NegBin(r, p) = number of trials until r-th success.
...
\PSyes{} Repeated independent trials until r-th success\PSbr{}\PSyes{} Constant probability per trial\PSbr{}\PSyes{} Generalizing geometric to multiple required successes
...
\tightlist
\item
  Number of photons hitting a detector in 1 \ensuremath{\mu}s
\item
  Number of packets arriving at a router in 1 second
\item
  Number of defects per cm\textsuperscript{2} on a silicon wafer
\item
...
X \textasciitilde{} Poisson(\ensuremath{\lambda}), where \ensuremath{\lambda} = average number of events per interval.
...
When n is large and p is small, with \ensuremath{\lambda} = np:

\PSdisplay{\text{Binomial}(n, p) \approx \text{Poisson}(\lambda = np)}

Rule of thumb: n \ensuremath{\ge} 20 and p \ensuremath{\le} 0.05.
...
\tightlist
\item
  Phase of a received signal (unknown, equally likely in [0, 2\ensuremath{\pi}))
\item
  Round-off error in an ADC (uniform in [-\ensuremath{\Delta}/2, \ensuremath{\Delta}/2] where \ensuremath{\Delta} = quantization step)
\item
...
X \textasciitilde{} Uniform(a, b) or X \textasciitilde{} U(a, b)
...
X \textasciitilde{} Exponential(\ensuremath{\lambda}) where \ensuremath{\lambda} = rate parameter (events per unit time).

Alternative parameterization: X \textasciitilde{} Exp(\ensuremath{\beta}) where \ensuremath{\beta} = 1/\ensuremath{\lambda} = mean.
...
Or with mean \ensuremath{\beta}: f\textsubscript{X}(x) = (1/\ensuremath{\beta})e\textsuperscript{-x/\ensuremath{\beta}}
...
If events arrive as a Poisson process with rate \ensuremath{\lambda}:

\begin{itemize}
\tightlist
\item
  Number of events in time T \textasciitilde{} Poisson(\ensuremath{\lambda}T)
\item
  Time between events \textasciitilde{} Exponential(\ensuremath{\lambda})
\end{itemize}
...
X \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}) where \ensuremath{\mu} = mean, \ensuremath{\sigma}\textsuperscript{2} = variance.
...
Z \textasciitilde{} N(0, 1): standardized form.

Any Normal can be standardized: Z = (X - \ensuremath{\mu})/\ensuremath{\sigma}

\PSsubsection{Key Probability Rules (\ensuremath{\sigma}-rules)}
...
\bottomrule\noalign{}
\endlastfoot
\ensuremath{\mu} \ensuremath{\pm} 1\ensuremath{\sigma} & 68.27\% \\
\ensuremath{\mu} \ensuremath{\pm} 2\ensuremath{\sigma} & 95.45\% \\
\ensuremath{\mu} \ensuremath{\pm} 3\ensuremath{\sigma} & 99.73\% \\
\ensuremath{\mu} \ensuremath{\pm} 4\ensuremath{\sigma} & 99.9937\% \\
\end{longtable}
}
...
\tightlist
\item
  \textbf{Symmetric} about \ensuremath{\mu}
\item
  \textbf{Linear transformation:} If X \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}), then aX + b \textasciitilde{} N(a\ensuremath{\mu} + b, a\textsuperscript{2}\ensuremath{\sigma}\textsuperscript{2})
\item
  \textbf{Sum of Gaussians:} If X \textasciitilde{} N(\ensuremath{\mu}\textsubscript{1}, \ensuremath{\sigma}\textsubscript{1}\textsuperscript{2}) and Y \textasciitilde{} N(\ensuremath{\mu}\textsubscript{2}, \ensuremath{\sigma}\textsubscript{2}\textsuperscript{2}) are independent, then X + Y \textasciitilde{} N(\ensuremath{\mu}\textsubscript{1}+\ensuremath{\mu}\textsubscript{2}, \ensuremath{\sigma}\textsubscript{1}\textsuperscript{2}+\ensuremath{\sigma}\textsubscript{2}\textsuperscript{2})
\item
...
Models the \textbf{total waiting time until the k-th event} in a Poisson process, or any sum of k independent exponential random variables.
...
X \textasciitilde{} Gamma(\ensuremath{\alpha}, \ensuremath{\beta}) where \ensuremath{\alpha} = shape parameter, \ensuremath{\beta} = scale parameter (or rate \ensuremath{\lambda} = 1/\ensuremath{\beta}).
...
where \ensuremath{\Gamma}(\ensuremath{\alpha}) = (\ensuremath{\alpha}-1)! for integer \ensuremath{\alpha}, and \ensuremath{\Gamma}(\ensuremath{\alpha}) = \ensuremath{\int}\textsubscript{0}\textsuperscript{\ensuremath{\infty}} t\textsuperscript{\ensuremath{\alpha}-1}e\textsuperscript{-t}dt in general.

\PSsubsection{CDF}

No closed form for general \ensuremath{\alpha}. Available via incomplete gamma function.
...
\bottomrule\noalign{}
\endlastfoot
\ensuremath{\alpha} = 1 & Exponential(\ensuremath{\beta}) \\
\ensuremath{\alpha} = n/2, \ensuremath{\beta} = 2 & Chi-square(n) \\
\ensuremath{\alpha} = integer k & Erlang(k, \ensuremath{\beta}) \\
\end{longtable}
}

\PSsubsection{When to Use}

\PSyes{} Sum of independent exponential lifetimes\PSbr{}\PSyes{} Waiting time for k-th Poisson event\PSbr{}\PSyes{} Skewed, non-negative data\PSbr{}\PSyes{} Flexible shape (adjustable skewness)
...
Models the \textbf{magnitude (envelope)} of a 2D Gaussian random vector. When X and Y are independent N(0, \ensuremath{\sigma}\textsuperscript{2}), then R = \ensuremath{\surd}(X\textsuperscript{2} + Y\textsuperscript{2}) follows a Rayleigh distribution.
...
R \textasciitilde{} Rayleigh(\ensuremath{\sigma}), where \ensuremath{\sigma} is the parameter (NOT the standard deviation of R).
...
\tightlist
\item
  R\textsuperscript{2} \textasciitilde{} Exponential(2\ensuremath{\sigma}\textsuperscript{2}) — the power is exponentially distributed
\item
  If X, Y \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2}) independent, then \ensuremath{\surd}(X\textsuperscript{2}+Y\textsuperscript{2}) \textasciitilde{} Rayleigh(\ensuremath{\sigma})
\end{itemize}
...
If Z\textsubscript{1}, Z\textsubscript{2}, …, Z\textsubscript{n} are independent N(0,1), then:\PSdisplay{\chi^2 = Z_1^2 + Z_2^2 + \cdots + Z_n^2 \sim \chi^2(n)}
...
\item
  For large n: \ensuremath{\chi}\textsuperscript{2}(n) \ensuremath{\approx} N(n, 2n) by CLT
\end{itemize}
...
Geometric & Discrete & p & 1/p & (1-p)/p\textsuperscript{2} & geornd(p)+1 \\
Neg. Binomial & Discrete & r, p & r/p & r(1-p)/p\textsuperscript{2} & nbinrnd(r,p)+r \\
Poisson & Discrete & \ensuremath{\lambda} & \ensuremath{\lambda} & \ensuremath{\lambda} & poissrnd(\ensuremath{\lambda}) \\
Uniform & Continuous & a, b & (a+b)/2 & (b-a)\textsuperscript{2}/12 & unifrnd(a,b) \\
Exponential & Continuous & \ensuremath{\lambda} (or \ensuremath{\beta}=1/\ensuremath{\lambda}) & 1/\ensuremath{\lambda} & 1/\ensuremath{\lambda}\textsuperscript{2} & exprnd(\ensuremath{\beta}) \\
Gaussian & Continuous & \ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2} & \ensuremath{\mu} & \ensuremath{\sigma}\textsuperscript{2} & normrnd(\ensuremath{\mu},\ensuremath{\sigma}) \\
Gamma & Continuous & \ensuremath{\alpha}, \ensuremath{\beta} & \ensuremath{\alpha}\ensuremath{\beta} & \ensuremath{\alpha}\ensuremath{\beta}\textsuperscript{2} & gamrnd(\ensuremath{\alpha},\ensuremath{\beta}) \\
Rayleigh & Continuous & \ensuremath{\sigma} & \ensuremath{\sigma}\ensuremath{\surd}(\ensuremath{\pi}/2) & (4-\ensuremath{\pi})\ensuremath{\sigma}\textsuperscript{2}/2 & raylrnd(\ensuremath{\sigma}) \\
...
\PSsolution{} (a) Poisson(\ensuremath{\lambda}=2), (b) Gaussian(0, \ensuremath{\sigma}\textsuperscript{2}), (c) Exponential(\ensuremath{\lambda}), (d) Binomial(100, 0.03), (e) Rayleigh(\ensuremath{\sigma})

\end{PSbox}

\begin{PSbox}[unbreakable]{prob}{Problem 2: Exponential vs Poisson}

Packets arrive at a router as a Poisson process with rate \ensuremath{\lambda} = 5 packets/second.

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\tightlist
\item
  What is P(more than 8 packets in 1 second)?
\item
  What is P(no packet arrives in the next 0.5 seconds)?
\item
...
\tightlist
\item
  X \textasciitilde{} Poisson(5): P(X \textgreater{} 8) = 1 - poisscdf(8, 5) = 0.0681
\item
  T \textasciitilde{} Exp(5): P(T \textgreater{} 0.5) = e\textsuperscript{-5\ensuremath{\times}0.5} = e\textsuperscript{-2.5} = 0.0821
\item
  Exponential with rate \ensuremath{\lambda} = 5
\item
  Mean = 1/\ensuremath{\lambda} = 0.2 seconds
\end{enumerate}
...
A resistor has nominal value 1000\ensuremath{\Omega} with manufacturing variation modeled as N(1000, 25) (\ensuremath{\sigma} = 5\ensuremath{\Omega}).

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\tightlist
\item
  What fraction of resistors are within \ensuremath{\pm}10\ensuremath{\Omega} of nominal (spec: 990-1010\ensuremath{\Omega})?
\item
  What fraction fail the \ensuremath{\pm}1\% tolerance test?
\item
...
\tightlist
\item
  P(990 \ensuremath{\le} X \ensuremath{\le} 1010) = P(-2 \ensuremath{\le} Z \ensuremath{\le} 2) = 0.9545 \ensuremath{\to} 95.45\%
\item
  \ensuremath{\pm}1\% = \ensuremath{\pm}10\ensuremath{\Omega} \ensuremath{\to} same as (a), fail rate = 1 - 0.9545 = 4.55\%
\item
  \ensuremath{\pm}3\ensuremath{\sigma} = \ensuremath{\pm}15\ensuremath{\Omega} \ensuremath{\to} range [985, 1015]\ensuremath{\Omega}
\end{enumerate}
