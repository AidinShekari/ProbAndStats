...
2. PMF/PDF، CDF، میانگین و واریانس را برای تمام توزیع‌های اصلی بیان کنند
...
X ~ Bernoulli(p)، که در آن p = احتمال موفقیت.

### PMF

$$p_X(x) = \begin{cases} p & x = 1 \\ 1-p = q & x = 0 \end{cases}$$

شکل فشرده: p_X(x) = pˣ(1-p)^(1-x) برای x ∈ {0, 1}
...
قرارداد: X = شماره آزمایشی که نخستین موفقیت در آن رخ می‌دهد (X ∈ {1, 2, 3, ...}).
...
X ~ NegBin(r, p) = تعداد آزمایش‌ها تا r-امین موفقیت.
...
- تعداد فوتون‌های برخوردکننده با یک آشکارساز در 1 μs
- تعداد بسته‌های واردشده به یک مسیریاب در 1 ثانیه
- تعداد نقص‌ها در هر cm² روی یک ویفر سیلیکونی
- تعداد رویدادهای پرتو کیهانی در یک حسگر در هر ساعت
- تعداد جهش‌های تداخل در یک پنجره اندازه‌گیری

### تعریف ریاضی

X ~ Poisson(λ)، که در آن λ = میانگین تعداد رویدادها در هر بازه.
...
وقتی n بزرگ و p کوچک باشد، با λ = np:

$$\text{Binomial}(n, p) \approx \text{Poisson}(\lambda = np)$$

قاعده سرانگشتی: n ≥ 20 و p ≤ 0.05.
...
- فاز یک سیگنال دریافتی (نامعلوم، با احتمال برابر در [0, 2π))
- خطای گِردکردن در یک مبدل آنالوگ به دیجیتال (یکنواخت در [-Δ/2, Δ/2] که Δ = گام کوانتیزاسیون)
- زمان ورود در یک شکاف زمانی (موقعیت نامعلوم)
- پروتکل دسترسی تصادفی: انتخاب یک زمان عقب‌نشینی تصادفی

### تعریف ریاضی

X ~ Uniform(a, b) یا X ~ U(a, b)
...
X ~ Exponential(λ) که در آن λ = پارامتر نرخ (رویداد در واحد زمان).

پارامتربندی جایگزین: X ~ Exp(β) که در آن β = 1/λ = میانگین.

### PDF

$$f_X(x) = \lambda e^{-\lambda x}, \quad x \geq 0$$

یا با میانگین β: f_X(x) = (1/β)e^(-x/β)
...
اگر رویدادها به‌صورت یک فرایند پواسون با نرخ λ وارد شوند:
- تعداد رویدادها در زمان T ~ Poisson(λT)
- زمان میان رویدادها ~ Exponential(λ)
...
X ~ N(μ, σ²) که در آن μ = میانگین و σ² = واریانس.
...
Z ~ N(0, 1): شکل استانداردشده.

هر توزیع نرمال را می‌توان استانداردسازی کرد: Z = (X - μ)/σ

### قواعد کلیدی احتمال (قواعد σ)
...
1. **متقارن** حول μ
2. **تبدیل خطی:** اگر X ~ N(μ, σ²)، آنگاه aX + b ~ N(aμ + b, a²σ²)
3. **جمع گاوسی‌ها:** اگر X ~ N(μ₁, σ₁²) و Y ~ N(μ₂, σ₂²) مستقل باشند، آنگاه X + Y ~ N(μ₁+μ₂, σ₁²+σ₂²)
...
X ~ Gamma(α, β) که در آن α = پارامتر شکل و β = پارامتر مقیاس (یا نرخ λ = 1/β).

### PDF

$$f_X(x) = \frac{x^{\alpha-1} e^{-x/\beta}}{\beta^\alpha \Gamma(\alpha)}, \quad x \geq 0$$

که در آن برای α صحیح Γ(α) = (α-1)!، و در حالت کلی Γ(α) = ∫₀^∞ t^(α-1)e^(-t)dt.

### CDF

برای α کلی فرم بسته ندارد. از طریق تابع گامای ناقص در دسترس است.
...
**اندازه (پوش)** یک بردار تصادفی گاوسی دوبعدی را مدل می‌کند. هنگامی که X و Y مستقل و N(0, σ²) باشند، آنگاه R = √(X² + Y²) از توزیع رایلی پیروی می‌کند.
...
R ~ Rayleigh(σ)، که در آن σ پارامتر است (نه انحراف معیار R).
...
- R² ~ Exponential(2σ²) — توان به‌صورت نمایی توزیع شده است
- اگر X, Y ~ N(0, σ²) مستقل باشند، آنگاه √(X²+Y²) ~ Rayleigh(σ)
...
اگر Z₁, Z₂, ..., Zₙ مستقل و N(0,1) باشند، آنگاه:
...
- برای n بزرگ: χ²(n) ≈ N(n, 2n) بر اساس CLT
...
| هندسی | گسسته | p | 1/p | (1-p)/p² | geornd(p)+1 |
| دوجمله‌ای منفی | گسسته | r, p | r/p | r(1-p)/p² | nbinrnd(r,p)+r |
| پواسون | گسسته | λ | λ | λ | poissrnd(λ) |
| یکنواخت | پیوسته | a, b | (a+b)/2 | (b-a)²/12 | unifrnd(a,b) |
| نمایی | پیوسته | λ (یا β=1/λ) | 1/λ | 1/λ² | exprnd(β) |
| گاوسی | پیوسته | μ, σ² | μ | σ² | normrnd(μ,σ) |
| گاما | پیوسته | α, β | αβ | αβ² | gamrnd(α,β) |
| رایلی | پیوسته | σ | σ√(π/2) | (4-π)σ²/2 | raylrnd(σ) |
...
**حل:** (الف) Poisson(λ=2)، (ب) Gaussian(0, σ²)، (ج) Exponential(λ)، (د) Binomial(100, 0.03)، (ه) Rayleigh(σ)

### مسئله 2: نمایی در برابر پواسون
بسته‌ها به‌صورت یک فرایند پواسون با نرخ λ = 5 بسته بر ثانیه به یک مسیریاب می‌رسند.

(a) P(بیش از 8 بسته در 1 ثانیه) چقدر است؟
(b) P(هیچ بسته‌ای در 0.5 ثانیه بعدی نرسد) چقدر است؟
...
(c) نمایی با نرخ λ = 5
(d) میانگین = 1/λ = 0.2 ثانیه

### مسئله 3: احتمال گاوسی
یک مقاومت مقدار نامی 1000Ω با تغییرات ساخت مدل‌شده به‌صورت N(1000, 25) دارد (σ = 5Ω).

(a) چه کسری از مقاومت‌ها در محدوده ±10Ω حول مقدار نامی هستند (مشخصه: 990-1010Ω)؟
(b) چه کسری در آزمون رواداری ±1% رد می‌شوند؟
(c) کدام بازه رواداری شامل 99.7% مقاومت‌ها است؟

**حل:**
(a) P(990 ≤ X ≤ 1010) = P(-2 ≤ Z ≤ 2) = 0.9545 → 95.45%
(b) ±1% = ±10Ω → همانند (a)، نرخ رد = 1 - 0.9545 = 4.55%
(c) ±3σ = ±15Ω → محدوده [985, 1015]Ω
