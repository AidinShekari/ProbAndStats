...
یک \textbf{آزمایش منفرد} با دو خروجی ممکن را مدل می‌کند: «موفقیت» \lr{(1)} یا «شکست» \lr{(0)}.
...
\lr{\PSno{}} آزمایش‌های متعدد (از دوجمله‌ای استفاده کنید)\PSbr{}\lr{\PSno{}} خروجی دودویی نیست\PSbr{}\lr{\PSno{}} احتمال میان آزمایش‌ها تغییر می‌کند
...
\lr{\PSyes{}} تعداد ثابت آزمایش‌ها \lr{n}\PSbr{}\lr{\PSyes{}} هر آزمایش مستقل است\PSbr{}\lr{\PSyes{}} هر آزمایش همان احتمال \lr{p} را دارد\PSbr{}\lr{\PSyes{}} شمارش تعداد «موفقیت‌ها»

\PSsubsection{زمان عدم استفاده}

\lr{\PSno{}} آزمایش‌ها مستقل نیستند (خرابی‌های همبسته)\PSbr{}\lr{\PSno{}} احتمال میان آزمایش‌ها تغییر می‌کند\PSbr{}\lr{\PSno{} n} ثابت نیست (از دوجمله‌ای منفی یا پواسون استفاده کنید)\PSbr{}\lr{\PSno{} n} بسیار بزرگ با \lr{p} کوچک (از تقریب پواسون استفاده کنید)
...
\lr{\PSno{}} احتمال موفقیت در طول زمان تغییر می‌کند (پیری، یادگیری)\PSbr{}\lr{\PSno{}} آزمایش‌ها مستقل نیستند\PSbr{}\lr{\PSno{}} به‌دنبال \lr{r}-امین موفقیت هستید (از دوجمله‌ای منفی استفاده کنید)
...
\lr{\PSno{}} فقط نخستین موفقیت لازم است (از هندسی استفاده کنید \lr{—} ساده‌تر)\PSbr{}\lr{\PSno{}} آزمایش‌ها مستقل نیستند\PSbr{}\lr{\PSno{}} تعداد آزمایش‌ها ثابت است (از دوجمله‌ای استفاده کنید)
...
\lr{\PSno{}} رویدادها مستقل نیستند (ورودهای خوشه‌ای)\PSbr{}\lr{\PSno{}} نرخ با زمان تغییر می‌کند (فرایند ناهمگن)\PSbr{}\lr{\PSno{}} رویدادها نادر نیستند (\lr{p} کوچک نیست) \lr{—} از دوجمله‌ای استفاده کنید\PSbr{}\lr{\PSno{}} میانگین \lr{\ensuremath{\neq}} واریانس (به بیش‌پراکندگی/کم‌پراکندگی توجه کنید)
...
\lr{\PSno{}} مقادیر نزدیک به مرکز محتمل‌تر هستند (از گاوسی استفاده کنید)\PSbr{}\lr{\PSno{}} مقادیر نامنفی با کاهش نمایی هستند (از نمایی استفاده کنید)\PSbr{}\lr{\PSno{}} محدوده کران‌دار نیست
...
\item
  زمان میان رویدادها \lr{\textasciitilde{}} \lr{Exponential(\ensuremath{\lambda})}
\end{itemize}
...
\lr{\PSno{}} نرخ خرابی با سن افزایش می‌یابد (فرسودگی) \lr{—} از وایبول یا گاما استفاده کنید\PSbr{}\lr{\PSno{}} نرخ خرابی با سن کاهش می‌یابد (سوختگی اولیه) \lr{—} از وایبول استفاده کنید\PSbr{}\lr{\PSno{}} داده افزایش خطر را نشان می‌دهد \lr{—} بی‌حافظه نیست
...
\lr{\PSno{}} کمیت‌های اکیداً مثبت (گاوسی مقادیر منفی را مجاز می‌داند)\PSbr{}\lr{\PSno{}} داده‌های به‌شدت چوله\PSbr{}\lr{\PSno{}} کمیت‌های کران‌دار (گاوسی تکیه‌گاه نامتناهی دارد)\PSbr{}\lr{\PSno{}} شمارش‌های گسسته\PSbr{}\lr{\PSno{}} پدیده‌های سنگین‌دنباله (از توزیع \lr{t} یا کوشی استفاده کنید)
...
\lr{\PSno{}} داده می‌تواند منفی باشد\PSbr{}\lr{\PSno{}} داده متقارن است (از گاوسی استفاده کنید)\PSbr{}\lr{\PSno{}} توزیع ساده‌تری برازش می‌شود (نمایی برای \lr{k=1})
...
\tightlist
\item
  \lr{R\textsuperscript{2} \textasciitilde{} Exponential(2\ensuremath{\sigma}\textsuperscript{2})} \lr{—} توان به‌صورت نمایی توزیع شده است
\item
...
\lr{\PSno{}} مؤلفه خط دید قوی موجود است (از رایسی استفاده کنید)\PSbr{}\lr{\PSno{}} اندازه/پوش را مدل نمی‌کنید\PSbr{}\lr{\PSno{}} مؤلفه‌ها گاوسی نیستند یا واریانس برابر ندارند
...
اگر \lr{Z\textsubscript{1}, Z\textsubscript{2}, …, Z\textsubscript{n}} مستقل و \lr{N(0,1)} باشند، آنگاه:\PSdisplay{\chi^2 = Z_1^2 + Z_2^2 + \cdots + Z_n^2 \sim \chi^2(n)}
...
\lr{\PSno{}} متغیرها گاوسی نیستند\PSbr{}\lr{\PSno{}} متغیرها مستقل نیستند\PSbr{}\lr{\PSno{}} متغیرها نرمال استاندارد نیستند (نیاز به مقیاس‌بندی دارند)
...
\tightlist
\item
  \lr{P(\rl{بیش از 8 بسته در 1 ثانیه})} چقدر است؟
\item
  \lr{P(\rl{هیچ بسته‌ای در \PSnum{0.5} ثانیه بعدی نرسد})} چقدر است؟
\item
...
\emph{ماژول بعدی: {ماژول \lr{4} \lr{—} امید ریاضی و گشتاورها}}\PSbr{}
