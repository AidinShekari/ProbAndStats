...
\item
  \lr{\mbox{PMF/PDF}}، \lr{\mbox{CDF}}، میانگین و واریانس را برای تمام توزیع‌های اصلی بیان کنند
\item
...
\lr{\mbox{X \textasciitilde{} Bernoulli(p)}}، که در آن \lr{\mbox{p}} = احتمال موفقیت.
...
شکل فشرده: \lr{p\textsubscript{X}(x) = p\textsuperscript{x}(1-p)\textsuperscript{1-x}} برای \lr{\mbox{x \ensuremath{\in} \{0, 1\}}}
...
بیشترین واریانس در \lr{\mbox{p = 0.5}} است (بیشترین عدم‌قطعیت).
...
\textbf{تعداد موفقیت‌ها} را در \lr{\mbox{n}} آزمایش \textbf{مستقل} که هر یک همان احتمال موفقیت \lr{\mbox{p}} را دارند مدل می‌کند.
...
\tightlist
\item
  تعداد خطاهای بیت در \lr{\mbox{n}} بیت ارسال‌شده
\item
  تعداد قطعات معیوب در یک دسته \lr{\mbox{n}}‌تایی
\item
  تعداد انتقال‌های موفق بسته از میان \lr{\mbox{n}} تلاش
\item
  تعداد حسگرهایی که یک سیگنال را آشکار می‌کنند (از میان \lr{\mbox{n}} حسگر)
\end{itemize}

\begin{PSbox}[unbreakable]{def}{تعریف ریاضی}

\lr{\mbox{X \textasciitilde{} Binomial(n, p)}}
...
که در آن \lr{\mbox{C(n,k) = n}}! / \lr{\mbox{(k!(n-k)!)}}
...
\lr{\mbox{\PSyes{}}} تعداد ثابت آزمایش‌ها \lr{\mbox{n}}\PSbr{}\lr{\mbox{\PSyes{}}} هر آزمایش مستقل است\PSbr{}\lr{\mbox{\PSyes{}}} هر آزمایش همان احتمال \lr{\mbox{p}} را دارد\PSbr{}\lr{\mbox{\PSyes{}}} شمارش تعداد «موفقیت‌ها»

\PSsubsection{زمان عدم استفاده}

\lr{\mbox{\PSno{}}} آزمایش‌ها مستقل نیستند (خرابی‌های همبسته)\PSbr{}\lr{\mbox{\PSno{}}} احتمال میان آزمایش‌ها تغییر می‌کند\PSbr{}\lr{\mbox{\PSno{} n}} ثابت نیست (از دوجمله‌ای منفی یا پواسون استفاده کنید)\PSbr{}\lr{\mbox{\PSno{} n}} بسیار بزرگ با \lr{\mbox{p}} کوچک (از تقریب پواسون استفاده کنید)
...
\lr{\mbox{X \textasciitilde{} Geometric(p)}}

قرارداد: \lr{\mbox{X}} = شماره آزمایشی که نخستین موفقیت در آن رخ می‌دهد \lr{\mbox{(X \ensuremath{\in} \{1, 2, 3, …\})}}.
...
(\lr{\mbox{k-1}} شکست، سپس یک موفقیت)
...
\textbf{معنای مهندسی:} اگر یک سیستم در \lr{\mbox{m}} تلاش موفق نشده باشد، احتمال نیاز به \lr{\mbox{n}} تلاش بیشتر همانند شروع از صفر است. «شکست‌های گذشته بر احتمال آینده اثر نمی‌گذارند.»
...
\tightlist
\item
  تعداد انتقال‌ها تا تحویل موفق \lr{\mbox{r}} بسته
\item
  تعداد قطعات بازرسی‌شده تا یافتن \lr{\mbox{r}} قطعه معیوب
\item
  تعداد شکاف‌های زمانی تا در دسترس قرار گرفتن \lr{\mbox{r}} کانال
\end{itemize}

\begin{PSbox}[unbreakable]{def}{تعریف ریاضی}

\lr{\mbox{X \textasciitilde{} NegBin(r, p)}} = تعداد آزمایش‌ها تا \lr{\mbox{r}}-امین موفقیت.
...
(انتخاب این‌که کدام \lr{\mbox{r-1}} آزمایش از میان \lr{\mbox{k-1}} آزمایش نخست موفق بوده‌اند؛ آزمایش \lr{\mbox{k}}-ام همان \lr{\mbox{r}}-امین موفقیت است.)
...
وقتی \lr{\mbox{r = 1}}، توزیع دوجمله‌ای منفی به توزیع هندسی کاهش می‌یابد.
...
\tightlist
\item
  تعداد فوتون‌های برخوردکننده با یک آشکارساز در \lr{\mbox{1 \ensuremath{\mu}s}}
\item
  تعداد بسته‌های واردشده به یک مسیریاب در 1 ثانیه
\item
  تعداد نقص‌ها در هر \lr{\mbox{cm\textsuperscript{2}}} روی یک ویفر سیلیکونی
\item
...
\lr{\mbox{X \textasciitilde{} Poisson(\ensuremath{\lambda})}}، که در آن \lr{\mbox{\ensuremath{\lambda}}} = میانگین تعداد رویدادها در هر بازه.
...
وقتی \lr{\mbox{n}} بزرگ و \lr{\mbox{p}} کوچک باشد، با \lr{\mbox{\ensuremath{\lambda} = np}}:

\PSdisplay{\PSmtext{Binomial}(n, p) \approx \PSmtext{Poisson}(\lambda = np)}

قاعده سرانگشتی: \lr{\mbox{n \ensuremath{\ge} 20}} و \lr{\mbox{p \ensuremath{\le} 0.05}}.

\PSsubsection{زمان استفاده}

\lr{\mbox{\PSyes{}}} شمارش رویدادها در یک بازه ثابت\PSbr{}\lr{\mbox{\PSyes{}}} رویدادها مستقل رخ می‌دهند\PSbr{}\lr{\mbox{\PSyes{}}} رویدادها با نرخ میانگین ثابت رخ می‌دهند\PSbr{}\lr{\mbox{\PSyes{}}} رویدادهای نادر با فرصت‌های فراوان (\lr{\mbox{n}} بزرگ، \lr{\mbox{p}} کوچک)\PSbr{}\lr{\mbox{\PSyes{}}} میانگین \lr{\mbox{\ensuremath{\approx}}} واریانس در داده

\PSsubsection{زمان عدم استفاده}

\lr{\mbox{\PSno{}}} رویدادها مستقل نیستند (ورودهای خوشه‌ای)\PSbr{}\lr{\mbox{\PSno{}}} نرخ با زمان تغییر می‌کند (فرایند ناهمگن)\PSbr{}\lr{\mbox{\PSno{}}} رویدادها نادر نیستند (\lr{\mbox{p}} کوچک نیست) \lr{—} از دوجمله‌ای استفاده کنید\PSbr{}\lr{\mbox{\PSno{}}} میانگین \lr{\mbox{\ensuremath{\neq}}} واریانس (به بیش‌پراکندگی/کم‌پراکندگی توجه کنید)
...
کمیتی را مدل می‌کند که \textbf{احتمال برابر} برای اختیار کردن هر مقدار در بازه \lr{\mbox{[a, b]}} دارد. نماینده \textbf{بیشترین عدم‌قطعیت} در یک محدوده معلوم است.
...
\tightlist
\item
  فاز یک سیگنال دریافتی (نامعلوم، با احتمال برابر در \lr{\mbox{[0, 2\ensuremath{\pi})}})
\item
  خطای گِردکردن در یک مبدل آنالوگ به دیجیتال (یکنواخت در \lr{\mbox{[-\ensuremath{\Delta}/2, \ensuremath{\Delta}/2]}} که \lr{\mbox{\ensuremath{\Delta}}} = گام کوانتیزاسیون)
\item
...
\lr{\mbox{X \textasciitilde{} Uniform(a, b)}} یا \lr{\mbox{X \textasciitilde{} U(a, b)}}
...
\lr{\mbox{X \textasciitilde{} Exponential(\ensuremath{\lambda})}} که در آن \lr{\mbox{\ensuremath{\lambda}}} = پارامتر نرخ (رویداد در واحد زمان).

پارامتربندی جایگزین: \lr{\mbox{X \textasciitilde{} Exp(\ensuremath{\beta})}} که در آن \lr{\mbox{\ensuremath{\beta} = 1/\ensuremath{\lambda}}} = میانگین.
...
یا با میانگین \lr{\mbox{\ensuremath{\beta}}}: \lr{f\textsubscript{X}(x) = (1/\ensuremath{\beta})e\textsuperscript{-x/\ensuremath{\beta}}}
...
\textbf{معنای مهندسی:} قطعه‌ای که \lr{\mbox{s}} ساعت کار کرده است، از نظر آماری «به‌خوبی نو» است. عمر باقی‌مانده آن همان توزیع یک قطعه کاملاً نو را دارد.

\PSsubsection{رابطه با پواسون}

اگر رویدادها به‌صورت یک فرایند پواسون با نرخ \lr{\mbox{\ensuremath{\lambda}}} وارد شوند:

\begin{itemize}
\tightlist
\item
  تعداد رویدادها در زمان \lr{\mbox{T \textasciitilde{} Poisson(\ensuremath{\lambda}T)}}
\item
  زمان میان رویدادها \lr{\textasciitilde{}} \lr{\mbox{Exponential(\ensuremath{\lambda})}}
\end{itemize}
...
\lr{X \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2})} که در آن \lr{\mbox{\ensuremath{\mu}}} = میانگین و \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}} = واریانس.
...
\lr{\mbox{Z \textasciitilde{} N(0, 1)}}: شکل استانداردشده.

هر توزیع نرمال را می‌توان استانداردسازی کرد: \lr{\mbox{Z = (X - \ensuremath{\mu})/\ensuremath{\sigma}}}

\PSsubsection{قواعد کلیدی احتمال (قواعد \lr{\mbox{\ensuremath{\sigma}}})}
...
\bottomrule\noalign{}
\endlastfoot
\lr{\mbox{\ensuremath{\mu} \ensuremath{\pm} 1\ensuremath{\sigma}}} & \PSnum{68.27\%} \\
\lr{\mbox{\ensuremath{\mu} \ensuremath{\pm} 2\ensuremath{\sigma}}} & \PSnum{95.45\%} \\
\lr{\mbox{\ensuremath{\mu} \ensuremath{\pm} 3\ensuremath{\sigma}}} & \PSnum{99.73\%} \\
\lr{\mbox{\ensuremath{\mu} \ensuremath{\pm} 4\ensuremath{\sigma}}} & \PSnum{99.9937\%} \\
\end{longtable}
}
...
\tightlist
\item
  \textbf{متقارن} حول \lr{\mbox{\ensuremath{\mu}}}
\item
...
\lr{\mbox{\PSno{}}} کمیت‌های اکیداً مثبت (گاوسی مقادیر منفی را مجاز می‌داند)\PSbr{}\lr{\mbox{\PSno{}}} داده‌های به‌شدت چوله\PSbr{}\lr{\mbox{\PSno{}}} کمیت‌های کران‌دار (گاوسی تکیه‌گاه نامتناهی دارد)\PSbr{}\lr{\mbox{\PSno{}}} شمارش‌های گسسته\PSbr{}\lr{\mbox{\PSno{}}} پدیده‌های سنگین‌دنباله (از توزیع \lr{\mbox{t}} یا کوشی استفاده کنید)
...
\begin{PSbox}[breakable]{ex}{مثال شبیه‌سازی: تابع \lr{\mbox{Q}} و \lr{\mbox{BER}}}
...
\textbf{کل زمان انتظار تا \lr{\mbox{k}}-امین رویداد} در یک فرایند پواسون، یا هر جمعی از \lr{\mbox{k}} متغیر تصادفی نمایی مستقل را مدل می‌کند.
...
\tightlist
\item
  کل زمان تعمیر برای \lr{\mbox{k}} قطعه (هر یک با زمان تعمیر نمایی)
\item
  زمان تا \lr{\mbox{k}}-امین ورود بسته
\item
  زمان سرویس تجمعی برای \lr{\mbox{k}} کار صف‌بندی‌شده
\item
  مدل‌سازی مقدار بارش
\end{itemize}
...
\lr{X \textasciitilde{} Gamma(\ensuremath{\alpha}, \ensuremath{\beta})} که در آن \lr{\mbox{\ensuremath{\alpha}}} = پارامتر شکل و \lr{\mbox{\ensuremath{\beta}}} = پارامتر مقیاس (یا نرخ \lr{\mbox{\ensuremath{\lambda} = 1/\ensuremath{\beta}}}).
...
که در آن برای \lr{\mbox{\ensuremath{\alpha}}} صحیح \lr{\ensuremath{\Gamma}(\ensuremath{\alpha}) = (\ensuremath{\alpha}-1)}!، و در حالت کلی \lr{\ensuremath{\Gamma}(\ensuremath{\alpha}) = \ensuremath{\int}\textsubscript{0}\textsuperscript{\ensuremath{\infty}} t\textsuperscript{\ensuremath{\alpha}-1}e\textsuperscript{-t}dt}.

\PSsubsection{\lr{\mbox{CDF}}}

برای \lr{\mbox{\ensuremath{\alpha}}} کلی فرم بسته ندارد. از طریق تابع گامای ناقص در دسترس است.
...
\bottomrule\noalign{}
\endlastfoot
\lr{\mbox{\ensuremath{\alpha} = 1}} & \lr{\mbox{Exponential(\ensuremath{\beta})}} \\
\lr{\mbox{\ensuremath{\alpha} = n/2}}، \lr{\mbox{\ensuremath{\beta} = 2}} & \lr{\mbox{Chi-square(n)}} \\
\lr{\mbox{\ensuremath{\alpha} = integer k}} & \lr{\mbox{Erlang(k, \ensuremath{\beta})}} \\
\end{longtable}
}
...
\lr{\mbox{\PSno{}}} داده می‌تواند منفی باشد\PSbr{}\lr{\mbox{\PSno{}}} داده متقارن است (از گاوسی استفاده کنید)\PSbr{}\lr{\mbox{\PSno{}}} توزیع ساده‌تری برازش می‌شود (نمایی برای \lr{\mbox{k=1}})
...
\textbf{اندازه (پوش)} یک بردار تصادفی گاوسی دوبعدی را مدل می‌کند. هنگامی که \lr{\mbox{X}} و \lr{\mbox{Y}} مستقل و \lr{\mbox{N(0, \ensuremath{\sigma}\textsuperscript{2})}} باشند، آنگاه \lr{R = \ensuremath{\surd}(X\textsuperscript{2} + Y\textsuperscript{2})} از توزیع رایلی پیروی می‌کند.
...
\item
  مدل‌سازی سرعت باد
\item
  خطای فاصله در مکان‌یابی دوبعدی (هنگامی که خطاها در \lr{\mbox{x}} و \lr{\mbox{y}} گاوسی هستند)
\end{itemize}

\begin{PSbox}[unbreakable]{def}{تعریف ریاضی}

\lr{\mbox{R \textasciitilde{} Rayleigh(\ensuremath{\sigma})}}، که در آن \lr{\mbox{\ensuremath{\sigma}}} پارامتر است (نه انحراف معیار \lr{\mbox{R}}).
...
\item
  اگر \lr{\mbox{X}}، \lr{Y \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2})} مستقل باشند، آنگاه \lr{\ensuremath{\surd}(X\textsuperscript{2}+Y\textsuperscript{2}) \textasciitilde{} Rayleigh(\ensuremath{\sigma})}
\end{itemize}
...
اگر \lr{\mbox{Z\textsubscript{1}}}، \lr{\mbox{Z\textsubscript{2}}}، \lr{…}، \lr{\mbox{Z\textsubscript{n}}} مستقل و \lr{\mbox{N(0,1)}} باشند، آنگاه:\PSdisplay{\chi^2 = Z_1^2 + Z_2^2 + \cdots + Z_n^2 \sim \chi^2(n)}

\lr{\mbox{n}} = درجات آزادی.
...
برای \lr{\mbox{n}} کلی فرم بسته ندارد. از طریق تابع گامای ناقص محاسبه می‌شود.
...
\tightlist
\item
  \lr{\mbox{Chi-square(n) = Gamma(n/2, 2)}}
\item
  \lr{\mbox{Chi-square(1)}} = مربع \lr{\mbox{N(0,1)}}
\item
  \lr{\mbox{Chi-square(2) = Exponential(1/2)}}
\item
  برای \lr{\mbox{n}} بزرگ: \lr{\ensuremath{\chi}\textsuperscript{2}(n) \ensuremath{\approx} N(n, 2n)} بر اساس \lr{\mbox{CLT}}
\end{itemize}

\PSsubsection{زمان استفاده}

\lr{\mbox{\PSyes{}}} جمع مربعات متغیرهای تصادفی گاوسی\PSbr{}\lr{\mbox{\PSyes{}}} آزمون واریانس و بازه‌های اطمینان\PSbr{}\lr{\mbox{\PSyes{}}} آزمون‌های نیکویی برازش\PSbr{}\lr{\mbox{\PSyes{}}} آشکارسازی انرژی در \lr{\mbox{N}} بُعد
...
\endlastfoot
خروجی دودویی & برنولی \\
«\lr{\mbox{n}} آزمایش، \lr{\mbox{k}} موفقیت» & دوجمله‌ای \\
«چند تا تا نخستین\lr{…}» & هندسی \\
«رویداد در هر بازه» & پواسون \\
«شانس برابر در هر جای \lr{\mbox{[a,b]}}» & یکنواخت \\
«زمان تا رویداد بعدی» & نمایی \\
«جمع بسیاری از اثرها» یا «نویز» & گاوسی \\
«کل زمان انتظار برای \lr{\mbox{k}} رویداد» & گاما \\
...
\bottomrule\noalign{}
\endlastfoot
برنولی & گسسته & \lr{\mbox{p}} & \lr{\mbox{p}} & \lr{\mbox{p(1-p)}} & \lr{\mbox{binornd(1,p)}} \\
دوجمله‌ای & گسسته & \lr{\mbox{n}}، \lr{\mbox{p}} & \lr{\mbox{np}} & \lr{\mbox{np(1-p)}} & \lr{\mbox{binornd(n,p)}} \\
هندسی & گسسته & \lr{\mbox{p}} & \lr{\mbox{1/p}} & \lr{\mbox{(1-p)/p\textsuperscript{2}}} & \lr{\mbox{geornd(p)+1}} \\
دوجمله‌ای منفی & گسسته & \lr{\mbox{r}}، \lr{\mbox{p}} & \lr{\mbox{r/p}} & \lr{\mbox{r(1-p)/p\textsuperscript{2}}} & \lr{\mbox{nbinrnd(r,p)+r}} \\
پواسون & گسسته & \lr{\mbox{\ensuremath{\lambda}}} & \lr{\mbox{\ensuremath{\lambda}}} & \lr{\mbox{\ensuremath{\lambda}}} & \lr{\mbox{poissrnd(\ensuremath{\lambda})}} \\
یکنواخت & پیوسته & \lr{\mbox{a}}، \lr{\mbox{b}} & \lr{\mbox{(a+b)/2}} & \lr{\mbox{(b-a)\textsuperscript{2}/12}} & \lr{\mbox{unifrnd(a,b)}} \\
نمایی & پیوسته & \lr{\mbox{\ensuremath{\lambda}}} (یا \lr{\mbox{\ensuremath{\beta}=1/\ensuremath{\lambda}}}) & \lr{\mbox{1/\ensuremath{\lambda}}} & \lr{\mbox{1/\ensuremath{\lambda}\textsuperscript{2}}} & \lr{\mbox{exprnd(\ensuremath{\beta})}} \\
گاوسی & پیوسته & \lr{\mbox{\ensuremath{\mu}}}، \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}} & \lr{\mbox{\ensuremath{\mu}}} & \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}} & \lr{\mbox{normrnd(\ensuremath{\mu},\ensuremath{\sigma})}} \\
گاما & پیوسته & \lr{\mbox{\ensuremath{\alpha}}}، \lr{\mbox{\ensuremath{\beta}}} & \lr{\mbox{\ensuremath{\alpha}\ensuremath{\beta}}} & \lr{\mbox{\ensuremath{\alpha}\ensuremath{\beta}\textsuperscript{2}}} & \lr{\mbox{gamrnd(\ensuremath{\alpha},\ensuremath{\beta})}} \\
رایلی & پیوسته & \lr{\mbox{\ensuremath{\sigma}}} & \lr{\mbox{\ensuremath{\sigma}\ensuremath{\surd}(\ensuremath{\pi}/2)}} & \lr{(4-\ensuremath{\pi})\ensuremath{\sigma}\textsuperscript{2}/2} & \lr{\mbox{raylrnd(\ensuremath{\sigma})}} \\
کای‌دو & پیوسته & \lr{\mbox{n}} & \lr{\mbox{n}} & \lr{\mbox{2n}} & \lr{\mbox{chi2rnd(n)}} \\
\end{longtable}
}
...
\PSsolution{} (الف) \lr{\mbox{Poisson(\ensuremath{\lambda}=2)}}، (ب) \lr{\mbox{Gaussian(0, \ensuremath{\sigma}\textsuperscript{2})}}، (ج) \lr{\mbox{Exponential(\ensuremath{\lambda})}}، (د) \lr{\mbox{Binomial(100, 0.03)}}، (ه) \lr{\mbox{Rayleigh(\ensuremath{\sigma})}}

\end{PSbox}

\begin{PSbox}[unbreakable]{prob}{مسیله 2: نمایی در برابر پواسون}

بسته‌ها به‌صورت یک فرایند پواسون با نرخ \lr{\mbox{\ensuremath{\lambda} = 5}} بسته بر ثانیه به یک مسیریاب می‌رسند.

\begin{enumerate}
\def\labelenumi{(\PSalph{enumi})}
\tightlist
\item
  \lr{\mbox{P(\rl{بیش از 8 بسته در 1 ثانیه})}} چقدر است؟
\item
...
\tightlist
\item
  \lr{\mbox{X \textasciitilde{} Poisson(5)}}: \lr{\mbox{P(X \textgreater{} 8) = 1 - poisscdf(8, 5) = 0.0681}}
\item
  \lr{\mbox{T \textasciitilde{} Exp(5)}}: \lr{P(T \textgreater{} 0.5) = e\textsuperscript{-5\ensuremath{\times}0.5} = e\textsuperscript{-2.5} = 0.0821}
\item
  نمایی با نرخ \lr{\mbox{\ensuremath{\lambda} = 5}}
\item
  میانگین = \lr{\mbox{1/\ensuremath{\lambda} = 0.2}} ثانیه
\end{enumerate}
...
یک مقاومت مقدار نامی \lr{\mbox{1000\ensuremath{\Omega}}} با تغییرات ساخت مدل‌شده به‌صورت \lr{\mbox{N(1000, 25)}} دارد \lr{\mbox{(\ensuremath{\sigma} = 5\ensuremath{\Omega})}}.

\begin{enumerate}
\def\labelenumi{(\PSalph{enumi})}
\tightlist
\item
  چه کسری از مقاومت‌ها در محدوده \lr{\mbox{\ensuremath{\pm}10\ensuremath{\Omega}}} حول مقدار نامی هستند (مشخصه: \lr{\mbox{990-1010\ensuremath{\Omega}}})؟
\item
  چه کسری در آزمون رواداری \lr{\mbox{\ensuremath{\pm}1\%}} رد می‌شوند؟
\item
...
\item
  \lr{\ensuremath{\pm}1\% = \ensuremath{\pm}10\ensuremath{\Omega} \ensuremath{\to}} همانند \lr{\mbox{(a)}}، نرخ رد = \lr{\mbox{1 - 0.9545 = 4.55\%}}
\item
  \lr{\ensuremath{\pm}3\ensuremath{\sigma} = \ensuremath{\pm}15\ensuremath{\Omega} \ensuremath{\to}} محدوده \lr{\mbox{[985, 1015]\ensuremath{\Omega}}}
\end{enumerate}
