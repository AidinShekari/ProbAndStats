...
\textbf{مدل:} هر بیت یک آزمایش برنولی با احتمال خطای \lr{\mbox{p}} است (همان \lr{\mbox{BER}}).

\begin{itemize}
\tightlist
\item
  \lr{\mbox{n}} بیت ارسال‌شده، \lr{\mbox{k}} خطا مشاهده‌شده
\item
  \lr{\mbox{\ensuremath{\hat{p}} = k/n}} (برآوردگر \lr{\mbox{MLE}} برای \lr{\mbox{BER}})
\item
  بازه اطمینان 95\%: \lr{\ensuremath{\hat{p}} \ensuremath{\pm} 1.96\ensuremath{\surd}(\ensuremath{\hat{p}}(1-\ensuremath{\hat{p}})/n)}
\end{itemize}
...
\tightlist
\item
  \textbf{توزیع:} \lr{\mbox{N(0, N\textsubscript{0}/2)}} در هر بُعد
\item
  \textbf{چرا گاوسی؟} \lr{\mbox{CLT}} \lr{—} نویز حرارتی جمع سهم‌های بسیاری از الکترون‌ها است
\item
  \textbf{مشخصه‌سازی:} کاملاً با چگالی طیفی توان \lr{\mbox{N\textsubscript{0}}} یا واریانس \lr{\ensuremath{\sigma}\textsuperscript{2} = N\textsubscript{0}B} توصیف می‌شود (\lr{\mbox{B}} = پهنای‌باند)
\end{itemize}

\PSsubsection{\PSnum{16.1.3} کانال محوشدگی رایلی}

\textbf{مدل:} ضریب کانال \lr{h = h\textsubscript{I} + j\ensuremath{\cdot}h\textsubscript{Q}} که در آن \lr{\mbox{h\textsubscript{I}}}، \lr{h\textsubscript{Q} \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2})} و مستقل هستند.
...
\tightlist
\item
  \lr{\mbox{h = h\textsubscript{LOS} + h\textsubscript{scatter}}}
\item
  \lr{\textbar{}h\textbar{} \textasciitilde{} Rician(\ensuremath{\nu}, \ensuremath{\sigma})} که در آن \lr{K = \ensuremath{\nu}\textsuperscript{2}/(2\ensuremath{\sigma}\textsuperscript{2})} ضریب \lr{\mbox{K}} است
\end{itemize}

ضریب \lr{\mbox{K}}: \lr{\mbox{K = 0}} (رایلی) \lr{\mbox{\ensuremath{\to} K = \ensuremath{\infty}}} (بدون محوشدگی، \lr{\mbox{AWGN}})

\PSsubsection{\PSnum{16.1.5} آماره‌های \lr{\mbox{SNR}}}

\textbf{\lr{\mbox{SNR}} لحظه‌ای:} \lr{\ensuremath{\gamma} = \textbar{}h\textbar{}\textsuperscript{2} \ensuremath{\times} (E\textsubscript{s}/N\textsubscript{0})}

برای رایلی: \lr{\ensuremath{\gamma} \textasciitilde{} Exponential(\ensuremath{\bar{\gamma}})} که در آن \lr{\mbox{\ensuremath{\bar{\gamma}} = SNR}} میانگین

\textbf{میانگین \lr{\mbox{BER}} در محوشدگی:}\PSdisplay{\bar{P}_e = \int_0^\infty P_e(\gamma) \cdot f_\gamma(\gamma) \, d\gamma}

برای \lr{\mbox{BPSK}} در کانال رایلی: \lr{\ensuremath{\bar{P}}\textsubscript{e} = (1/2)(1 - \ensuremath{\surd}(\ensuremath{\bar{\gamma}}/(1+\ensuremath{\bar{\gamma}})))}

\PSsection{16.2}{پردازش سیگنال}

\PSsubsection{\PSnum{16.2.1} مشخصه‌سازی نویز اندازه‌گیری}

\textbf{مسیله:} مشخصه‌سازی نویز در سیگنال اندازه‌گیری‌شده \lr{\mbox{x(t) = s(t) + n(t)}}.
...
\item
  آماره‌های نویز را برآورد کنید: \lr{\mbox{\ensuremath{\hat{\mu}}\textsubscript{n}}}، \lr{\ensuremath{\hat{\sigma}}\textsuperscript{2}\textsubscript{n}}
\item
  گاوسی‌بودن را آزمون کنید (هیستوگرام، نمودار \lr{\mbox{QQ}})
\item
  \lr{\mbox{SNR}} را محاسبه کنید: \lr{SNR = P\textsubscript{signal} / \ensuremath{\hat{\sigma}}\textsuperscript{2}\textsubscript{n}}
\end{enumerate}
...
\textbf{روش:} سیگنال دریافتی را با الگوی معلوم همبسته کنید. تحت \lr{\mbox{H\textsubscript{0}}} (فقط نویز)، خروجی همبسته‌ساز توزیع معلومی دارد \lr{\mbox{\ensuremath{\to}}} آستانه را تعیین کنید.
...
\textbf{مسیله:} مقاومت‌ها مقدار نامی \lr{\mbox{R\textsubscript{0}}} با رواداری ساخت دارند.

\textbf{مدل:} \lr{R \textasciitilde{} N(R\textsubscript{0}, \ensuremath{\sigma}\textsubscript{R}\textsuperscript{2})} که در آن \lr{\mbox{\ensuremath{\sigma}\textsubscript{R}}} به رده رواداری بستگی دارد.

برای رواداری \lr{\mbox{\ensuremath{\pm}5\%}} (\PSnum{99.7\%} در محدوده): \lr{3\ensuremath{\sigma} = 0.05R\textsubscript{0} \ensuremath{\to} \ensuremath{\sigma} = 0.0167R\textsubscript{0}}

\PSsubsection{\PSnum{16.3.2} تحلیل مدار با مونت‌کارلو}

\textbf{مسیله:} یک تقسیم‌کننده ولتاژ از \lr{\mbox{R\textsubscript{1}}} و \lr{\mbox{R\textsubscript{2}}} استفاده می‌کند. توزیع ولتاژ خروجی چیست؟

\lr{V\textsubscript{out} = V\textsubscript{in} \ensuremath{\times} R\textsubscript{2}/(R\textsubscript{1} + R\textsubscript{2})}

با تصادفی‌بودن \lr{\mbox{R\textsubscript{1}}} و \lr{\mbox{R\textsubscript{2}}}: \lr{\mbox{V\textsubscript{out}}} یک متغیر تصادفی است!
...
اگر \lr{\mbox{Z = f(X, Y)}} و \lr{\mbox{X}}، \lr{\mbox{Y}} عدم‌قطعیت‌های کوچک داشته باشند:
...
\textbf{مثال:} توان \lr{\mbox{P = V\textsuperscript{2}/R}}. \lr{\ensuremath{\sigma}\textsubscript{P}\textsuperscript{2} \ensuremath{\approx} (2V/R)\textsuperscript{2}\ensuremath{\sigma}\textsubscript{V}\textsuperscript{2} + (V\textsuperscript{2}/R\textsuperscript{2})\textsuperscript{2}\ensuremath{\sigma}\textsubscript{R}\textsuperscript{2}}
...
پرسش طراحی: چه ظرفیت \lr{\mbox{C}} تضمین می‌کند \lr{P(\rl{بار} \textgreater{} \rl{ظرفیت}) \textless{} 0.01}؟
...
اندازه‌گیری: \lr{\mbox{y(k) = x(k) + n(k)}} که در آن \lr{n \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2}\textsubscript{n})}

کنترل‌کننده روی \lr{\mbox{y(k)}} عمل می‌کند \lr{\mbox{\ensuremath{\to}}} خطای ردیابی شامل سهم نویز می‌شود.
...
\tightlist
\item
  میانگین‌گیری (فیلتر پایین‌گذر): نویز را به اندازه \lr{\mbox{\ensuremath{\surd}n}} کاهش می‌دهد
\item
...
مدل سیستم: \lr{\mbox{G(s) = K/(\ensuremath{\tau}s + 1)}} که در آن \lr{\mbox{K}} و \lr{\mbox{\ensuremath{\tau}}} نامعین هستند.

رویکرد مونت‌کارلو: \lr{\mbox{K}} و \lr{\mbox{\ensuremath{\tau}}} را از توزیع‌هایشان نمونه‌گیری کنید، پاسخ سیستم را بارها شبیه‌سازی کنید و آماره‌های خروجی را محاسبه کنید.
...
\tightlist
\item
  مدل‌های مولد: یادگیری مستقیم \lr{\mbox{P(X)}}
\item
