...
- p̂ = k/n (برآوردگر MLE برای BER)
- بازه اطمینان 95%: p̂ ± 1.96√(p̂(1-p̂)/n)
...
- **توزیع:** N(0, N₀/2) در هر بُعد
- **چرا گاوسی؟** CLT — نویز حرارتی جمع سهم‌های بسیاری از الکترون‌ها است
- **مشخصه‌سازی:** کاملاً با چگالی طیفی توان N₀ یا واریانس σ² = N₀B توصیف می‌شود (B = پهنای‌باند)

### 16.1.3 کانال محوشدگی رایلی

**مدل:** ضریب کانال h = h_I + j·h_Q که در آن h_I, h_Q ~ N(0, σ²) و مستقل هستند.

- پوش |h| ~ Rayleigh(σ)
- توان |h|² ~ Exponential(1/(2σ²))
- **احتمال قطع:** P(|h|² < γ_th) = 1 - exp(-γ_th/(2σ²))
...
- h = h_LOS + h_scatter
- |h| ~ Rician(ν, σ) که در آن K = ν²/(2σ²) ضریب K است

ضریب K: K = 0 (رایلی) → K = ∞ (بدون محوشدگی، AWGN)

### 16.1.5 آماره‌های SNR

**SNR لحظه‌ای:** γ = |h|² × (E_s/N₀)

برای رایلی: γ ~ Exponential(γ̄) که در آن γ̄ = SNR میانگین

**میانگین BER در محوشدگی:**
$$\bar{P}_e = \int_0^\infty P_e(\gamma) \cdot f_\gamma(\gamma) \, d\gamma$$

برای BPSK در کانال رایلی: P̄_e = (1/2)(1 - √(γ̄/(1+γ̄)))
...
2. آماره‌های نویز را برآورد کنید: μ̂_n، σ̂²_n
3. گاوسی‌بودن را آزمون کنید (هیستوگرام، نمودار QQ)
4. SNR را محاسبه کنید: SNR = P_signal / σ̂²_n
...
**روش:** سیگنال دریافتی را با الگوی معلوم همبسته کنید. تحت H₀ (فقط نویز)، خروجی همبسته‌ساز توزیع معلومی دارد → آستانه را تعیین کنید.
...
**مسئله:** مقاومت‌ها مقدار نامی R₀ با رواداری ساخت دارند.

**مدل:** R ~ N(R₀, σ_R²) که در آن σ_R به رده رواداری بستگی دارد.

برای رواداری ±5% (99.7% در محدوده): 3σ = 0.05R₀ → σ = 0.0167R₀

### 16.3.2 تحلیل مدار با مونت‌کارلو

**مسئله:** یک تقسیم‌کننده ولتاژ از R₁ و R₂ استفاده می‌کند. توزیع ولتاژ خروجی چیست؟

V_out = V_in × R₂/(R₁ + R₂)

با تصادفی‌بودن R₁ و R₂: V_out یک متغیر تصادفی است!
...
**مثال:** توان P = V²/R. σ_P² ≈ (2V/R)²σ_V² + (V²/R²)²σ_R²
...
**مسئله:** بار اوج نامعین است. مدل: L ~ N(μ_L, σ_L²)

پرسش طراحی: چه ظرفیت C تضمین می‌کند P(بار > ظرفیت) < 0.01؟

C = μ_L + 2.326σ_L (برای احتمال تخطی 1%)

### 16.4.2 قابلیت اطمینان سیستم

**سیستم سری:** همه قطعات باید کار کنند.

R_sys = R₁ × R₂ × ... × Rₙ (برای قطعات مستقل)
...
اندازه‌گیری: y(k) = x(k) + n(k) که در آن n ~ N(0, σ²_n)

کنترل‌کننده روی y(k) عمل می‌کند → خطای ردیابی شامل سهم نویز می‌شود.

**رویکردهای حل:**
- میانگین‌گیری (فیلتر پایین‌گذر): نویز را به اندازه √n کاهش می‌دهد
...
مدل سیستم: G(s) = K/(τs + 1) که در آن K و τ نامعین هستند.

رویکرد مونت‌کارلو: K و τ را از توزیع‌هایشان نمونه‌گیری کنید، پاسخ سیستم را بارها شبیه‌سازی کنید و آماره‌های خروجی را محاسبه کنید.
