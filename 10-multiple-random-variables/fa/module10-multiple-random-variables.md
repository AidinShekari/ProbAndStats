...
X₁, ..., Xₙ به‌طور متقابل مستقل هستند اگر:
...
|--------------|-----------|---------|
| خروجی آرایه حسگر | X₁,...,Xₙ = قرائت‌های حسگر | پردازش آرایه |
| کانال MIMO | h₁,...,hₙ = بهره‌های کانال | مخابرات بی‌سیم |
| بردار نویز | N₁,...,Nₙ = نمونه‌های نویز | پردازش سیگنال |
...
1. **متقارن:** Σ = Σᵀ
2. **نیم‌معین مثبت:** vᵀΣv ≥ 0 برای تمام v
3. **قطر = واریانس‌ها:** Σᵢᵢ = Var(Xᵢ)
4. **غیرقطری = کوواریانس‌ها:** Σᵢⱼ = Cov(Xᵢ, Xⱼ)
5. **مؤلفه‌های مستقل → Σ قطری**
...
که در آن |Σ| = دترمینان Σ.

### ویژگی‌های کلیدی

1. **حاشیه‌ای‌ها گاوسی هستند:** هر زیرمجموعه‌ای از مؤلفه‌ها به‌طور مشترک گاوسی است
2. **شرطی‌ها گاوسی هستند:** X₁|X₂=x₂ گاوسی است
3. **تبدیل‌های خطی:** Y = AX + b ~ N(Aμ+b, AΣAᵀ)
4. **ناهمبسته = مستقل** (ویژه گاوسی!)
5. **کاملاً مشخص‌شده** با μ و Σ (فقط 2 پارامتر برای هر متغیر به‌علاوه همبستگی‌ها لازم است)
...
اگر X یک بردار تصادفی با میانگین μ_X و کوواریانس Σ_X باشد:
...
**شکل‌دهی پرتو (Beamforming):** خروجی y = wᵀX (بردار وزن اعمال‌شده به آرایه حسگر)
- E[y] = wᵀμ
- Var(y) = wᵀΣw

**سفیدسازی:** ماتریس W را بیابید که در آن WΣWᵀ = I (داده را ناهمبسته می‌کند)
- W = Σ^(-1/2)

**تحلیل مؤلفه‌های اصلی:** چرخش برای قطری‌کردن Σ
...
### مسئله 1
بردار تصادفی X = [X₁, X₂, X₃]ᵀ با μ = [1, 0, -1]ᵀ و ماتریس کوواریانس Σ = [4 1 0; 1 2 -1; 0 -1 3].
(a) Var(X₁ + X₂ + X₃) را بیابید. (ب) Cov(X₁, X₂+X₃) را بیابید. (ج) آیا X₁ و X₃ ناهمبسته هستند؟

**حل:**
(a) Var(X₁+X₂+X₃) = 1ᵀΣ1 = 4+2+3 + 2(1) + 2(0) + 2(-1) = 9 + 0 = 9
توجه: = Σᵢⱼ تمام درایه‌ها = 4+1+0+1+2+(-1)+0+(-1)+3 = 9
(b) Cov(X₁, X₂+X₃) = Cov(X₁,X₂) + Cov(X₁,X₃) = 1 + 0 = 1
(c) Cov(X₁,X₃) = 0، بنابراین بله، X₁ و X₃ ناهمبسته هستند.

### مسئله 2
X ~ N(0, Σ) با Σ = [1 ρ; ρ 1]. توزیع شرطی X₁|X₂=x₂ را بیابید.

**حل:** برای گاوسی دوبعدی:
X₁|X₂=x₂ ~ N(μ₁ + ρ(σ₁/σ₂)(x₂-μ₂), σ₁²(1-ρ²))
= N(ρx₂, 1-ρ²)

میانگین شرطی تابعی خطی از x₂ است و واریانس شرطی با ضریب (1-ρ²) کاهش می‌یابد.

### مسئله 3
کد MATLAB را برای تولید یک بردار گاوسی سه‌بعدی با μ و Σ معلوم، محاسبه کوواریانس نمونه و بررسی آن بنویسید.
