...
\tightlist
\item
  \textbf{احتمال:} «اگر نویز \lr{N(0, 0.01)} باشد، \lr{P(error)} چقدر است؟»
\item
  \textbf{آمار:} «\lr{1000} نمونه نویز اندازه‌گیری کردم. \lr{\ensuremath{\sigma}\textsuperscript{2}} چقدر است؟»
\end{itemize}
...
\lr{\PSno{}} نمونه‌های متوالی یک سیگنال همبسته\PSbr{}\lr{\PSno{}} اندازه‌گیری‌ها در طول یک فرایند رانش‌دار\PSbr{}\lr{\PSno{}} انتخاب غیرتصادفی (مثلاً آزمایش فقط قطعات «مشکوک»)
...
\textbf{میانگین \lr{\ensuremath{\bar{X}}}:}\PSdisplay{E[\bar{X}] = \mu}

\lr{\ensuremath{\bar{X}}} یک برآوردگر \textbf{نااریب} برای \lr{\ensuremath{\mu}} است (به‌طور میانگین برابر مقدار واقعی است).

\textbf{واریانس \lr{\ensuremath{\bar{X}}}:}\PSdisplay{\PSmtext{Var}(\bar{X}) = \frac{\sigma^2}{n}}
...
\textbf{شهود:} \lr{\ensuremath{\bar{X}}} از همان داده محاسبه می‌شود، بنابراین انحراف‌ها از \lr{\ensuremath{\bar{X}}} کوچک‌تر از انحراف‌ها از \lr{\ensuremath{\mu}} هستند. با برآورد میانگین، «یک درجه آزادی را از دست می‌دهیم».
...
اگر جامعه \lr{N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2})} باشد:\PSdisplay{(n-1)S^2/\sigma^2 \sim \chi^2(n-1)}

\PSsection{12.8}{انحراف معیار نمونه}

\PSdisplay{S = \sqrt{S^2} = \sqrt{\frac{1}{n-1}\sum_{i=1}^n (X_i - \bar{X})^2}}

توجه: \lr{E[S] \ensuremath{\neq} \ensuremath{\sigma}} (\lr{S} برای \lr{\ensuremath{\sigma}} کمی اریب است، هرچند \lr{S\textsuperscript{2}} برای \lr{\ensuremath{\sigma}\textsuperscript{2}} نااریب است).
...
\emph{ماژول بعدی: {ماژول 13 \lr{—} برآورد پارامتر}}\PSbr{}
