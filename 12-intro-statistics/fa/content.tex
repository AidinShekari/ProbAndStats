...
\item
  گذار از احتمال (مدل\lr{\mbox{\ensuremath{\to}}}پیش‌بینی) به آمار (داده\lr{\mbox{\ensuremath{\to}}}مدل) را درک کنند
\end{enumerate}
...
\tightlist
\item
  \textbf{احتمال:} «اگر نویز \lr{\mbox{N(0, 0.01)}} باشد، \lr{\mbox{P(error)}} چقدر است؟»
\item
  \textbf{آمار:} «\lr{\mbox{1000}} نمونه نویز اندازه‌گیری کردم. \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}} چقدر است؟»
\end{itemize}
...
\tightlist
\item
  \lr{\mbox{\ensuremath{\mu}}} = میانگین واقعی جامعه
\item
  \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}} = واریانس واقعی جامعه
\item
  \lr{\mbox{p}} = نرخ واقعی نقص
\item
  \lr{\mbox{\ensuremath{\lambda}}} = نرخ واقعی خرابی
\end{itemize}
...
\tightlist
\item
  \lr{\mbox{\ensuremath{\bar{X}}}} = میانگین نمونه (برآورد \lr{\mbox{\ensuremath{\mu}}})
\item
  \lr{\mbox{S\textsuperscript{2}}} = واریانس نمونه (برآورد \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}})
\item
  \lr{\mbox{\ensuremath{\hat{p}}}} = نسبت نمونه (برآورد \lr{\mbox{p}})
\end{itemize}
...
آزمایش 1: جمع‌آوری 100 نمونه \lr{\mbox{\ensuremath{\to}}} محاسبه \lr{\mbox{S\textsuperscript{2} = 0.038}} (یک تحقق)\PSbr{}آزمایش 2: جمع‌آوری 100 نمونه \lr{\mbox{\ensuremath{\to}}} محاسبه \lr{\mbox{S\textsuperscript{2} = 0.042}} (تحقق متفاوت)\PSbr{}آزمایش 3: جمع‌آوری 100 نمونه \lr{\mbox{\ensuremath{\to}}} محاسبه \lr{\mbox{S\textsuperscript{2} = 0.041}} (تحقق دیگر)

هر \lr{\mbox{S\textsuperscript{2}}} به‌دلیل تصادفی‌بودن نمونه‌گیری متفاوت است.
...
یک \textbf{نمونه تصادفی} \lr{\mbox{X\textsubscript{1}}}، \lr{\mbox{X\textsubscript{2}}}، \lr{…}، \lr{\mbox{X\textsubscript{n}}} مجموعه‌ای از \lr{\mbox{n}} متغیر تصادفی مستقل و هم‌توزیع \lr{\mbox{(i.i.d.)}} از توزیع جامعه است.
...
\textbf{توزیع نمونه‌گیری} یک آماره همان توزیع احتمال آن آماره در تمام نمونه‌های ممکن به اندازه \lr{\mbox{n}} است.

\end{PSbox}

\PSsubsection{چرا اهمیت دارد}

یک آماره (مانند \lr{\mbox{\ensuremath{\bar{X}}}}) یک متغیر تصادفی است. توزیع آن به ما می‌گوید:
...
\PSsection{12.6}{میانگین نمونه \lr{\mbox{\ensuremath{\bar{X}}}}}
...
\textbf{میانگین \lr{\mbox{\ensuremath{\bar{X}}}}:}\PSdisplay{E[\bar{X}] = \mu}

\lr{\mbox{\ensuremath{\bar{X}}}} یک برآوردگر \textbf{نااریب} برای \lr{\mbox{\ensuremath{\mu}}} است (به‌طور میانگین برابر مقدار واقعی است).

\textbf{واریانس \lr{\mbox{\ensuremath{\bar{X}}}}:}\PSdisplay{\PSmtext{Var}(\bar{X}) = \frac{\sigma^2}{n}}
...
\PSsubsection{توزیع نمونه‌گیری \lr{\mbox{\ensuremath{\bar{X}}}}}

\begin{itemize}
\tightlist
\item
  اگر جامعه گاوسی باشد: \lr{\ensuremath{\bar{X}} \textasciitilde{} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}/n)} \textbf{به‌طور دقیق} برای هر \lr{\mbox{n}}
\item
  اگر جامعه گاوسی نباشد: \lr{\ensuremath{\bar{X}} \ensuremath{\approx} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}/n)} \textbf{به‌طور تقریبی} برای \lr{\mbox{n}} بزرگ (بر اساس \lr{\mbox{CLT}})
\end{itemize}

\PSsection{12.7}{واریانس نمونه \lr{\mbox{S\textsuperscript{2}}}}
...
\PSsubsection{چرا \lr{\mbox{n-1}}؟ (تصحیح بسل)}

استفاده از \lr{\mbox{n}} در مخرج یک برآوردگر \textbf{اریب} می‌دهد (به‌طور نظام‌مند \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}} را کم‌برآورد می‌کند):
...
استفاده از \lr{\mbox{n-1}} این را تصحیح می‌کند:\PSdisplay{E[S^2] = \sigma^2}

\textbf{شهود:} \lr{\mbox{\ensuremath{\bar{X}}}} از همان داده محاسبه می‌شود، بنابراین انحراف‌ها از \lr{\mbox{\ensuremath{\bar{X}}}} کوچک‌تر از انحراف‌ها از \lr{\mbox{\ensuremath{\mu}}} هستند. با برآورد میانگین، «یک درجه آزادی را از دست می‌دهیم».

\PSsubsection{درجات آزادی}

با \lr{\mbox{n}} نقطه داده و برآورد \lr{\mbox{\ensuremath{\bar{X}}}}:

\begin{itemize}
\tightlist
\item
  \lr{\mbox{n}} انحراف \lr{\mbox{(X\textsubscript{i} - \ensuremath{\bar{X}})}} جمعاً صفر می‌شوند: \lr{\ensuremath{\Sigma}(X\textsubscript{i} - \ensuremath{\bar{X}}) = 0}
\item
  تنها \lr{\mbox{n-1}} تای آن‌ها «آزاد» هستند \lr{—} آخری تعیین‌شده است
\item
  \lr{\mbox{df = n - 1}}
\end{itemize}
...
اگر جامعه \lr{\mbox{N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2})}} باشد:\PSdisplay{(n-1)S^2/\sigma^2 \sim \chi^2(n-1)}

\PSsection{12.8}{انحراف معیار نمونه}

\PSdisplay{S = \sqrt{S^2} = \sqrt{\frac{1}{n-1}\sum_{i=1}^n (X_i - \bar{X})^2}}

توجه: \lr{\mbox{E[S] \ensuremath{\neq} \ensuremath{\sigma}}} (\lr{\mbox{S}} برای \lr{\mbox{\ensuremath{\sigma}}} کمی اریب است، هرچند \lr{\mbox{S\textsuperscript{2}}} برای \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}} نااریب است).

\PSsection{12.9}{ارتباط با \lr{\mbox{CLT}}}

از ماژول 11: \lr{\ensuremath{\bar{X}} \ensuremath{\to} N(\ensuremath{\mu}, \ensuremath{\sigma}\textsuperscript{2}/n)} وقتی \lr{\mbox{n \ensuremath{\to} \ensuremath{\infty}}}.

این بدان معناست که توزیع نمونه‌گیری \lr{\mbox{\ensuremath{\bar{X}}}} تقریباً گاوسی است. این همان چیزی است که به ما امکان می‌دهد:
...
\begin{PSbox}[unbreakable]{ex}{مثال 1: توزیع نمونه‌گیری \lr{\mbox{\ensuremath{\bar{X}}}}}
...
یک جامعه دارای \lr{\mbox{\ensuremath{\mu} = 50}} و \lr{\mbox{\ensuremath{\sigma} = 10}} است. یک نمونه تصادفی به اندازه \lr{\mbox{n = 64}} گرفته می‌شود.

\begin{enumerate}
\def\labelenumi{(\PSalph{enumi})}
\tightlist
\item
  \lr{\mbox{E[\ensuremath{\bar{X}}]}} چقدر است؟ (ب) \lr{\mbox{Var(\ensuremath{\bar{X}})}} چقدر است؟ (ج) \lr{\mbox{P(48 \textless{} \ensuremath{\bar{X}} \textless{} 52)}} چقدر است؟
\end{enumerate}
...
\tightlist
\item
  \lr{\mbox{E[\ensuremath{\bar{X}}] = 50}}
\item
  \lr{\mbox{Var(\ensuremath{\bar{X}}) = 100/64 = 1.5625}}، \lr{\mbox{SE = 1.25}}
\item
...
چرا در فرمول واریانس نمونه از \lr{\mbox{n-1}} استفاده می‌کنیم و نه \lr{\mbox{n}}؟ هم به‌صورت ریاضی و هم به‌صورت شهودی توضیح دهید.

\PSsolution{} از نظر ریاضی: \lr{E[(1/n)\ensuremath{\Sigma}(X\textsubscript{i}-\ensuremath{\bar{X}})\textsuperscript{2}] = (n-1)/n \ensuremath{\cdot} \ensuremath{\sigma}\textsuperscript{2}}، بنابراین تقسیم بر \lr{\mbox{n-1}} نتیجه می‌دهد \lr{E[S\textsuperscript{2}] = \ensuremath{\sigma}\textsuperscript{2}}. از نظر شهودی: ما از \lr{\mbox{\ensuremath{\bar{X}}}} همان داده استفاده می‌کنیم (نه \lr{\mbox{\ensuremath{\mu}}} واقعی)، بنابراین انحراف‌ها از \lr{\mbox{\ensuremath{\bar{X}}}} به‌طور مصنوعی کوچک‌اند. یک درجه آزادی «صرف» برآورد میانگین می‌شود.

\end{PSbox}

\begin{PSbox}[unbreakable]{prob}{مسیله 3}

یک مهندس توان نویز را از 20 نمونه اندازه‌گیری می‌کند و \lr{\mbox{S\textsuperscript{2} = 0.045 V\textsuperscript{2}}} به دست می‌آید. واریانس واقعی \lr{\ensuremath{\sigma}\textsuperscript{2} = 0.04 V\textsuperscript{2}} است. آیا این نگران‌کننده است؟ برای ارزیابی از توزیع نمونه‌گیری استفاده کنید.

\PSsolution{} تحت \lr{\mbox{H\textsubscript{0}}}: \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}=0.04}}، داریم \lr{(n-1)S\textsuperscript{2}/\ensuremath{\sigma}\textsuperscript{2} \textasciitilde{} \ensuremath{\chi}\textsuperscript{2}(19)}. مقدار مشاهده‌شده: \lr{\mbox{19(0.045)/0.04 = 21.375}}. \lr{P(\ensuremath{\chi}\textsuperscript{2}(19) \textgreater{} 21.375) \ensuremath{\approx} 0.31}. این غیرمعمول نیست \lr{—} حدود 31\% از نمونه‌ها هنگامی که \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}=0.04}} است، \lr{\mbox{S\textsuperscript{2} \ensuremath{\ge} 0.045}} می‌دهند. نگران‌کننده نیست.
