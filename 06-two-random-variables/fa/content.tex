...
برای متغیرهای تصادفی پیوسته \lr{X}، \lr{Y}، \lr{PDF} مشترک \lr{f\textsubscript{X,Y}(x,y)} در رابطه زیر صدق می‌کند:\PSdisplay{P((X,Y) \in A) = \iint_A f_{X,Y}(x,y) \, dx \, dy}
...
\lr{PDF} مشترک یک \textbf{رویه} روی صفحه \lr{(x,y)} است. احتمال = حجم زیر رویه روی یک ناحیه.
...
با توجه به \lr{PMF} مشترک در جدول بخش \PSnum{6.2}، موارد زیر را بیابید: \lr{(a) P(X\ensuremath{\le}1, Y=2)}، \lr{(b) f\_\{X\textbar{}Y\}(x\textbar{}Y=1)}، \lr{(c) E[X\textbar{}Y=1]}.
...
\emph{ماژول بعدی: {ماژول \lr{7} \lr{—} توابع دو متغیر تصادفی}}\PSbr{}
