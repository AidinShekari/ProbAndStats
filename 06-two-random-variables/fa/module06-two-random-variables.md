...
**مثال:** X = تعداد ارسال‌های مجدد (0,1,2)، Y = دسته تأخیر تأیید (1=سریع، 2=کند)
...
دو ولتاژ نویز همبسته (X, Y) با همبستگی ρ:
...
PDF حاشیه‌ای f_X(x) همان **تصویر** (انتگرال) رویه مشترک روی محور x است. به‌همین‌ترتیب f_Y(y) روی محور y تصویر می‌شود.

### مثال مهندسی

PDF مشترک: f_{X,Y}(x,y) = 2e^(-x)e^(-2y)، x ≥ 0، y ≥ 0
...
- اگر کانال خوب باشد (X=1): Y ~ N(10, 1)
- اگر کانال ضعیف باشد (X=0): Y ~ N(2, 4)
- P(X=1) = 0.7، P(X=0) = 0.3
...
(یا به‌طور معادل برای PMF‌ها: p_{X,Y}(x,y) = p_X(x)·p_Y(y))

### پیامدهای استقلال
- f_{X|Y}(x|y) = f_X(x) (دانستن Y مقدار X را تغییر نمی‌دهد)
- E[XY] = E[X]E[Y]
- Var(X + Y) = Var(X) + Var(Y)

### آزمون استقلال
بررسی کنید: آیا توزیع مشترک به حاصل‌ضرب یک تابع فقط از x و یک تابع فقط از y تجزیه می‌شود؟
- f_{X,Y}(x,y) = 2e^(-x)·e^(-2y) = [e^(-x)]·[2e^(-2y)] ✓ مستقل!
- f_{X,Y}(x,y) = x + y برای 0≤x,y≤1 → تجزیه نمی‌شود → مستقل نیست
...
E[X|Y=y] **بهترین پیش‌بینی** X است به‌شرط این‌که Y = y را مشاهده کرده‌ایم (به معنای میانگین مربعات).

### مثال مهندسی: مقاومت وابسته به دما

مقدار یک مقاومت R به دما T وابسته است. اگر R|T ~ N(1000 + 0.5T, 4):
...
### مسئله 1
PDF مشترک: f_{X,Y}(x,y) = 6(1-y) برای 0 ≤ x ≤ y ≤ 1 و در جای دیگر صفر.
(a) نرمال‌سازی را بررسی کنید. (b) حاشیه‌ای‌ها را بیابید. (c) آیا X و Y مستقل هستند؟ (d) P(X < 0.5, Y < 0.5) را بیابید.

**حل:**
(a) ∫₀¹ ∫₀ʸ 6(1-y) dx dy = ∫₀¹ 6y(1-y) dy = 6[y²/2 - y³/3]₀¹ = 6(1/2 - 1/3) = 1 ✓
(b) f_X(x) = ∫ₓ¹ 6(1-y) dy = 6[(1-y²/2) - (y)]ₓ¹ = 3(1-x)²؛ f_Y(y) = 6y(1-y)
(c) f_{X,Y} ≠ f_X · f_Y → مستقل نیستند (همچنین تکیه‌گاه مثلثی است، نه مستطیلی)
(d) P(X<0.5, Y<0.5) = ∫₀^0.5 ∫₀^y 6(1-y) dx dy = ∫₀^0.5 6y(1-y) dy = 6[y²/2 - y³/3]₀^0.5 = 0.500

### مسئله 2
دو حسگر یک سیگنال را با نویز مستقل اندازه‌گیری می‌کنند. X = S + N₁، Y = S + N₂ که در آن S=5V، N₁~N(0,1)، N₂~N(0,4). آیا X و Y مستقل هستند؟ E[X]، E[Y] را بیابید و توضیح دهید چرا Cov(X,Y) ≠ 0.

**حل:** X و Y مستقل نیستند زیرا در S مشترک‌اند (هرچند نویز مستقل است). E[X] = E[Y] = 5. Cov(X,Y) = Cov(S+N₁, S+N₂) = Var(S) + Cov(S,N₁) + Cov(N₂,S) + Cov(N₁,N₂). چون S ثابت است، Var(S)=0، بنابراین اگر S تصادفی بود، آن‌ها همبسته می‌شدند. با S ثابت، X و Y در این حالت در واقع مستقل هستند. اگر S تصادفی با Var(S) = σ_S² باشد، آنگاه Cov(X,Y) = σ_S².

### مسئله 3
با توجه به PMF مشترک در جدول بخش 6.2، موارد زیر را بیابید: (a) P(X≤1, Y=2)، (b) f_{X|Y}(x|Y=1)، (c) E[X|Y=1].
