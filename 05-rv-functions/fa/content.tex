...
خروجی حسگر \lr{V = 0.01\ensuremath{\cdot}T + 0.5} (\lr{V} بر حسب ولت، \lr{T} بر حسب \lr{\ensuremath{^{\circ}}C})
...
اگر \lr{g} اکیداً صعودی با وارون \lr{g\textsuperscript{-1}} باشد:\PSdisplay{f_Y(y) = f_X(g^{-1}(y)) \cdot \frac{d}{dy}[g^{-1}(y)]}
...
که در آن \lr{x\textsubscript{1}, x\textsubscript{2}}, \lr{…} تمام ریشه‌های \lr{g(x) = y} هستند.
...
ورودی \lr{X \textasciitilde{} N(0, \ensuremath{\sigma}\textsuperscript{2})}. خروجی \lr{Y = \textbar{}X}\lr{\textbar{}}.

برای \lr{y \textgreater{} 0: x = +y} و \lr{x = -y} جواب هستند. \lr{\textbar{}}\lr{g’(x)\textbar{} = 1}.
...
\item
  ژاکوبی را محاسبه کنید: \lr{J = \textbar{}dx/dy\textbar{} = \textbar{}d[g\textsuperscript{-1}(y)]/dy}\lr{\textbar{}}
\item
  اعمال کنید: \lr{f\textsubscript{Y}(y) = f\textsubscript{X}(g\textsuperscript{-1}(y)) \ensuremath{\cdot} \textbar{}J}\lr{\textbar{}}
\end{enumerate}
...
\item
  ژاکوبی: \lr{\textbar{}}\lr{\ensuremath{\partial}(I,Q)/\ensuremath{\partial}(R,\ensuremath{\theta})\textbar{} = R}
\end{itemize}
...
\lr{PDF: L} از 0 تا \lr{20 dB} تغییر می‌کند. \lr{g\textsuperscript{-1}(l) = 10\textasciicircum{}(l/20)}، \lr{\textbar{}}\lr{dg\textsuperscript{-1}/dl\textbar{} = (ln10/20)\ensuremath{\cdot}10\textsuperscript{l/20}}.\PSbr{}\lr{f\textsubscript{L}(l) = (1/9)\ensuremath{\cdot}(ln10/20)\ensuremath{\cdot}10\textsuperscript{l/20}} برای \lr{0 \ensuremath{\le} l \ensuremath{\le} 20}.
...
\emph{ماژول بعدی: {ماژول 6 \lr{—} دو متغیر تصادفی}}\PSbr{}
