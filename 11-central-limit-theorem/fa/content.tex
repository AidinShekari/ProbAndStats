...
برای \lr{\mbox{X\textsubscript{1}}}، \lr{\mbox{X\textsubscript{2}}}، \lr{…}، \lr{\mbox{X\textsubscript{n}}} مستقل و هم‌توزیع \lr{\mbox{(i.i.d.)}} با میانگین \lr{\mbox{\ensuremath{\mu}}} و واریانس متناهی \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}}:

\PSdisplay{\bar{X}_n = \frac{1}{n}\sum_{i=1}^n X_i \xrightarrow{P} \mu \quad \PSmtext{as } n \to \infty}

(«همگرایی در احتمال»: \lr{P(\textbar{}\ensuremath{\bar{X}}\textsubscript{n} - \ensuremath{\mu}\textbar{} \textgreater{} \ensuremath{\varepsilon}) \ensuremath{\to} 0} برای هر \lr{\mbox{\ensuremath{\varepsilon} \textgreater{} 0}})
...
برای اطمینان 95\% از این‌که خطا \lr{\textless{}} \lr{\mbox{\ensuremath{\varepsilon}}} باشد: به \lr{n \ensuremath{\ge} \ensuremath{\sigma}\textsuperscript{2}/(0.05 \ensuremath{\cdot} \ensuremath{\varepsilon}\textsuperscript{2})} نیاز است
...
برای \lr{\mbox{n}} بزرگ:\PSdisplay{\bar{X}_n \approx N\left(\mu, \frac{\sigma^2}{n}\right)}
...
\tightlist
\item
  \lr{\mbox{X\textsubscript{1}}}، \lr{…}، \lr{\mbox{X\textsubscript{n}}} مستقل هستند
\item
  هم‌توزیع هستند (با \lr{\mbox{CLT}} لیپانوف/لینده‌برگ می‌توان این شرط را سست کرد)
\item
  میانگین متناهی \lr{\mbox{\ensuremath{\mu}}} و واریانس متناهی \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}}}
\item
  \lr{\mbox{n}} «به‌قدر کافی بزرگ» است (قاعده سرانگشتی: \lr{\mbox{n \ensuremath{\ge} 30}}، اما به شکل توزیع بستگی دارد)
\end{enumerate}
...
\tightlist
\item
  نیاز ندارد \lr{\mbox{X\textsubscript{i}}} گاوسی باشد
\item
  نیاز ندارد \lr{\mbox{X\textsubscript{i}}} پیوسته باشد
\item
...
\toprule\noalign{}
\rowcolor{PSnavy}\PSth{توزیع اصلی} & \PSth{\lr{\mbox{n}} لازم برای تقریب خوب} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
متقارن (یکنواخت) & \lr{\mbox{n \ensuremath{\approx} 5-10}} \\
کمی چوله & \lr{\mbox{n \ensuremath{\approx} 20-30}} \\
به‌شدت چوله (نمایی) & \lr{\mbox{n \ensuremath{\approx} 30-50}} \\
به‌غایت چوله & \lr{\mbox{n \ensuremath{\approx} 50-100+}} \\
\end{longtable}
}
...
\tightlist
\item
  \lr{\mbox{n=1}}: یکنواخت (مسطح)
\item
  \lr{\mbox{n=2}}: مثلثی
\item
  \lr{\mbox{n=3}}: شروع به زنگوله‌ای‌شدن می‌کند
\item
  \lr{\mbox{n=5}}: تقریباً گاوسی
\item
  \lr{\mbox{n=12}}: عملاً از گاوسی قابل تفکیک نیست
\end{itemize}
...
\tightlist
\item
  \lr{\mbox{n=1}}: نمایی (به‌شدت چوله به راست)
\item
  \lr{\mbox{n=3}}: همچنان چوله اما کمتر \lr{\mbox{(Gamma(3))}}
\item
  \lr{\mbox{n=10}}: تقریباً متقارن، زنگوله‌ای
\item
  \lr{\mbox{n=30}}: اساساً گاوسی
\end{itemize}

\PSsubsection{دوجمله‌ای \lr{\mbox{\ensuremath{\to}}} تقریب نرمال}

\lr{\mbox{Binomial(n, p) \ensuremath{\approx} N(np, np(1-p))}} برای \lr{\mbox{n}} بزرگ
...
در یک شبکه سلولی، تداخل کل = جمع سیگنال‌های بسیاری از کاربران مستقل. \lr{\mbox{CLT \ensuremath{\to}}} تداخل \lr{\mbox{\ensuremath{\approx}}} گاوسی (برای کاربران زیاد).

\PSsubsection{چرا خطاهای اندازه‌گیری گاوسی هستند}

خطای کل = جمع بسیاری از منابع خطای کوچک و مستقل (کالیبراسیون، کوانتیزاسیون، گرمایی، لرزش و \lr{…}) \lr{\mbox{\ensuremath{\to} CLT \ensuremath{\to}}} خطا \lr{\mbox{\ensuremath{\approx}}} گاوسی.

\PSsubsection{چرا تغییرات ساخت گاوسی هستند}

مقدار مقاومت = مقدار نامی + جمع بسیاری از تغییرات کوچک فرایند (دوپینگ، حکاکی، دما و \lr{…}) \lr{\mbox{\ensuremath{\to} CLT \ensuremath{\to}}} مقدار \lr{\mbox{\ensuremath{\approx}}} گاوسی حول مقدار نامی.
...
برای تقریب \lr{\mbox{P(S\textsubscript{n} \ensuremath{\le} x)}} که در آن \lr{S\textsubscript{n} = X\textsubscript{1} + … + X\textsubscript{n}}:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \lr{\mbox{\ensuremath{\mu} = E[X\textsubscript{i}]}}، \lr{\ensuremath{\sigma}\textsuperscript{2} = Var(X\textsubscript{i})} را محاسبه کنید
\item
...
1000 بسته، احتمال خطای هر یک \PSnum{0.02}. \lr{\mbox{X}} = مجموع خطاها.\PSbr{}دقیق: \lr{\mbox{X \textasciitilde{} Binomial(1000, 0.02)}}\PSbr{}تقریب \lr{\mbox{CLT}}: \lr{\mbox{X \ensuremath{\approx} N(20, 19.6)}}
...
یک کارخانه پیچ تولید می‌کند. طول \lr{\mbox{X}} \lr{\textasciitilde{}} توزیعی با \lr{\mbox{\ensuremath{\mu} = 10cm}} و \lr{\mbox{\ensuremath{\sigma} = 0.2cm}}. یک دسته 100 پیچی اندازه‌گیری می‌شود.

\begin{enumerate}
\def\labelenumi{(\PSalph{enumi})}
\tightlist
\item
  توزیع تقریبی \lr{\mbox{\ensuremath{\bar{X}}}} چیست؟ (ب) \lr{\mbox{P(\ensuremath{\bar{X}} \textgreater{} 10.05)}}؟ (ج) \lr{\mbox{P(9.96 \textless{} \ensuremath{\bar{X}} \textless{} 10.04)}}؟
\end{enumerate}
...
\tightlist
\item
  \lr{\ensuremath{\bar{X}} \textasciitilde{} N(10, 0.04/100) = N(10, 0.0004)}، \lr{\mbox{\ensuremath{\sigma}\textsubscript{X}̄ = 0.02}}
\item
...
یک لینک مخابراتی 10000 بیت با \lr{\mbox{BER = 0.001}} دارد. با استفاده از \lr{\mbox{CLT}} مقدار \lr{\mbox{P(\rl{بیش از 15 خطا})}} را تقریب بزنید.

\PSsolution{} \lr{\mbox{X \textasciitilde{} Binomial(10000, 0.001)}}. \lr{\mbox{\ensuremath{\mu}=10}}، \lr{\mbox{\ensuremath{\sigma}\textsuperscript{2}=9.99}}.\PSbr{}\lr{P(X \textgreater{} 15) \ensuremath{\approx} 1 - \ensuremath{\Phi}((15.5-10)/\ensuremath{\surd}9.99) = 1 - \ensuremath{\Phi}(1.74) = 0.0409}

\end{PSbox}

\begin{PSbox}[unbreakable]{prob}{مسیله 3}

کد \lr{\mbox{MATLAB}} را برای بررسی \lr{\mbox{CLT}} برای توزیع کای‌دو(1) (بسیار چوله) بنویسید و همگرایی را برای \lr{\mbox{n=2,5,10,50}} نشان دهید.
