...
برای X₁, X₂, ..., Xₙ مستقل و هم‌توزیع (i.i.d.) با میانگین μ و واریانس متناهی σ²:

$$\bar{X}_n = \frac{1}{n}\sum_{i=1}^n X_i \xrightarrow{P} \mu \quad \text{as } n \to \infty$$

(«همگرایی در احتمال»: P(|X̄ₙ - μ| > ε) → 0 برای هر ε > 0)

### قانون قوی اعداد بزرگ

$$\bar{X}_n \xrightarrow{a.s.} \mu \quad \text{as } n \to \infty$$

(«همگرایی تقریباً مطمئن»: P(X̄ₙ → μ) = 1)
...
بر اساس چبیشف: P(|X̄ₙ - μ| > ε) ≤ σ²/(nε²)

برای اطمینان 95% از این‌که خطا < ε باشد: به n ≥ σ²/(0.05 · ε²) نیاز است
...
3. میانگین متناهی μ و واریانس متناهی σ²
4. n «به‌قدر کافی بزرگ» است (قاعده سرانگشتی: n ≥ 30، اما به شکل توزیع بستگی دارد)

### آنچه CLT نیاز ندارد
- نیاز ندارد Xᵢ گاوسی باشد
- نیاز ندارد Xᵢ پیوسته باشد
...
|----------------------|--------------------------------|
| متقارن (یکنواخت) | n ≈ 5-10 |
| کمی چوله | n ≈ 20-30 |
| به‌شدت چوله (نمایی) | n ≈ 30-50 |
| به‌غایت چوله | n ≈ 50-100+ |
...
- n=3: همچنان چوله اما کمتر (Gamma(3))
- n=10: تقریباً متقارن، زنگوله‌ای
- n=30: اساساً گاوسی

### دوجمله‌ای → تقریب نرمال

Binomial(n, p) ≈ N(np, np(1-p)) برای n بزرگ

این همان CLT اعمال‌شده به جمع آزمایش‌های برنولی است!
- تصحیح پیوستگی: P(X ≤ k) ≈ Φ((k + 0.5 - np)/(√(np(1-p))))
...
در یک شبکه سلولی، تداخل کل = جمع سیگنال‌های بسیاری از کاربران مستقل. CLT → تداخل ≈ گاوسی (برای کاربران زیاد).

### چرا خطاهای اندازه‌گیری گاوسی هستند

خطای کل = جمع بسیاری از منابع خطای کوچک و مستقل (کالیبراسیون، کوانتیزاسیون، گرمایی، لرزش و ...) → CLT → خطا ≈ گاوسی.

### چرا تغییرات ساخت گاوسی هستند

مقدار مقاومت = مقدار نامی + جمع بسیاری از تغییرات کوچک فرایند (دوپینگ، حکاکی، دما و ...) → CLT → مقدار ≈ گاوسی حول مقدار نامی.
...
برای تقریب P(Sₙ ≤ x) که در آن Sₙ = X₁ + ... + Xₙ:
1. μ = E[Xᵢ]، σ² = Var(Xᵢ) را محاسبه کنید
2. Sₙ ≈ N(nμ, nσ²)
3. P(Sₙ ≤ x) ≈ Φ((x - nμ)/(σ√n))
...
دقیق: X ~ Binomial(1000, 0.02)
تقریب CLT: X ≈ N(20, 19.6)
...
### مسئله 1
یک کارخانه پیچ تولید می‌کند. طول X ~ توزیعی با μ = 10cm و σ = 0.2cm. یک دسته 100 پیچی اندازه‌گیری می‌شود.
(a) توزیع تقریبی X̄ چیست؟ (ب) P(X̄ > 10.05)؟ (ج) P(9.96 < X̄ < 10.04)؟
...
### مسئله 2
یک لینک مخابراتی 10000 بیت با BER = 0.001 دارد. با استفاده از CLT مقدار P(بیش از 15 خطا) را تقریب بزنید.

**حل:** X ~ Binomial(10000, 0.001). μ=10، σ²=9.99.
